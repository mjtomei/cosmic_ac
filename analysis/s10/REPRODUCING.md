...
- **Rewritten bypass variants** — no longer true of the scored sample, as of
  2026-09-13. `bypass_rewrite.py` seeds only its choice of in-context examples,
  not the model's sampling, so the variants cannot be regenerated by anyone
  including us; that is why the 341 that were scored are now committed as text
  in `detector_bench_input.jsonl` and `detector_bench_scored_input.jsonl`
  (with the 65 unmodified originals they derive from). The wider `bypass_text/`
  working store stays gitignored as the unpublished remainder. See
  `DATA-NOTICE.md`.
...
## Pooled segments ship as hashes, not text (2026-09-13)

`detector_bench_controls.jsonl` (1,255 pre-2022 controls) and
`flagged_hits_pool.json` (the estimator's 316 hits) carry `text_sha256` and
`n_chars` instead of the segment text. Between them they span twenty chambers,
three of which reserve Hansard reuse to the Speaker or Clerk (BC, VIC, QLD) and
four of which publish no terms at all — see `SOURCE-LICENCES.md`.

Resolve them exactly as this document describes for any other `seg_id`, then
compare the SHA-256: a match proves you hold the string that was scored. Neither
file is an input to any analysis — `flagged_hits_bound.py` never reads the text
field and `detector_transfer.py` reads only the scores CSVs — so this costs
nothing downstream. `build_text_pointers.py --check` re-verifies the hashes
against local text.
