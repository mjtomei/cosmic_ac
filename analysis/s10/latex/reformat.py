...
def figure_ref_anchors(t):
    ...
    # that named them by position now names them by label.
    (r"is\s+the\s+first\s+table\s+of\s+Appendix",
     r"is \\cref{tab:elements-levels} in Appendix"),
    (r"\(the\s+second\s+table\s+of\s+Appendix",
     r"(\\cref{tab:elements-components} in Appendix"),
    ]
    for a, b in subs:
...
TABLE_CAPS = [
 ...
 ("term & alone & joint", "tab:four-predictors-occ", "The occupational blocks of the same simultaneous fit — both altitude ladders and Indoors — each alone and in the joint model with the categorical blocks (n = 3{,}631); t in parentheses. Their terms are per standard deviation, so the last column converts jointly significant effects into years of cohort per standard deviation."),
...
COLSPECS = {
 ...
 # seven columns: tighten the intercolumn glue and wrap the wide cells, or the
 # \texttt{} header and the two bracketed intervals push it off the measure.
 "tab:bypass-yield":   (r"@{}>{\raggedright\arraybackslash}p{0.175\linewidth}@{\hspace{6pt}}r"
                        r"@{\hspace{6pt}}r@{\hspace{6pt}}r@{\hspace{3pt}}"
                        r">{\raggedright\arraybackslash}p{0.115\linewidth}@{\hspace{6pt}}"
