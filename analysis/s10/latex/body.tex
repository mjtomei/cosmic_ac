...
AI-text detector (Pangram), calibrated against each chamber\textquotesingle s own pre-2022
record (specificity 1,260/1,260), and a detector-independent lexical
\emph{register} built from the vocabulary language models overuse. In current
speech, \textbf{9.03\% of words} are machine-drafted --- a conservative floor --- with an
elevenfold spread across chambers and a concentration in scripted genres. The
register, however, \textbf{predates the models by decades}: its rise begins in
...
\textbf{S10 --- draft write-up, 2026-08-11; §4.6a and Appendix A extended
...
\textbf{§4.6a\textquotesingle s literature citations were web-verified 2026-08-24.} The result is
...
give the script and invocation. Two of the nine (§4.6\textquotesingle s cohort share, §4.7\textquotesingle s
...
\hypertarget{1-the-question-and-why-it-needs-two-instruments}{%
\section{1. The question, and why it needs two instruments}\label{1-the-question-and-why-it-needs-two-instruments}}
...
\tightlist
\item
  \textbf{Prevalence} --- how much text in the record was drafted by a machine. This
  needs a calibrated detector.
\item
  \textbf{Permeation} --- whether human speech is drifting toward machine register
  independent of drafting. This needs instruments that do \emph{not} depend on a
...
and reproducible (\texttt{build\_pangram\_expansion.py}, \texttt{build\_shortband.py}).

\hypertarget{21-two-contamination-hazards-both-found-by-looking}{%
\subsection{2.1 Two contamination hazards, both found by looking}\label{21-two-contamination-hazards-both-found-by-looking}}
...
\texttt{transcript\_regime\_check.py} measures two markers that track editorial
...
\item
  \textbf{Tasmania} --- named by nobody --- moved: contraction density \textbf{3.4 → 15.9
...
\hypertarget{3-method}{%
\section{3. Method}\label{3-method}}

\hypertarget{31-the-calibration-that-carries-the-argument}{%
\subsection{3.1 The calibration that carries the argument}\label{31-the-calibration-that-carries-the-argument}}

Prevalence is only meaningful against a measured false-positive rate. \textbf{A
chamber\textquotesingle s false-positive rate depends on its own editorial register, so
...
With specificity \texttt{Sp} and sensitivity \texttt{Se}, an observed flag rate \texttt{π} relates
to true prevalence \texttt{τ} by Rogan--Gladen:

\begin{verbatim}
π = τ·Se + (1−τ)(1−Sp)      ⇒     τ = (π − (1−Sp)) / (Se − (1−Sp))
\end{verbatim}

When \texttt{Sp\ =\ 1} this collapses to \texttt{τ\ =\ π/Se}. \textbf{\texttt{Se} is not estimated.} Since a
detector\textquotesingle s sensitivity cannot exceed 1, \texttt{τ\ =\ π/Se\ ≥\ π}, so with \texttt{Sp\ =\ 1}
...
\hypertarget{32-detector-and-a-defect-worth-recording}{%
\subsection{3.2 Detector, and a defect worth recording}\label{32-detector-and-a-defect-worth-recording}}
...
\texttt{model:\ "pangram-4"} gave \textbf{20/20 exact agreement including all six Mixed}
(\texttt{api\_route\_check.py}).
...
\hypertarget{33-why-the-frequency-arm-is-descriptive-not-inferential}{%
\subsection{3.3 Why the frequency arm is descriptive, not inferential}\label{33-why-the-frequency-arm-is-descriptive-not-inferential}}

This belongs in methods rather than an appendix, because it explains the
framing of §4.5.

The Kobak et al. excess-vocabulary approach (arXiv:2406.07016; \emph{Sci Adv}
...
\textbf{It has no trend control.} \texttt{in\_time\_placebo.py} runs the identical
estimator on \emph{pre-LLM} window pairs, where the true effect is zero by
...
\hypertarget{34-instruments-retired-and-why}{%
\subsection{3.4 Instruments retired, and why}\label{34-instruments-retired-and-why}}
...
\hypertarget{41-calibration-1260--1260}{%
\subsection{4.1 Calibration: 1,260 / 1,260}\label{41-calibration-1260--1260}}
...
estimated from our own data: with \texttt{Sp\ =\ 1} and any real detector\textquotesingle s \texttt{Se\ ≤\ 1},
the observed flag rate is a \textbf{conservative floor} on true prevalence, so every
§4.2 figure is if anything an underestimate of machine text. Using \texttt{Se\ =\ 1}
...
Sensitivity on \emph{edited} machine text is the real unknown and is discussed in §5.
(\texttt{prevalence\_report.py})
...
(\texttt{nb\_p3\_vs\_p4.py})

\hypertarget{42-prevalence-90-of-words-with-an-elevenfold-spread}{%
\subsection{4.2 Prevalence: 9.0\% of words, with an elevenfold spread}\label{42-prevalence-90-of-words-with-an-elevenfold-spread}}
...
alternative would suppress the rate.} \texttt{segment.py} packs speaker turns into
...
individually in the table below, and their \textbf{equal-weight mean matches the
word-weighted pooled rate} (9.12\% against the 9.03\% headline;
\texttt{banded\_prevalence.py}), so no single large chamber drives the headline.
...
\textbf{217 are full AI verdicts and 147 (40\%) are Mixed} --- Pangram reporting that
part of the segment is human. Counting a Mixed segment\textquotesingle s words as fully machine
...
count.\footnote{Fractions live in \texttt{fraction\_ai\_harvested.json} (151 harvested
  individually) and in the \texttt{fraction\_ai} column of
  \texttt{pangram\_p4\_verdicts.csv} (1,431 recorded by the API). The 154 AI verdicts
  ...
  mixed-format business in the middle of §4.3\textquotesingle s genre ladder.}
...
\bottomrule\noalign{}
\endlastfoot
NSW & \textbf{19.8\%} & {[}13.2, 26.9{]} & & IE & 7.5\% & {[}3.6, 12.2{]} \\
...
AB & 7.8\% & {[}3.9, 12.2{]} & & \textbf{US Senate} & \textbf{1.8\%} & {[}0.1, 4.1{]} \\
\end{longtable}
...
segment counts cannot express that.\footnote{\texttt{python\ -c\ "import\ banded\_prevalence\ as\ B;\ B.table(B.load())"}.}
...
to the headline and neither makes nor breaks it.\footnote{\texttt{pangram\_ch\_verdicts.csv}, rows \texttt{caprev*}/\texttt{cactl*}. Its short band
  ...
  (\texttt{build\_cafed\_ctl\_redraw.py}); all seven replacements scored Human, as had
...
only thing that changed.\footnote{\texttt{python\ build\_mb\_redraw.py}, then \texttt{pangram\_mb\_redraw\_verdicts.csv}.
...
\hypertarget{43-genre-drafting-concentrates-in-scripted-business}{%
\subsection{4.3 Genre: drafting concentrates in scripted business}\label{43-genre-drafting-concentrates-in-scripted-business}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{SO31} --- one-minute scripted set-pieces & \textbf{32.3\%} & {[}21.0, 44.1{]} \\
Government Orders --- mixed prepared and spontaneous & 19.9\% & {[}10.4, 30.3{]} \\
\textbf{Oral Questions} --- not on notice & \textbf{9.8\%} & {[}2.3, 19.2{]} \\
all three pre-AI controls (0/180) & \textbf{0.0\%} & {[}0.0, 2.1{]} \\
\end{longtable}

\textbf{SO31 vs OQ: 3.29×, +22.5pp, permutation p = 0.0025.}
...
unchanged and \textbf{the gap settles at 3.29×}. The test is a label permutation rather than
Fisher exact, because Fisher takes segment counts and the estimator is a ratio
of summed words.\footnote{\texttt{python\ prevalence\_report.py} for the rates and bootstrap intervals;
...
of 60) gives z = 3.70, p = 2.2 × 10⁻⁴.\footnote{\texttt{python\ prevalence\_report.py} for the rates and bootstrap intervals;
  the SO31-vs-OQ permutation is 50,000 label shuffles of the pooled
  prevalence segments. Reported here rather than the old Fisher p = 0.00034,
  which tested a quantity the study no longer reports. The same script prints
  the Cochran--Armitage trend statistic on the segment counts (doses
  OQ \textless{} DEBATE \textless{} SO31); the adjacent-rung Fisher exacts are computed from the
  3×2 segment table.} The two adjacent steps are each
...
Not one of the five flagged segments is spontaneous exchange.\footnote{\texttt{python\ genre\_oq\_audit.py}, which prints the five flags with their
...
(§5), which cuts the same way: the filter keeps Oral Questions\textquotesingle{} longer segments,
so the surviving cell is biased \emph{up}, and true Question-Period exchange sits
lower still.

\hypertarget{44-the-opus-screen-tracks-pangram}{%
\subsection{4.4 The Opus screen tracks Pangram}\label{44-the-opus-screen-tracks-pangram}}
...
overlap, and it is a cheap stratifier for future work.\footnote{\texttt{python\ opus\_screen\_auc.py}. The deployed screen is
...
detectors and does nothing for a strong one --- consistent with §3.4\textquotesingle s finding
that reasoning never closes the \textasciitilde0.25 AUC frontier gap, but sharper: the
...
tokens (\textasciitilde2× all-in)** with no loss. (\texttt{opus\_effort\_ab.py}, \texttt{opus\_effort\_ab.csv})

\hypertarget{45-the-register-shift-starts-in-199496-decades-before-the-machines}{%
\subsection{4.5 The register shift starts in 1994--96, decades before the machines}\label{45-the-register-shift-starts-in-199496-decades-before-the-machines}}

Descriptive series (§3.3), UK Commons extended back to 1985. The register
\emph{declines} through the late 1980s and turns upward around \textbf{1994--96} ---
before the consumer web, and long before any language model. Whatever this
measures, LLMs did not start it.\footnote{\texttt{python\ long\_trend.py\ -\/-seg\ uk/segments\_uk\_deep.jsonl} for the annual
  ...
  1994. A day-clustered bootstrap (\texttt{python\ long\_trend\_bootstrap.py}, 2,000
...
\hypertarget{45a-the-climb-is-everywhere-except-the-united-states}{%
\subsection{4.5a The climb is everywhere except the United States}\label{45a-the-climb-is-everywhere-except-the-united-states}}
...
measured with the same ruler.\footnote{\texttt{python\ occurrence\_trends.py\ -\/-build\ -\/-report}. Word-weighted, pooled
...
\bottomrule\noalign{}
\endlastfoot
UK Commons & 380 (1994) & \textbf{1,444} (2026) & \textbf{×3.80} & rises \\
Ireland & 1,603 (2018) & \textbf{1,975} (2026) & ×1.23 & rises \\
CA federal & 1,865 (2015) & 1,735 (2026) & ×0.93 & flat \\
\textbf{US House} & \textbf{1,515} (1994) & \textbf{1,709} (2026) & \textbf{×1.13} & \textbf{flat} \\
\textbf{US Senate} & \textbf{1,314} (1994) & \textbf{1,562} (2026) & \textbf{×1.19} & \textbf{flat} \\
\end{longtable}

The US chambers are flat across thirty-two years, and they start \emph{above} where
the UK still sits two decades later: US House 1,515 in 1994 against the UK\textquotesingle s
...
got there first and stopped". The 1994 values settle it.\footnote{\texttt{python\ us/fetch\_us\_hist.py\ -\/-years\ 1994-2005,2023\ -\/-per-year\ 60\ -\/-tag\ US\_DEEP\_DONE}, then re-extract with \texttt{us/us\_extract.py\ zips\ segments\_us.jsonl} and rebuild. 780 sitting-day zips at 60 days/year;
...
apparently-above to clearly-below once corrected.\footnote{\texttt{python\ convergence\_check.py}, with shares from
  \texttt{ai\_share\_of\_instrument.py}. Machine-written text carries the instrument at
...
\textbf{A caveat that constrains how this can be read.} The instrument is Kobak et
al.\textquotesingle s list, derived from PubMed abstracts --- American scientific English. Chamber
\emph{levels} are therefore partly definitional: a US-derived yardstick will score US
speech high whatever is happening. The defensible comparison is the \emph{within}-chamber
...
specifically:\footnote{\texttt{python\ alignment\_vs\_american.py}. Three families at 1,600 prompts;
...
\hypertarget{46-a-generational-gradient-net-of-calendar-drift}{%
\subsection{4.6 A generational gradient, net of calendar drift}\label{46-a-generational-gradient-net-of-calendar-drift}}

Every member-year of legislative speech carries two time-stamps: the year the
words were \textbf{spoken} (calendar period) and the speaker\textquotesingle s \textbf{birth year}
...
\item
  ...
  effects (all 22 chambers, 61,312 member-years): birth \textbf{+0.88 per 1,000 words
  per decade} (t = 33.8) against spoken year \textbf{+1.25 per decade} (t = 29.2).
  A legislator born a decade later uses about +0.88/1,000 more of the register
  \emph{in the same year and chamber}. †\footnote{\texttt{python\ cohort\_vs\_period.py}. Member-year register rate regressed on
...
\item
  ...
  stronger of the two in 15 of the 22 chambers. †\footnote{\texttt{python\ cohort\_vs\_period.py}. Member-year register rate regressed on
    spoken year and birth year with chamber fixed effects, word-weighted,
    HC-robust; all 22 chambers now carry member birth years, 61,312
    member-years, after the nine Australian and UK-devolved chambers were
    collected on 2026-08-17. Within-chamber correlations are on the same rows.}
\item
  ...
  which \emph{strengthen} it rather than explain it away (§4.6a). †\footnote{\texttt{python\ formation\_window.py}, clustered on member (CR1): birth-decade
...
\item
  ...
  careers} and the rest arrives with the composition of the chamber. †\footnote{\texttt{python\ cohort\_vs\_period.py}, final block. Within-member (member fixed
...
Stated conclusion: \textbf{both mechanisms operate.} Later-born cohorts arrive using
...
ministerial office: in the chambers whose record marks rank it \emph{strengthens}
...
\hypertarget{46a-class-and-the-register-jointly-significant-individually-noisy--and-education-is-not-it}{%
\subsection{4.6a Class and the register: jointly significant, individually noisy --- and education is not it}\label{46a-class-and-the-register-jointly-significant-individually-noisy--and-education-is-not-it}}

The arc of this section is a correction applied twice, recorded because both
applications were the study\textquotesingle s own committed checks. On the eight provinces,
...
(\texttt{member\_level\_estimation.py}; the reasoning for each choice is in its
...
\includegraphics{class_by_era_grouped.png}

\emph{The class profile of the register, 1995--2026. One point per class per period
...
readable as movement.} †\footnote{\texttt{python\ plot\_class\_by\_era\_grouped.py}, reading
  \texttt{class\_by\_era\_grouped.csv} (per-point means, standard errors and member
  counts); the unpooled version is \texttt{class\_by\_era.csv} / \texttt{class\_by\_era.png}.
  An earlier member-\textbf{year} aggregation of the same data appeared to show the
...
and §4.5 dates the register\textquotesingle s turn to 1994--96. We are looking at a cycle
...
\bottomrule\noalign{}
\endlastfoot
secondary & −0.281 & 234 & \textbf{−0.166 (t = −2.69)} \\
college & −0.161 & 371 & \textbf{−0.139 (t = −2.67)} \\
\textbf{bachelor} & \textbf{+0.042} & 1,194 & baseline \\
graduate & −0.025 & 1,271 & −0.008 (t = −0.22) \\
\textbf{professional} & −0.130 & 991 & \textbf{−0.091 (t = −2.29)} \\
\end{longtable}

Bachelor and graduate form a plateau at the top; both arms fall away
significantly, and the descending arm is the \textbf{professional} degree --- law and
...
model and the education block goes to \textbf{p = 0.22} while class holds at
\textbf{p = 0.0000} --- because the two instruments name the same stratum. Teachers,
...
\hypertarget{all-four-predictors-at-once}{%
\subsubsection{All four predictors at once}\label{all-four-predictors-at-once}}
...
against an estimate from a different set of members): †\footnote{\texttt{python\ joint\_predictors.py}. Canonical member-level spec --- one
  observation per legislator, equal weight, register z-scored within chamber
  against the chamber\textquotesingle s full member population, HC1 errors, joint Wald over
  each block\textquotesingle s term vector.}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{cohort} (per decade) & +0.280 (t 24.3) & \textbf{+0.270 (t 26.9)} \\
\textbf{class} (block Wald) & \textbf{p = 0.0000} & \textbf{p = 0.0000} \\
· II over I & +0.093 (t 3.02) & +0.072 (t 2.10) \\
· VIIab & −0.300 (t −2.90) & −0.299 (t −2.80) \\
\textbf{education} (block Wald) & p = 0.0025 & \textbf{p = 0.2150} \\
\textbf{prominence} (linear, \emph{not} the right form --- see D.3) & +0.035 (t 3.08) & +0.020 (t 2.05) \\
\end{longtable}

\textbf{They do not deserve equal billing.} Cohort is untouched by the others and an
...
group means within 0.1σ of each other. \textbf{The register tracks class
...
chambers. For scale: the whole class gradient spans \textasciitilde0.36σ; cohort covers
that in about thirteen years of birth date.

\hypertarget{class-the-provincial-estimates-retained-as-the-discovery-record-see-the-panel-result-above}{%
\subsubsection{Class: the provincial estimates (retained as the discovery record; see the panel result above)}\label{class-the-provincial-estimates-retained-as-the-discovery-record-see-the-panel-result-above}}
...
adjudicated (\texttt{provinces/OCCUPATION\_CODING.md}) --- gives 897 members and 5,294
...
finding and nothing.\footnote{\texttt{python\ covariate\_study.py}, \texttt{python\ class\_origin.py\ -\/-dist}. Coding
  workflow and rubric ambiguities in \texttt{provinces/OCCUPATION\_CODING.md}; the
...
\hypertarget{education-the-provincial-ladder-did-not-replicate-see-above}{%
\subsubsection{Education: the provincial ladder (did not replicate; see above)}\label{education-the-provincial-ladder-did-not-replicate-see-above}}
...
\hypertarget{the-shape-has-a-name--held-now-as-hypothesis-not-finding}{%
\subsubsection{The shape has a name --- held now as hypothesis, not finding}\label{the-shape-has-a-name--held-now-as-hypothesis-not-finding}}
...
\hypertarget{flight-class-i-avoids-the-words-that-became-common}{%
\subsubsection{Flight: class I avoids the words that became common}\label{flight-class-i-avoids-the-words-that-became-common}}
...
Class I under-uses \texttt{primary} (2.35× lift), \texttt{advocating} (1.85×), \texttt{advocates}
(1.57×), \texttt{outcomes} (1.54×); it over-uses \texttt{remarkable}, \texttt{individuals},
\texttt{broader}, \texttt{ultimately} --- words that barely moved.
...
diffusion mechanism in \emph{Der Zweck im Recht} (1883); Durkheim\textquotesingle s summary of it
...
higher-status parents once they become common.\footnote{\texttt{python\ class\_markedness.py}, \texttt{build\_class\_word\_year.py}. An earlier
  ...
  Jhering 1883 with the quote traced to Durkheim\textquotesingle s 1887 summary, Lieberson
  2000, Labov 1972); the Labov year/publisher error the file flagged is
  corrected. See \texttt{CLASS-REGISTER-LITERATURE.md}.}

\hypertarget{prominence-a-gradient-in-the-provinces-an-arc-in-the-national-chambers}{%
\subsubsection{Prominence: a gradient in the provinces, an arc in the national chambers}\label{prominence-a-gradient-in-the-provinces-an-arc-in-the-national-chambers}}
...
\emph{positions} --- use less than backbenchers (Appendix D.4). Prominence arcs in the
...
--- members whose education we know have \textbf{1.80× the median article length} of
...
\hypertarget{word-mix-the-effects-live-in-rate-not-vocabulary--and-machine-text-sits-outside-the-geometry}{%
\subsubsection{Word mix: the effects live in rate, not vocabulary --- and machine text sits outside the geometry}\label{word-mix-the-effects-live-in-rate-not-vocabulary--and-machine-text-sits-outside-the-geometry}}

A composition check (\texttt{vector\_analysis.py}, methodology and full results in
...
education and cohort correlate with the mix at \textbar r\textbar{} ≤ 0.20 --- \textbf{the standing
...
\hypertarget{limits-and-they-are-real}{%
\subsubsection{Limits, and they are real}\label{limits-and-they-are-real}}
...
\hypertarget{46b-what-the-class-shape-was-occupation-pre-registered-and-run}{%
\subsection{4.6b What the class shape was: occupation, pre-registered and run}\label{46b-what-the-class-shape-was-occupation-pre-registered-and-run}}
...
\texttt{plans/PREREG-occupational-accountability.md} is the registration with every
...
\texttt{prereg\_member\_table.json} (one join; n = 4,762 members with register and
...
\textbf{The registered four-level U is the result.} The design intent, registered
in the document\textquotesingle s hierarchy section and throughout the pre-run record
...
for all of it: §5\textquotesingle s calibration bullet --- small effects by any benchmark,
reliably estimated, structurally replicated, floors rather than
ceilings.\footnote{\texttt{python\ prereg\_stage1.py}, \texttt{prereg\_stage2.py},
  \texttt{prereg\_synthesis\_check.py}, \texttt{prereg\_covariate\_strength.py},
  \texttt{prereg\_strength\_2526.py}; results snapshots
  \texttt{prereg\_stage1\_results.txt}, \texttt{prereg\_stage2\_results.txt}. The join and
  ...
  \texttt{instrument\_final\_cells.json}.}

\hypertarget{47-the-register-is-a-post-training-artifact}{%
\subsection{4.7 The register is a post-training artifact}\label{47-the-register-is-a-post-training-artifact}}
...
families, 1,600 prompt pairs each, 1.19M base words.\footnote{Ladder stages: \texttt{python\ olmo\_ladder.py\ report}, which prints the
  end-to-end base→instruct row (\textbf{+1.2412}) alongside the three adjacent
  ...
  families: \texttt{python\ rlhf\_pref\_compile.py}, reading the generation built by
...
§4.5\textquotesingle s reading that alignment concentrates something humans already favoured ---
an association, not an identified mechanism, and the section\textquotesingle s argument no
longer rests on it. The correction
STRENGTHENS the arm\textquotesingle s design claim: \textbf{RLVR was the pre-registered placebo} ---
...
\hypertarget{47a-coverage-post-training-moves-the-models-vocabulary-onto-hansards}{%
\subsection{4.7a Coverage: post-training moves the model\textquotesingle s vocabulary onto Hansard\textquotesingle s}\label{47a-coverage-post-training-moves-the-models-vocabulary-onto-hansards}}

§4.7\textquotesingle s excess runs over Kobak\textquotesingle s 407 style words, and only about half appear in
...
At matched volume, style words by number of occurrences:\footnote{\texttt{python\ style\_word\_frequency.py}. Three families at 1,600 prompts;
...
This is §4.7\textquotesingle s claim arriving by a different route. It needs no control
...
\hypertarget{48-permeation-detector-independent-and-small-but-positive}{%
\subsection{4.8 Permeation: detector-independent and small but positive}\label{48-permeation-detector-independent-and-small-but-positive}}
...
\textbf{+0.0099, positive in 9 of 10 cells, permutation p = 0.017.} †\footnote{\texttt{python\ word\_context\_delta.py\ pooled}, which prints the pooled figure,
  the ten chamber × family cells, the clustered bootstrap and the permutation
  test. Earlier drafts cited \texttt{word\_context\_delta.py} alone, whose \texttt{report}
  ...
  3,000 draws. Per-model cell table at \texttt{METHODOLOGY.md:1009}.}
...
\hypertarget{49-quality-better-formed-not-worse-engaged--and-evadable-under-effort}{%
\subsection{4.9 Quality: better-formed, not worse-engaged --- and evadable under effort}\label{49-quality-better-formed-not-worse-engaged--and-evadable-under-effort}}

Graded against the Discourse Quality Index (Steenbergen, Bächtiger, Spörndli
\& Steiner 2003), using the original authors\textquotesingle{} own codings of a 1998 UK Commons
debate as in-context anchors. Two of the seven dimensions carry a \texttt{-1}
...
are stated separately (review item Q1). \textbf{Applicability itself collapses at
high screen scores}: respect\_demands is applicable for 61.4\% of segments
scoring \textless10, 63.0\% at 10--49, and \textbf{31.1\%} at ≥50; respect\_counterargs runs
50.8\% / 51.2\% / \textbf{28.9\%} (n = 498/297/45; \texttt{applicability\_by\_band.py}). So
the engagement null below means \emph{engages no worse when there is something to
...
sources, same answer.\footnote{Stage 1: \texttt{cd\ quality\_expansion\ \&\&\ python\ analyze.py}. Stage 2:
  \texttt{python\ analyze\_stage2.py} --- the column\textquotesingle s own script (review item Q7),
  reproducing it to three decimals. Coding disclosed at the table per the
  same item: stage 2\textquotesingle s regressor pools \textbf{Mixed verdicts with AI};
  ...
  \texttt{python\ applicability\_by\_band.py}. Per-grade id-keyed rows for both
...
respect\_groups & \textbf{+0.287} (t +2.0) ✱\footnote{The one starred cell that does not survive multiple-testing
  ...
  (Benjamini \& Hochberg, \emph{JRSS-B} 57, 1995, 289--300) leaves every other
...
\emph{The middle column is the first column times sd/100 (review item Q4): the
stage-1 regressor is a 0--100 score whose observed maximum is 70 and sd 15.3,
so the left column\textquotesingle s magnitudes are fitted 0→100 differences the data never
spans --- per sd, stage 1 sits beside stage 2\textquotesingle s binary contrast rather than 4×
...
negatively. \textbf{This supersedes "form up, deliberation down"} (Appendix B).

\textbf{Length, stated as a channel rather than adjusted away (review item Q2).}
...
machine\textquotesingle s transcripts, 2026-08-20 --- \texttt{q2\_q8\_controls.py}): with a
within-chamber log-length term, justification is \textbf{+0.290 raw → +0.157
...
retraction; \textbf{respect\_groups is length-free} (+0.220 → +0.222); common\_good
barely moves (+0.229 → +0.199, t 3.8); and evidence\textquotesingle s marginal +0.065 is
...
claim: stage 2 is nearly one-segment-per-speaker (respect\_groups t 6.1 →
5.8), and in stage 1 (418 speakers) the only casualty is the already-
quarantined respect\_groups cell (t 2.0 → 1.8).
...
figures here are the two final searches only.\footnote{\texttt{python\ bypass\_report.py} reads the verdict files and prints every
  ...
  exploratory band check. Strict re-scores in \texttt{nb\_reflip\_fractions.json},
  \texttt{bp\_reflip\_fractions.json}, \texttt{go\_reflip\_fractions.json}; the figure is
  \texttt{bypass\_figure.py}. \textbf{Reproducibility gap, stated plainly (review item
...
\includegraphics{bypass_search.png}
...
\textbf{The headline: asking a general-purpose model to rework a speech in a loop
beats the commercial evasion industry by about tenfold.} Against the 13
commercial "humanizer" services benchmarked in Pangram\textquotesingle s own technical report,
which get \textbf{2.31\%} of documents past it, this search gets \textbf{22.5\%} of
targets past it: \textbf{9.8×}. Against Pangram\textquotesingle s clean-conditions false-negative
rate of 0.34\%, it is 66×.

That is the finding. The detector is not weak --- its specificity is 0 in 1,260
(§4.1), and a single "make this sound human" prompt makes text \emph{more}
...
\textbf{per target} & \textbf{query + retry} (≤18 attempts) & \textbf{22.5\%} {[}14.4, 33.5{]} & an attacker who submits, sees the verdict, and tries again \\
\end{longtable}

Both are realistic; they bracket the exposure. The gap between them is about
\textbf{2×}, not the "threefold" an earlier draft claimed by comparing across
...
folding them in was the old \textbf{8.5\%} figure, which the previous draft kept "to
...
\begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
& searched & zero-yield & ≥1 Human & ≥1 at \texttt{fraction\_ai\ =\ 0.0} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
not from per-text outcomes (\texttt{BYPASS\_METHODOLOGY.md}).
...
one chamber\textquotesingle s rate being half or double the other\textquotesingle s (review item B6). What is
...
\textbf{Against 0 AI labels in 1,260 genuine pre-AI controls (§4.1).} That is what
...
Pangram 4, Epoch AI Style Imitation & 2.86\% & FNR, doc-level, adversarial, vendor\footnote{Corrected 2026-08-24 (review item X2): 2.86\% (17/594) is Pangram 4\textquotesingle s
  ...
  mean-accuracy metric --- a different unit --- with Pangram at \textasciitilde5.9\% miss on the
  ...
  unchanged.} \\
\textbf{this study, per variant} & \textbf{11.1\%} & \textbf{induced FNR, one blind submission (no attacker detector access) --- 4.8× the humanizers} \\
\emph{this study, per target} & \emph{22.5\%} & \emph{induced, attacker may query and retry (≤18 attempts) --- 9.8× the humanizers} \\
...
raised both bars, and by \emph{more} on the flip bar (2.8×) than on the success bar
...
estimand, needing no covariate. \textbf{Every dimension is null on both.}\footnote{\texttt{cd\ quality\_expansion\ \&\&\ python\ analyze\_stage3.py\ {[}RUNDIR{]}} and
  ...
  their id-keyed rows are committed as \texttt{stage3\_grades\_by\_id.json} /
  \texttt{stage4\_grades\_by\_id.json} beside the raw archives, with the stage-3
...
difference. \texttt{stage5\_scores.json}, \texttt{PREREG-stage5-successful-evasion.md}.)
...
passes, judge pinned to the stage-1/2 model.\footnote{\texttt{quality\_expansion/pool6.json} (blind), \texttt{key6.json} (never left
  the grading machine\textquotesingle s counterpart --- the key stayed local),
  \texttt{results\_stage6.json}, \texttt{analyze\_stage6.py};
  \texttt{workflows/stage6\_grade.js}; design registered before grading in
  \texttt{plans/S10-stage6-unreviewed-continuations.md}. Run 2026-08-19.}
...
nuisance parameter.\footnote{An earlier version of this section reported a \emph{length-adjusted}
  column instead and used it to conclude that justification\textquotesingle s apparent gain
...
\textbf{What this means for the prevalence number.} Evasion is real, clears the
...
that it measures \emph{undisguised} machine drafting. But it is not free in effort:
...
\hypertarget{5-limits}{%
\section{5. Limits}\label{5-limits}}
...
\item
  \textbf{Cohort mechanism is unidentified.} Three exposure tests null, and the
...
\item
  ...
  r = +0.758. Controlling the judge\textquotesingle s own \texttt{ai\_guess} (Appendix D) leaves the
...
\hypertarget{6-policy-context}{%
\section{6. Policy context}\label{6-policy-context}}
...
(\texttt{ai\_policy\_scan.md}; figures unverified against primary sources.)
...
\textbf{Rice does not claim evidence of absence, and should not be cited as
...
Pimlico\textquotesingle s agreement is \textbf{not} corroboration. Its method is the same family as
...
\hypertarget{8-discussion-detection-as-a-norms-instrument-and-what-to-measure-instead}{%
\section{8. Discussion: detection as a norms instrument, and what to measure instead}\label{8-discussion-detection-as-a-norms-instrument-and-what-to-measure-instead}}
...
\hypertarget{81-the-limit}{%
\subsection{8.1 The limit}\label{81-the-limit}}
...
\hypertarget{82-where-the-norms-argument-actually-lands}{%
\subsection{8.2 Where the norms argument actually lands}\label{82-where-the-norms-argument-actually-lands}}
...
\hypertarget{83-the-substitution-and-why-our-null-is-the-argument-for-it}{%
\subsection{8.3 The substitution, and why our null is the argument for it}\label{83-the-substitution-and-why-our-null-is-the-argument-for-it}}
...
The case rests on §4.9\textquotesingle s primary finding, and it is independent of evasion:
...
than averaged in (respect\_groups; Appendix D.1, §4.9), and across stages the
...
\hypertarget{84-where-text-is-a-proxy-for-a-persons-internal-state}{%
\subsection{8.4 Where text is a proxy for a person\textquotesingle s internal state}\label{84-where-text-is-a-proxy-for-a-persons-internal-state}}

Deliberation is judged on the artifact, so §8.3 suffices. Education is not:
there the text is a proxy for what is in someone\textquotesingle s head, and a proxy that can
be generated is no proxy at all. That case needs a \textbf{more rigorous proof of
...
\hypertarget{85-measuring-the-human-contribution-against-an-automated-counterpart}{%
\subsection{8.5 Measuring the human contribution against an automated counterpart}\label{85-measuring-the-human-contribution-against-an-automated-counterpart}}
...
\hypertarget{86-future-work}{%
\subsection{8.6 Future work}\label{86-future-work}}
...
\tightlist
\item
  \textbf{The substance channel.} §4 measures \emph{register}. Whether machine
  ...
  exist: the base-versus-instruct generation that produced §4.7\textquotesingle s +0.387 yields
...
1a. \textbf{A register-feature instrument, and the test that would settle §4.5a.}
Every arm in this study measures register through a \emph{word list}, which is
why §4.5a\textquotesingle s American result can be read two ways: the correlation is carried
...
1b. \textbf{Vendor attribution for the family signature.} §4.6a\textquotesingle s vector result ---
...
would itself bear on §4.5a\textquotesingle s homogenization reading. Two caveats bound
...
features rather than one number. It also gives §8.6.1\textquotesingle s register/substance
...
1c. \textbf{The anti-correlation of gut judgment, measured properly.} §8.2\textquotesingle s
...
1d. \textbf{A search that targets the human judge, to measure the vulnerability\textquotesingle s
ceiling.} The bypass arm (§4.9) optimises rewrites against a \emph{statistical}
...
1e. \textbf{The detection game --- 1c and 1d as one self-generating instrument.}
...
\texttt{plans/S10-detection-game.md}.

\textbf{B. Getting outside the parliamentary archive.} Three versions of one move.
§4.6\textquotesingle s mechanism is unidentified and three exposure tests inside the archive
returned null (Appendix A); the informative next evidence is a different \emph{kind},
...
\item
  ...
  speeches, the SO31-type format §4.3 measures at \textasciitilde37\% machine --- but US genre
...
4a. \textbf{A pre-AI tribute control --- the one specificity gap we know about.} Every
...
eulogies at \texttt{fraction\_ai} 1.0 (§4.3). Tribute register is elevated,
...
4b. \textbf{An LLM genre classifier, to make the genre ladder a panel result.} The
...
\item
  ...
  (\texttt{us/us\_extract.py}). It mirrors the closest prior work directly --- Suvanto
...
\tightlist
\item
  \textbf{Does the counterfactual residual behave?} Take the §4.9 pairs, generate
...
\item
  ...
  as its own number.
\item
  \textbf{Human-coded DQI subsample}, still the real validation of §8.3\textquotesingle s claim
  that the instrument is cheap \emph{and} sound (§5).
\item
  \textbf{Does anyone actually evade?} The norms argument in §8.2 rests on current
...
\item
  ...
  same instrument as §4.6a\textquotesingle s, pointed at a different institution, and it has
...
\item
  \textbf{The unconstrained occupational model}, deferred out of
  \texttt{PREREG-occupational-accountability.md} on 2026-08-18 so the confirmatory
...
\item
  ...
  S20, \texttt{plans/S20-register-search.md}).} Everything above measures against
  one register, and §5\textquotesingle s calibration names the cost: a single thin index
...
\item
  ...
  (+0.51σ/decade) \emph{and} cohorts arrive different --- but has never tested
...
\hypertarget{appendix-a--null-results}{%
\section{Appendix A --- Null results}\label{appendix-a--null-results}}
...
\item
  \textbf{Cohort × service-province adoption} --- null.
\item
  \textbf{Cohort × birth-province adoption} --- null. Spec B initially showed
...
\item
  \textbf{Skilled-immigration share} --- ρ = +0.26 on levels, +0.09 on growth, n = 6.
\item
  \textbf{Province-level post-secondary share} --- ρ = −0.04 on levels, +0.11 on
...
\item
  \textbf{Cross-province education composition} --- ρ = 0.00 against graduate
...
\item
  ...
  instead as \emph{risen} and \emph{common} is what produces §4.6a\textquotesingle s result.
\item
...
\item
  \textbf{Class contrasts at panel scale} --- every §4.6a class coefficient loses
...
\hypertarget{appendix-b--superseded-analyses}{%
\section{Appendix B --- Superseded analyses}\label{appendix-b--superseded-analyses}}

0a. \textbf{§4.7\textquotesingle s stage ordering} --- "the shift is largest at the preference
...
\setcounter{enumi}{-1}
\item
  \textbf{§4.6a, twice} --- first written as "standing predicts the register
...
\item
  ...
  above its own floor; bound ≤ \textasciitilde0.4\% at measured Se. Superseded by Pangram;
  retained because it explains why edited AI needs a commercial detector.
\item
  \textbf{New Brunswick on Pangram 3} --- superseded by the §4.1 rescore. Kept
...
\item
  \textbf{Content-word control} and \textbf{Mistral synthetic corpus} --- cut (§3.4).
\item
...
\item
  ...
  \item
    ...
    during the 1994--96 onset (\texttt{panel\_arrival\_premium.py}). An earlier claim
...
\item
  \textbf{Retired framings behind §4.6b, with their numbers.} Three instrument
...
\item
  \textbf{The bypass seed-verdict / band hypothesis} (exploratory; review items
  B8, and the earlier retraction). \emph{Do borderline originals evade more?} An
  ...
  finding and is stated nowhere in the register of §4.9\textquotesingle s rates.
\end{enumerate}

\hypertarget{appendix-c--replication-and-reproducibility}{%
\section{Appendix C --- Replication and reproducibility}\label{appendix-c--replication-and-reproducibility}}

\hypertarget{c1-cross-route-reproduction-of-the-detector}{%
\subsection{C.1 Cross-route reproduction of the detector}\label{c1-cross-route-reproduction-of-the-detector}}
...
route, and \texttt{pangram\_p4\_verdicts.csv} records which.

\hypertarget{c2-independent-re-analysis}{%
\subsection{C.2 Independent re-analysis}\label{c2-independent-re-analysis}}
...
\hypertarget{c3-bypass-sample-selection}{%
\subsection{C.3 Bypass sample selection}\label{c3-bypass-sample-selection}}
...
Appendix B. Full stacks in \texttt{BYPASS\_METHODOLOGY.md}.
...
\hypertarget{c4-artifacts}{%
\subsection{C.4 Artifacts}\label{c4-artifacts}}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{pangram\_p4\_verdicts.csv} & 4,258 verdicts, all Pangram 4, with metadata \\
\texttt{prevalence\_report.py} & §4.1--4.3, Wilson CIs, Rogan--Gladen \\
\texttt{nb\_p3\_vs\_p4.py} & §4.1 model-tier comparison \\
\texttt{api\_route\_check.py} & route/model equivalence test \\
\texttt{transcript\_regime\_check.py} & §2.1, writes \texttt{transcript\_regime.csv} \\
\texttt{build\_pangram\_expansion.py} & sampling, cleaning, regime floors \\
\texttt{in\_time\_placebo.py} & §3.3, the test that demoted the lexicon arm \\
\texttt{long\_trend.py} & §4.5 series and its 1994 trough \\
\texttt{cohort\_vs\_period.py}, \texttt{formation\_window.py} & §4.6 (birth vs period) \\
\texttt{covariate\_study.py} & §4.6a class and education, member level (\texttt{-\/-build-cache} first, \textasciitilde10 min) \\
\texttt{class\_origin.py} & §4.6a EGP/NS-SEC arithmetic over the checked-in coding \\
\texttt{class\_markedness.py}, \texttt{build\_class\_word\_year.py} & §4.6a chase-and-flight \\
\texttt{build\_member\_vectors.py}, \texttt{vector\_analysis.py} & §4.6a word-mix geometry; VECTOR-ANALYSIS.md is the full log \\
\texttt{claude\_gen/}, \texttt{claude\_gen\_old/}, \texttt{rlhf\_gen\_180/} & generated traces behind the family-signature table (audited) \\
\texttt{provinces/occupation\_coding.json} + \texttt{OCCUPATION\_CODING.md} & the coding itself, with its 96.4\% agreement rate \\
\texttt{wiki\_depth.py} & article length via Wikidata QID → MediaWiki, the notability control \\
\texttt{build\_allsource\_merge.py} & rebuilds \texttt{member\_allsource.json} from the workflow journal \\
\texttt{CLASS-REGISTER-LITERATURE.md} & §4.6a citations, \textbf{unverified}, with the suspect entries flagged \\
\texttt{band\_coverage\_check.py} & standing check that every length band is sampled at its chamber\textquotesingle s own rate \\
\texttt{olmo\_ladder.py} & §4.7 ladder stages \\
\texttt{rlhf\_pref\_compile.py} & §4.7 \textbf{+0.387} well-measured, 3 families at 1,600 prompts \\
\texttt{rlhf\_pref\_analyze.py} & the superseded +0.42 run, with null calibration (not \texttt{align\_ratio.py}) \\
\texttt{align\_ratio.py} & §4.7 Hansard-drift arm \\
\texttt{word\_context\_delta.py} & §4.8 in-context permeation \\
\texttt{bypass\_report.py} & §4.9 bypass: final two runs (primary), superseded two shown separately, band check \\
\texttt{go\_reflip\_fractions.json} + \texttt{nb\_}/\texttt{bp\_} & strict re-scores behind the 11.1\% \\
\texttt{quality\_expansion/} & §4.9, self-contained (\texttt{RUNME.md}) \\
\texttt{BYPASS\_METHODOLOGY.md} & §4.9 selection filters, per sample \\
\texttt{PRIOR\_ART.md}, \texttt{ai\_policy\_scan.md} & §6, §7 \\
\end{longtable}
...
DQI grading, stages 1-5 (§4.9) & \textbf{Claude Fable 5} & default \\
DQI grading, stage 6 (§4.9) & \textbf{Claude Opus} & medium \\
...
\hypertarget{appendix-d--robustness-and-sensitivity-checks}{%
\section{Appendix D --- Robustness and sensitivity checks}\label{appendix-d--robustness-and-sensitivity-checks}}

\hypertarget{d1-the-judge-leakage-control-documented-run-not-adopted}{%
\subsection{D.1 The judge-leakage control (documented, run, not adopted)}\label{d1-the-judge-leakage-control-documented-run-not-adopted}}
...
\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
dimension & stage 1 (internal): AI+FE → +judge\_ai & stage 2 (external): AI+FE → +judge\_ai \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
justification & +1.13 (t 4.1) → +0.83 (t 2.2) & +0.75 (t 4.1) → \textbf{+1.17 (t 3.6)} \\
common\_good & +0.58 (t 3.3) → +0.08 (t 0.3) & +0.89 (t 7.0) → \textbf{+0.71 (t 3.2)} \\
respect\_groups & +0.29 (t 2.0) → −0.14 & +0.64 (t 6.2) → +0.15 \\
respect\_demands & −0.18 → −1.21 (t −2.6) & −0.16 → −0.49 \\
respect\_counterargs & +0.14 → −0.83 & −0.14 → −1.07 (t −2.8) \\
...
\hypertarget{d2-prominence-wikipedia-article-length-on-the-full-panel}{%
\subsection{D.2 Prominence (Wikipedia article length) on the full panel}\label{d2-prominence-wikipedia-article-length-on-the-full-panel}}
...
tier-1 chambers from their evidence-URL titles (\texttt{build\_t1\_wiki\_depth.py},
...
\hypertarget{d3-prominence-in-buckets-which-is-how-it-should-be-read}{%
\subsection{D.3 Prominence in buckets, which is how it should be read}\label{d3-prominence-in-buckets-which-is-how-it-should-be-read}}
...
buckets and why no pooled coefficient is quoted anywhere. †\footnote{\texttt{python\ plot\_class\_by\_era.py}-adjacent computation; per-bucket means,
  standard errors and counts in \texttt{prominence\_buckets.csv}. Article lengths from
  the MediaWiki API via \texttt{build\_t1\_wiki\_depth.py}, run over the tier-1 set, the
  provincial fetch and the nine new chambers (2,310 of 2,315 resolved).}

\hypertarget{d4-is-the-cohort-gradient-ministerial-office}{%
\subsection{D.4 Is the cohort gradient ministerial office?}\label{d4-is-the-cohort-gradient-ministerial-office}}
...
\endlastfoot
all & +1.19 & +18.4 \\
\textbf{non-office only} & \textbf{+1.31} & \textbf{+17.9} \\
office only & +0.62 & +4.9 \\
\end{longtable}

\textbf{The gradient is not office, and the premise runs backwards.} Restricting to
non-office member-years \emph{strengthens} the birth effect (+1.19 → +1.31) rather
