...
binary segment rates (pooled sample; review item X12).\footnote{Parenthetical codes like (review item X12) index findings
  in the study\textquotesingle s committed adversarial-review records --- cited wherever a
...
under the pooled floor (≥99.7\%) at most 0.3 points of any chamber\textquotesingle s rate
...
of 60) gives z = 3.70, p = 2.2 × 10⁻⁴.\footnote{\texttt{python\ prevalence\_\allowbreak{}report.\allowbreak{}py} for the rates and bootstrap intervals;
  the SO31-vs-OQ permutation is 50,000 label shuffles of the pooled
  prevalence segments. Reported here rather than the old Fisher p = 0.00034,
  which tested a quantity the study no longer reports. The same script prints
  the Cochran--Armitage trend statistic on the segment counts (doses
...
something at a glance. Practically, the screen can be run for about \textbf{4× fewer reasoning
...
\bottomrule\noalign{}
\endlastfoot
UK Commons & 655 & 1,444 & ×2.20 & \textbf{+4.0\%/yr} \\
...
\textbf{US Senate} & 1,590 & 1,562 & ×0.98 & −0.1\%/yr \\
Prince Edward Island & 1,589 & 1,523 & ×0.96 & −0.2\%/yr \\
\textbf{US House} & 1,794 & 1,709 & ×0.95 & −0.2\%/yr \\
\end{longtable}
\endgroup
...
\bottomrule\noalign{}
\endlastfoot
UK Commons & 380 & \textbf{1,444} & \textbf{×3.80} & \textbf{+4.3\%/yr} \\
\textbf{US House} & \textbf{1,515} & 1,709 & ×1.13 & +0.4\%/yr \\
\textbf{US Senate} & \textbf{1,314} & 1,562 & ×1.19 & +0.5\%/yr \\
\end{longtable}
\endgroup
...
\item
  ...
  \emph{in the same year and chamber}. †\footnote{\texttt{python\ cohort\_\allowbreak{}vs\_\allowbreak{}period.\allowbreak{}py}. Member-year register rate regressed on
...
\item
  ...
  stronger of the two in 15 of the 22 chambers. †\footnote{\texttt{python\ cohort\_\allowbreak{}vs\_\allowbreak{}period.\allowbreak{}py}. Member-year register rate regressed on
    spoken year and birth year with chamber fixed effects, word-weighted,
    HC-robust; all 22 chambers now carry member birth years, 61,312
    member-years, after the nine Australian and UK-devolved chambers were
    collected on 2026-08-17. Within-chamber correlations are on the same rows.}
...
(\texttt{member\_\allowbreak{}level\_\allowbreak{}estimation.\allowbreak{}py}; the reasoning for each choice is in its
...
readable as movement. (\texttt{python\ plot\_\allowbreak{}class\_\allowbreak{}by\_\allowbreak{}era\_\allowbreak{}grouped.\allowbreak{}py}, reading
  \texttt{class\_\allowbreak{}by\_\allowbreak{}era\_\allowbreak{}grouped.\allowbreak{}csv} (per-point means, standard errors and member
...
(Wald p \textless{} 10⁻⁴ as level dummies).

It is not, however, an independent channel. Put class and education in one
model and the education block goes to \textbf{p = 0.215} while class holds at
\textbf{p \textless{} 10⁻⁴} --- because the two instruments name the same stratum. Teachers,
...
across 22 chambers); the prereg\textquotesingle s occupational-derivative block (dir middle +
apex delta + Indoors, each per sd --- \cref{sec:occupation}) then enters on the members with
...
of cohort: †\footnote{\texttt{python\ joint\_\allowbreak{}predictors.\allowbreak{}py}. Canonical member-level spec --- one
  ...
  the true +0.020). The occupational block is \cref{sec:occupation}\textquotesingle s dir middle + apex
  delta (grand model (ii)\textquotesingle s pair under the prereg\textquotesingle s never-together rule)
...
\caption{Every member-level predictor in one model: each block alone, the four-block joint fit (n = 4{,}056), and the occupational-derivative block added (n = 3{,}631); t in parentheses; the last column converts jointly significant effects into years of cohort.}\label{tab:four-predictors}\\
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{cohort} (per decade) & 4,056 & +0.276 (27.7) & \textbf{+0.270 (26.9)} & +0.264 (24.6) & \emph{the scale} \\
--- block Wald (1 df) & & p \textless{} 10⁻⁷⁰ & p \textless{} 10⁻⁶⁷ & p \textless{} 10⁻⁵⁹ & \\
\textbf{class} (baseline I, \emph{n} 1,660) & & & & & \\
· II lower service & 1,691 & +0.122 (3.63) & \textbf{+0.072 (2.10)} & +0.020 (0.47) & +3 \\
· III routine non-manual & 116 & +0.084 (0.95) & +0.018 (0.21) & −0.027 (−0.29) & --- \\
· IVab petty bourgeoisie & 280 & +0.005 (0.08) & −0.011 (−0.18) & −0.061 (−0.83) & --- \\
· IVc farmers & 109 & −0.339 (−3.48) & \textbf{−0.212 (−2.22)} & −0.199 (−1.90) & −8 \\
· V/VI skilled manual & 130 & −0.229 (−2.79) & \textbf{−0.247 (−3.14)} & −0.207 (−2.14) & −9 \\
· VIIab non-skilled manual & 70 & −0.384 (−3.70) & \textbf{−0.299 (−2.80)} & −0.147 (−1.01) & −11 \\
--- block Wald (6 df) & & p \textless{} 10⁻⁴ & \textbf{p \textless{} 10⁻⁴} & \textbf{p = 0.21} & \\
...
--- block Wald (4 df) & & p \textless{} 10⁻⁴ & \textbf{p = 0.215} & p = 0.27 & \\
\textbf{prominence} (linear, \emph{not} the right form --- see D.3) & 4,056 & +0.037 (3.59) & +0.020 (2.05) & +0.020 (1.91) & --- \\
--- block Wald (1 df) & & p = 0.0004 & p = 0.038 & p = 0.053 & \\
\textbf{occupational derivatives} (per sd, n 3,631) & & & & & \\
· dir middle & & +0.020 (0.78) & --- & −0.022 (−0.80) & --- \\
· apex delta & & +0.034 (1.31) & --- & \textbf{+0.076 (2.52)} & +3/sd \\
· Indoors & & +0.084 (5.28) & --- & +0.039 (1.95) & --- \\
--- block Wald (3 df) & & p \textless{} 10⁻⁴ & --- & \textbf{p = 0.0002} & \\
\end{longtable}
\endgroup

The + occupational column\textquotesingle s changes come from the block, not the thinner
...
\textbf{They do not deserve equal billing.} Cohort is untouched by anything and an
...
in Appendix~\ref{sec:prominence-buckets}, not this coefficient.
...
spans \textasciitilde0.37σ; cohort covers that in about fourteen years of birth date.
...
adjudicated (\texttt{provinces/\allowbreak{}OCCUPATION\_\allowbreak{}CODING.\allowbreak{}md}) --- gives 897 members and 5,294
...
−0.005 per half-decade, t −0.4).\footnote{\texttt{python\ class\_\allowbreak{}gap\_\allowbreak{}trend.\allowbreak{}py}, from \texttt{class\_\allowbreak{}by\_\allowbreak{}era\_\allowbreak{}grouped.\allowbreak{}csv} (the
  era figure\textquotesingle s own per-period means within chamber × period z); the gap\textquotesingle s
  inverse-variance-weighted trend across the seven half-decades.} Jhering set out the diffusion
...
\texttt{plans/\allowbreak{}PREREG-\allowbreak{}occupational-\allowbreak{}accountability.\allowbreak{}md} is the registration with every
...
\texttt{prereg\_\allowbreak{}member\_\allowbreak{}table.\allowbreak{}json} (one join; n = 4,762 members with register and
...
50.8\% / 51.2\% / \textbf{28.9\%} (n = 498/297/45; \texttt{applicability\_\allowbreak{}by\_\allowbreak{}band.\allowbreak{}py}). So
...
sources, same answer.\footnote{Stage 1: \texttt{cd\ quality\_\allowbreak{}expansion\ \&\&\ python\ analyze.\allowbreak{}py}. Stage 2:
  \texttt{python\ analyze\_\allowbreak{}stage2.\allowbreak{}py} --- the column\textquotesingle s own script (review item Q7),
...
blind-to-the-target search gets \textbf{22.5\%} of targets past it: \textbf{9.8×}.
...
Pangram 4, Epoch AI Style Imitation & 2.86\% & FNR, doc-level, adversarial, vendor\footnote{Corrected 2026-08-24 (review item X2): 2.86\% (17/594) is Pangram 4\textquotesingle s
  ...
  benchmark (the Pangram 4 report\cite{pangram4}\textquotesingle s own Table 23), not the Perkins benchmark\cite{perkins2024}, which an
...
raised both bars, and by \emph{more} on the success bar (5.5×) than on the flip bar
...
difference. \texttt{stage5\_scores.json}, \texttt{PREREG-\allowbreak{}stage5-\allowbreak{}successful-\allowbreak{}evasion.\allowbreak{}md}.)
...
(review item Q3\textquotesingle s MDEs), but every powered result runs favourable --- which is
...
\item
  ...
  floor claim needs \texttt{Sp\ =\ 1} on contemporary human speech; the controls end
...
\item
  ...
  \texttt{plans/\allowbreak{}S10-\allowbreak{}detection-\allowbreak{}game.\allowbreak{}md}.
\end{enumerate}
...
\item
  ...
  \texttt{PREREG-\allowbreak{}occupational-\allowbreak{}accountability.\allowbreak{}md} on 2026-08-18 so the confirmatory
...
\item
  ...
  S20, \texttt{plans/\allowbreak{}S20-\allowbreak{}register-\allowbreak{}search.\allowbreak{}md}).} Everything above measures against
...
and reproducible (\texttt{build\_\allowbreak{}pangram\_\allowbreak{}expansion.\allowbreak{}py}, \texttt{build\_shortband.py}).
...
\texttt{transcript\_\allowbreak{}regime\_\allowbreak{}check.\allowbreak{}py} measures two markers that track editorial
...
toward the register the detector keys on, so \texttt{Sp\ =\ 1} on 2025--26 human
