...
We therefore measure both, separately, across 22 chambers in five countries, on a corpus reaching back three decades --- the depth that makes the second question askable at all. Pangram\cite{pangram2024}, a commercial detector calibrated against each chamber\textquotesingle s own pre-2022 record (specificity 1,260 of 1,260), measures prevalence. A lexical register instrument built from the vocabulary language models overuse, run against each chamber\textquotesingle s own placebo-matched counterfactual, measures permeation.
...
large chamber drive the result: the equal-weight mean of the per-chamber rates (\cref{tab:prevalence}, drawn in \cref{fig:prevalence-dots}) is
9.12\%, against the 9.03\% word-weighted pooled rate.\footnote{\texttt{banded\_prevalence.py}.}
...
single 60/60 control\textquotesingle s Wilson lower bound is 94\%, so no chamber\textquotesingle s rate below
\textasciitilde6\% is calibration-exact on its own control alone; under the pooled floor
(\(\geq\)99.7\%) at most 0.3 points of any chamber\textquotesingle s rate could be false positives ---
...
in proportion to its own noise, puts the chambers\textquotesingle{} rates at \textbf{4.3\% to
18.3\%, a better-than-fourfold range}; the fit\textquotesingle s own diagnostics say the
spread is real, with logit-scale \(\tau\) = 0.56 and I\({}^{2}\) = 0.78 --- 78\% of the
...
(Appendix~\ref{app:future} item 8), and the study does not attribute the between-chamber
...
\caption{Register by length of service, two readings, each curve labelled in the panel. The within-member curve --- each legislator\textquotesingle s rate minus their own career mean --- rises with tenure, because within a career years served and calendar year are one clock (its slope reproduces the +0.51 within-member drift). The period-adjusted curve --- rate minus its chamber \(\times\) year mean --- falls with tenure, but that fall is the entry-cohort gradient (longer tenure at a fixed year = earlier cohort), not a within-career decline. Left-censored members dropped; error bars member-cluster bootstrap 95\% intervals.}
\label{fig:tenure}
\end{figure}
...
column --- and with the graduate rung split, master\textquotesingle s and doctorate both sit
...
word\textquotesingle s rise and class I\textquotesingle s relative use of it against class II is negative at
every volume threshold. Class I under-uses \texttt{primary} (\(2.35\times\) lift),
\texttt{advocating} (\(1.85\times\)), \texttt{advocates} (\(1.57\times\)), \texttt{outcomes} (\(1.54\times\)); it over-uses
\texttt{remarkable}, \texttt{individuals}, \texttt{broader}, \texttt{ultimately} --- words that barely
...
\bottomrule\noalign{}
\endlastfoot
class I vs II & \(-0.13\) & \(-0.13\) & \textbf{\(-0.23\)} & \(-0.23\) \\
...
class-by-era panels of \cref{sec:class}), and the II-over-I separation never widens
across thirty years (+0.141 \(\rightarrow\) +0.079, trend t \(-1.7\) --- drifting narrower, not
...
(11.3\% vs II\textquotesingle s 5.7\%, 27-segment cell, suggestive only). Because register and
...
social dynamics \cref{sec:chase-flight} documents, which are expected to persist.
...
cell: it moves \textbf{+0.0150} (positive in 7 of 10 chamber \(\times\) family cells,
permutation p = 0.017), reusing the identical scored occurrences.\footnote{\texttt{python\ word\_\allowbreak{}context\_\allowbreak{}prellm.\allowbreak{}py}, reusing the committed occurrence
  ...
  about twice the pre-to-post rate (\textasciitilde+0.0022/yr).}
That window establishes permeation without machine-drafted contamination.
The full pre-vs-post contrast is \textbf{+0.0099} (9 of 10 cells, permutation p =
0.022)†\footnote{\texttt{python\ word\_\allowbreak{}context\_\allowbreak{}delta.\allowbreak{}py\ pooled}, which prints the pooled figure,
  ...
  3,000 draws. Per-model cell table at \texttt{METHODOLOGY.md:1009}.}, but its post-era cells contain machine-drafted text (\textasciitilde9\% of
...
agrees: P(\(\leq\)0) runs between 0.017 and 0.024, and permutation p between 0.015
and 0.022 across seeds. What is not stable is the bootstrap\textquotesingle s lower endpoint,
...
RLVR objective doesn\textquotesingle t install it" and "the RLVR data isn\textquotesingle t chat" cannot be
separated here. Three checks bound what can be said.\footnote{\texttt{python\ ladder\_\allowbreak{}length\_\allowbreak{}control.\allowbreak{}py} (reproduces the shipped
...
\toprule\noalign{}
dimension & stage 1, per sd of score (n = 840) & stage 2, AI vs Human verdict (n = 682) & stage 1, fitted 0\(\rightarrow\)100 difference \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
dimension & stage 1, per sd of score (n = 840) & stage 2, AI vs Human verdict (n = 682) & stage 1, fitted 0\(\rightarrow\)100 difference \\
...
The engagement null above is conditional: AI-flagged speech \emph{engages no worse
...
can retry.} The search loop (\cref{fig:bypass}) generates and screens variants against an \emph{Opus}
self-score --- a general intelligence, not the target --- and \textbf{Pangram is not in
the loop}: it sees only final submissions. Against the 13 commercial
"humanizer" services benchmarked in Pangram\textquotesingle s own technical report --- products
built and tuned \emph{against} detectors --- which get \textbf{2.31\%} of documents past
it, this search\textquotesingle s commensurable one-submission rate is \textbf{11.1\%} of variants
(\textbf{\(4.8\times\)}); with query-and-retry it gets \textbf{22.5\%} of targets past it
(\textbf{\(9.8\times\)}). Against Pangram\textquotesingle s clean-conditions false-negative rate of 0.34\%,
the retry-enabled rate is \(66\times\).

The result does not arise from a weak detector or a trivial prompt. Pangram produced 0 false positives in 1,260 pre-2022 controls (\cref{sec:calibration}), and a single "make this sound human" prompt makes text \emph{more} detectable, not less. That is what makes the reversals worth reporting: they are movements \emph{into} a class the detector otherwise reaches zero times in 1,260 --- not boundary noise, but a hard specificity floor that a directed search nevertheless walks text across.
...
\toprule\noalign{}
run & searched & zero-yield & \(\geq\)1 Human & rate & \(\geq\)1 at \texttt{fraction\_ai\ =\ 0.0} & rate \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
run & searched & zero-yield & \(\geq\)1 Human & rate & \(\geq\)1 at \texttt{fraction\_ai\ =\ 0.0} & rate \\
...
shows a deliberative-quality \emph{cost} to the evading rewrite --- the paired nulls
...
(Appendix~\ref{app:future}, item 33): tie register intensity to productivity and success
...
sample, and Tasmania\textquotesingle s transcription regime shifted mid-corpus, which leaves
it with no usable control (\cref{sec:hazards}). The constant-window register series in
\cref{sec:climb-geography} additionally includes Prince Edward Island; New Brunswick, the
...
from unflagged legislative segments at \textbf{AUC 0.61} (0.60 restricted to the 2025--26
prevalence sample, where both classes are drifted contemporary speech),
against the screen\textquotesingle s 0.95 (Appendix~\ref{sec:screen-pangram}). On ground truth it does no better: \textbf{0.535}
...
\textbf{The folk ladder} is the measure\textquotesingle s primary form. It is named for its
...
nominated the elements that mark the \textbf{free / bottom / middle / top} of a
...
and coder count --- is \cref{tab:elements-levels} in Appendix~\ref{sub:element-signatures}.
...
\textbf{F. Court transcripts, and the expert-witness problem.}
...
\textbf{G. What the legislator panel enables beyond the register.}
...
\textbf{H. Follow-ons to the register and occupational arms.} Directions that
...
\textbf{Draft of 2026-08-11; \cref{sec:class} and Appendix~\ref{app:null} extended 2026-08-13.} All arms
complete, including the detector-bypass study and both prior-art
comparators. Everything is reproducible from \texttt{analysis/s10/}.
...
At panel scale the ladder does not replicate (Appendix~\ref{app:null} item 16): the
...
\textbf{There is no usable pooled slope.} A single slope assumes a gradient, and
only some chambers have one, so the earlier \(-0.033\sigma\) per log-byte estimate
