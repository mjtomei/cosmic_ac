...
short interjection becomes one.\footnote{\texttt{segment.py}.} A rate over segments is partly a
...
pieces by count, and \textbf{longer pieces are markedly more machine-drafted} ---
...
The weighting is applied \emph{after} a uniform draw over segments, not by sampling
...
are reported individually in \cref{tab:prevalence} and drawn in \cref{fig:prevalence-dots}, and their \textbf{equal-weight mean matches the
...
the 618-segment overlap, and it is a cheap stratifier for future work.\footnote{\texttt{python\ opus\_\allowbreak{}screen\_\allowbreak{}auc.\allowbreak{}py}. The deployed screen is
  \texttt{opus\_screen\_full.js} (473 batches of 40), distinct from the lean
  validation run used in the effort A/B in Appendix~\ref{app:repro}; the two differ in
...
pre-2023 text, which is \emph{provably} pre-LLM and therefore the cleanest available
negative; restricting the negatives to 2023-and-later contemporary speech only
\emph{lowers} the AUC, to \textbf{0.940} {[}0.915, 0.961{]}. Contemporary human speech is the
harder class to separate --- as the permeation finding (\cref{sec:permeation}) predicts --- so the
...
\bottomrule\noalign{}
\endlastfoot
justification & +0.173 (t +4.1) ✱ & \textbf{+0.290} (t +4.4) ✱ & \textbf{+1.134} \\
common\_good & +0.089 (t +3.3) ✱ & \textbf{+0.229} (t +4.5) ✱ & \textbf{+0.583} \\
respect\_groups & +0.044 (t +2.0) ✱\footnote{The one starred cell that does not survive multiple-testing
  ...
  judge leakage.} & \textbf{+0.220} (t +6.1) ✱ & \textbf{+0.287} \\
...
it with no usable control (\cref{sec:hazards}).
...
\bottomrule\noalign{}
\endlastfoot
Corpus-wide screen (\cref{sec:screen-pangram}), effort A/B (Appendix~\ref{app:repro}) & Claude Opus & low, and max in the A/B \\
