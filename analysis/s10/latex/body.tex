...
cohort = +2.13 per 1,000 per decade}.\footnote{\texttt{python\ apc\_\allowbreak{}solution\_\allowbreak{}line.\allowbreak{}py} (re-derives the committed
...
\textbf{\(-0.89\)/decade} (clustered t \(-3.8\)) and propagates to a \emph{linear} cohort
slope of \textbf{\(-0.02\) {[}\(-0.47\), +0.44{]}}: under the paper\textquotesingle s own predates premise,
...
linearity outright (Wald \(p \approx 10^{-8}\)), and the constraint-invariant contrasts
put the \textbf{post-1985 cohorts +0.84 above the linear profile (z = 3.9)} with
a late-minus-early local slope difference of \textbf{+0.84/decade (z = 3.6)}:
...
prefix and instruct-with-template arms are reported with the ladder\textquotesingle s
files). And the tempting stronger story --- \emph{that the preference data itself
