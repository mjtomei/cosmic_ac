...
carries the study\textquotesingle s claims. The corpus reaches back three decades, which is what lets the
...
and no less engaged. The finding we did not come for is the paper\textquotesingle s centre
...
\cref{sec:prevalence} figure is if anything an underestimate of machine text. Using \texttt{Se\ =\ 1}
...
Sensitivity on \emph{edited} machine text is the real unknown and is discussed in \cref{sec:limits}.
...
detectors and does nothing for a strong one --- consistent with \cref{sec:retired}\textquotesingle s finding
...
LLMs did not start it.\footnote{\texttt{python\ long\_trend.py\ -\/-seg\ uk/segments\_uk\_deep.jsonl} for the annual
...
\subsection{The climb is everywhere except the United States}\label{sec:climb-geography}
...
\caption{The register gap over time by chamber: first year, last year, and change.}\label{tab:climb}\\
\toprule\noalign{}
...
\item
  ...
  which \emph{strengthen} it rather than explain it away (\cref{sec:class}). †\footnote{\texttt{python\ formation\_window.py}, clustered on member (CR1): birth-decade
...
backbenchers, not more (Appendix~\ref{sec:cohort-ministerial}). The mechanism behind the cohort
gradient remains unidentified --- three exposure tests failed to isolate it
(Appendix~\ref{app:null}). Cohort is not the whole of who uses the register --- \cref{sec:class} adds
three predictors that survive it --- but none of them identifies the cohort
mechanism either.

\subsection{Class and the register: jointly significant, individually noisy --- and education is not it}\label{sec:class}
...
all seven half-decades (\cref{fig:class-era}), and the manual-and-farm tail stays at the floor
throughout.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{class_by_era_grouped.png}
\caption{The class profile of the register, 1995--2026. One point per class per period
...
readable as movement. (\texttt{python\ plot\_class\_by\_era\_grouped.py}, reading
...
\begin{longtable}[]{@{}lll@{}}
\caption{The four predictors alone and jointly (cross-fitted).}\label{tab:four-predictors}\\
\toprule\noalign{}
...
Appendix~\ref{sec:prominence-buckets}, not this coefficient.
...
\subsubsection{Class: the provincial estimates (retained as the discovery record; see the panel result above)}\label{sub:class-provincial}
...
\subsection{Pre-registered occupational test: the register peaks at the insulated middle}\label{sec:occupation}
...
the framing is retired --- Appendix~\ref{app:superseded}8 preserves its numbers, and the
...
overwhelming axis (t +24.5, carrying 0.144 of the model\textquotesingle s 0.165).\footnote{\texttt{python\ prereg\_negative\_controls.py}; n = 4,762, estimator
  ...
  \texttt{python\ prereg\_indoors\_joint.py} (n = 3,594, the covariate panel; the
  grand model mirrors \texttt{prereg\_covariate\_strength.py}).}
...
ordering was the pedestal, not the data (Appendix~\ref{app:superseded}). What remains true, and
...
\textbf{The register is permeating human speech independently of drafting:
+0.0099, positive in 9 of 10 cells, permutation p = 0.017.} †\footnote{\texttt{python\ word\_context\_delta.py\ pooled}, which prints the pooled figure,
  ...
  3,000 draws. Per-model cell table at \texttt{METHODOLOGY.md:1009}.} The
...
review item B2). \Cref{fig:bypass} lays out the loop.
...
Pangram 4, Epoch AI Style Imitation & 2.86\% & FNR, doc-level, adversarial, vendor\footnote{Corrected 2026-08-24 (review item X2): 2.86\% (17/594) is Pangram 4\textquotesingle s
  ...
  benchmark (Pangram 4 report\cite{pangram4}, Table 23), not the Perkins benchmark\cite{perkins2024}, which an
...
\section{Discussion: detection as a norms instrument, and what to measure instead}\label{sec:discussion}

\cref{sec:quality} is usually read as a result about one detector. It is better read as an
instance of a general limit, and the generalisation changes what the rest of
the study is for.

\subsection{The limit}\label{sec:disc-limit}
...
\subsection{Where text is a proxy for a person\textquotesingle s internal state}\label{sec:disc-proxy}

Deliberation is judged on the artifact, so \cref{sec:disc-substitution} suffices. Education is not:
...
\item
  ...
  operationalisation of the same kind (\cref{sec:future} items 2--4).
\item
...
\item
  ...
  Until a pre-AI tribute control exists (\cref{sec:future} item 4a), the specificity claim
...
1a. \textbf{A register-feature instrument, and the test that would settle \cref{sec:climb-geography}.}
...
1b. \textbf{Vendor attribution for the family signature.} \cref{sec:class}\textquotesingle s vector result ---
...
1c. \textbf{The anti-correlation of gut judgment, measured properly.} \cref{sec:disc-norms}\textquotesingle s
...
\subsection{The calibration that carries the argument}\label{sec:calibration-method}
...
and the reason its result differs from published comparators (\cref{sec:related}).

With specificity \texttt{Sp} and sensitivity \texttt{Se}, an observed flag rate \texttt{π} relates
to true prevalence \texttt{τ} by Rogan--Gladen\cite{rogan1978}:
...
\item
  ...
  cohort t-statistic itself (\cref{sec:generational}): member-year HC1 errors report t ≈ 17.7,
...
\texttt{transcript\_regime\_check.py} & \cref{sec:hazards}, writes \texttt{transcript\_regime.csv} \\
...
\texttt{class\_markedness.py}, \texttt{build\_class\_word\_year.py} & \cref{sec:class} chase-and-flight \\
\texttt{build\_member\_vectors.py}, \texttt{vector\_analysis.py} & \cref{sec:class} word-mix geometry; VECTOR-ANALYSIS.md is the full log \\
...
\texttt{CLASS-REGISTER-LITERATURE.md} & \cref{sec:class} citations, \textbf{unverified}, with the suspect entries flagged \\
