...
detectors run over 124,000 Australian federal speeches found no post-ChatGPT rise (Rice\cite{rice2026}); a phrase-spike claim in the UK Commons came with no prevalence
estimate behind it (Pimlico Journal\cite{pimlico2025}).
...
(fraction-weighted; 8.65\% discounting their every word): the floor\textquotesingle s transfer
...
\cref{sec:climb-geography} and Appendix~\ref{app:future}.
...
segment-weighted comparison measures the packer\textquotesingle s behaviour across genres
alongside the drafting. On segments the same data reads SO31 36.7\%, Government
Orders 23.3\% and Oral Questions 8.3\%, a \(4.40\times\) ratio; on binary counting SO31
reads 35.8\%, Government Orders 25.7\% and Oral Questions 11.8\%, a \(3.03\times\) ratio.
...
Word- and fraction-weighted the ordering is unchanged and \textbf{the gap settles at
\(3.29\times\)}. The test is a label permutation rather than Fisher exact, because
Fisher takes segment counts and the estimator is a ratio of summed words.\footnote{\texttt{python\ prevalence\_\allowbreak{}report.\allowbreak{}py} for the rates and bootstrap intervals;
...
\textbf{The ruler, audited --- what the thirty-year rise is made of.} Because the marker list was selected on the post-2022 jump and then run backwards, it invites two objections: that a few words carry the rise, and that selection on the endpoint creates a trend by construction. We tested both.\footnote{\texttt{python\ build\_\allowbreak{}word\_\allowbreak{}year\_\allowbreak{}counts.\allowbreak{}py} (one scan, 1.62B tokens;
...
panel, so the term carries \textasciitilde0.25\% of variance alone. Class II\textquotesingle s far smaller
+0.122, spread over 42\% of members, carries more (\textasciitilde0.36\%), and cohort\textquotesingle s
...
"how much of the panel". \Cref{tab:four-predictors}\textquotesingle s n and years columns show the first and
...
\textbf{The drift predates the models.} The instrument is the self-normalised,
...
possible in either cell. There the register moves \textbf{+0.0150} (positive in 7
of 10 chamber \(\times\) family cells, permutation p = 0.017), on the identical scored
occurrences.\footnote{\texttt{python\ word\_\allowbreak{}context\_\allowbreak{}prellm.\allowbreak{}py}, reusing the committed occurrence
  ...
  about twice the pre-to-post rate (\textasciitilde+0.0022/yr).} That is permeation without machine-drafted
contamination. The full pre-vs-post contrast is \textbf{+0.0099} (9 of 10 cells,
permutation p = 0.022)†\footnote{\texttt{python\ word\_\allowbreak{}context\_\allowbreak{}delta.\allowbreak{}py\ pooled}, which prints the pooled figure,
  ...
  3,000 draws. Per-model cell table at \texttt{METHODOLOGY.md:1009}.}, but its post-era cells contain machine-drafted
text (\textasciitilde9\% of words) and cannot separate continued human drift from that
contamination.
...
DPO\textquotesingle s generations are longest). Three independent corrected routes agree on
the picture below (exact stratified estimator shown; audit values in the
2026-08-11 review, M9). The RLVR row\textquotesingle s corrected shift has a confidence
...
pool\textquotesingle s label is a blinded, date-blind LLM screen of 37,801 segments; on the 618
segments where it overlaps Pangram it separates Pangram\textquotesingle s classes at AUC 0.954
{[}0.934, 0.971{]} (Appendix~\ref{sec:screen-pangram}), so its score reads as a machine-involvement
signal where no Pangram verdict exists. The second pool\textquotesingle s label is the Pangram
verdict itself.
...
\emph{Stage 1 reports per sd of the blinded screen score (sd 15.3 on the 0--100 scale, observed maximum 70), the commensurable comparison with stage 2\textquotesingle s binary contrast; unscaled fitted differences are in Appendix~\ref{app:cut}. ✱ marks the six cells the claim rests on --- justification, common\_good and respect\_groups in each stage. Five hold q \textless{} 0.003 under Benjamini--Hochberg across the table\textquotesingle s fourteen cells; the sixth is disclosed in its footnote.}
...
Pangram 4, Epoch AI Style Imitation & 2.86\% & document-level adversarial benchmark, vendor\footnote{2.86\% (17/594) is Pangram 4\textquotesingle s
...
\textbf{this study, per target} & \textbf{22.5\%} & \textbf{induced; attacker may query and retry, \(\leq\)18 attempts --- \(9.8\times\) the humanizers} \\
\end{longtable}
\endgroup
...
that interval is that the two chambers\textquotesingle{} \emph{humanising playbooks} flatly
...
\textbf{The success rates bound what these searches achieved, not what is
...
AI, for Mixed or Human --- a \emph{flip} --- runs higher throughout and rose with
effort in the same direction; those rates are in Appendix~\ref{app:cut} (item 7).
...
the mark ships across all the company\textquotesingle s products and its AWS, Google Cloud and
...
The seven dimensions are not one factor, and a weighting that emphasises engagement over form puts the composite contrast at the margin (\cref{sec:quality}) --- which is why the claims below are read per dimension rather than off the index.
...
pilot and has no uniform prevalence sample, and Tasmania\textquotesingle s transcription regime
shifted mid-corpus, leaving no usable control (\cref{sec:hazards}). The nineteen-chamber
constant-window register series of \cref{sec:climb-geography} differs again: it adds Prince Edward
Island, absent from the table above, and omits New Brunswick, the discovery
corpus of \cref{sec:register-shift}.
...
false-positive problem (\cref{sec:calibration}).
...
from unflagged legislative segments at \textbf{AUC 0.61} (0.60 restricted to the
2025--26 prevalence sample, where both classes are drifted contemporary
speech), against 0.95 for the study\textquotesingle s Opus screen, a separate model-based
judge of the same distinction (Appendix~\ref{sec:screen-pangram}). On ground truth it falls to
\textbf{0.535}, barely above chance --- though that set is the evasion run\textquotesingle s
adversarially quietened rewrites rather than ordinary model output, which runs
far hotter on these words (\cref{sec:posttraining}). One thin lexical rate carries
