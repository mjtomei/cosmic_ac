...
families. \emph{Instance detectors} score one text at a time, and applied naively to
parliamentary speech they have produced two unsatisfying results: off-the-shelf
detectors, run over 124,000 Australian federal speeches, found no post-ChatGPT rise (Rice\cite{rice2026}), and a phrase-spike claim in the UK Commons came with no prevalence
estimate behind it (Pimlico Journal\cite{pimlico2025}). \emph{Population-level methods} estimate the
machine-modified fraction of a whole corpus distributionally, and put post-2022
figures between 6.5\% and 17.5\% on peer reviews and scientific papers (Liang et al.\cite{liang2024monitoring,liang2025quantifying}). \emph{Marker-vocabulary instruments} track the words models overuse --- excess
vocabulary in biomedical abstracts (Kobak et al.\cite{kobak2025}) and in scholarly writing at large (Gray\cite{gray2025}). Closest to this study, Suvanto et al.\cite{suvanto2026} detect undisclosed LLM
...
The question splits in two. \emph{Prevalence} --- how much of the record a machine drafted --- needs a calibrated detector. \emph{Permeation} --- whether human speech drifts toward the machine register independent of drafting --- needs an instrument independent of the detector, because the human baseline is itself moving. Conflating the halves is the main error available here.

We therefore measure both, separately, across 22 chambers in five countries (twenty of them in the prevalence arm), on a corpus reaching back three decades --- the depth that makes the second question askable at all. Pangram\cite{pangram2024}, a commercial detector calibrated against each chamber\textquotesingle s own pre-2022 record (specificity 1,260 of 1,260), measures prevalence. A lexical register instrument built from the vocabulary language models overuse, run against each chamber\textquotesingle s own placebo-matched counterfactual, measures permeation.

The two instruments differ in what they can be trusted with. The detector is a black box with a measured false-positive floor. The register is transparent, cheap at corpus scale, and robust to the light editing that defeats detectors, but coarse. The claims therefore rest on a division of labour rather than on the two instruments agreeing on one number: the calibrated detector carries prevalence, the register carries history and social structure. They meet at one point, the genre ladder, where both face the same confound --- drift in what kind of business the record contains. Each meets it in the open: the detector\textquotesingle s flags are audited inside the unscripted genre, and the register\textquotesingle s trend is post-stratified against the drift.
...
The two halves of the study meet at one measurable point. Post-training elevates the same marker vocabulary that, among humans, peaks at the insulated organisational middle --- a shared rate on one word list, not a shared mix, and a weak effect. That overlap is a quantitative handle on what Brynjolfsson\cite{brynjolfsson2022} calls the Turing trap: the worry that human-imitating AI is developed to substitute for workers rather than augment them. We do not argue the case. We supply a way to measure it, and a template for the oversight measurements the worry calls for.
...
\part*{Part A --- The occupational altitude measure}\setcounter{section}{0}\renewcommand{\thesection}{A\arabic{section}}

\section{Introduction}\label{sec:a-intro}
...
occupations outside it. Part B tells the study\textquotesingle s story --- prevalence, the
register\textquotesingle s thirty-year history, and post-training as its mechanism --- and uses
...
\textbf{The measure.} Socioeconomic position is usually read off a categorical
map --- Erikson and Goldthorpe\textquotesingle s\cite{erikson1992} EGP schema for class --- or off a fitted scale
such as Ganzeboom\textquotesingle s ISEI or Treiman\textquotesingle s SIOPS, which place an occupation by the
...
takes it a step further: the occupational content is scored from O*NET\textquotesingle s full descriptor
universe, by coders who were never shown the outcome or the
hypothesis, on a level the categorical maps do not draw.

The construction is in \cref{sec:occ-classification}. Each legislator\textquotesingle s prior occupation is
double-blind coded to an O*NET-SOC code (96.4\% raw agreement), and the code\textquotesingle s
...
middle and top --- built two semi-independent ways: a \emph{theoretical ladder}
composed from four accountability components, and a \emph{folk ladder} scored from
the element signatures that eighteen blind coders assigned to the everyday
image of a corporate hierarchy.

\textbf{The claim.} The machine register --- a member\textquotesingle s rate of use of the
vocabulary language models overuse, measured in that member\textquotesingle s own speech and
defined in \cref{sec:member-spec} --- peaks at the insulated
...
timeline recorded to the minute (\cref{sec:occupation}), and it held: on both constructions
...
\textbf{What the measure is, and is not.} Against the market-fitted measures it is
convergent without being any of them: member-weighted Cramér\textquotesingle s V between the
folk argmax level and each anchor runs 0.31 to 0.47, while regressing each level on
occupational education and log wage leaves more than half of every level\textquotesingle s
variance unexplained (\(R^{2}\) 0.32--0.45). In the joint model the asymmetry is
...
coding of the occupational content the ladders score at finer grain (\cref{sec:four-predictors}).

\textbf{Order.} \cref{sec:a-method} gives the construction and the estimation specification, and
restates the outcome so this part can be read on its own. \cref{sec:a-results} reports the
results in the order the arm developed: the class and education contrasts
that first showed the shape (\cref{sec:class}), the measure built to explain it (\cref{sec:occupation}),
prominence as a separate gradient (\cref{sec:prominence}), every member-level predictor in
one model (\cref{sec:four-predictors}), and an exploratory reading of the gradients (\cref{sec:chase-flight}). \cref{sec:a-limits}
states the measure\textquotesingle s limits; \cref{sec:a-discussion} sets it against the market\textquotesingle s measures.

\section{Data and method}\label{sec:a-method}
...
\textbf{The folk ladder} is the measure\textquotesingle s primary form. It is named for its origin:
...
elements that mark the \textbf{free / bottom / middle / top} of a corporate
...
consensus sign. A level score is the mean of its consensus elements\textquotesingle{}
...
are the subject of a committed sensitivity run.\footnote{The nomination workflow and consensus are \texttt{element\_levels.json}
  ...
  \texttt{element\_\allowbreak{}audit\_\allowbreak{}4col.\allowbreak{}json}.} The complete signature
--- every element id, name, level, sign and coder count --- is \cref{tab:elements-levels} in Appendix~\ref{sub:element-signatures}.

\textbf{The theoretical ladder} is the semi-independent check, named for the
...
blind coding workflows assigned elements to four accountability components --- \textbf{U} upward (advising without the final say, discretion
...
yielding 71 consensus elements (\cref{tab:elements-components} in Appendix~\ref{sub:element-signatures}); the free /
...
register; the timeline, to the minute, is in Appendix~\ref{app:cut}
item 24.

\subsection{The member-level specification}\label{sec:member-spec}

\textbf{The outcome.} The dependent variable throughout Part A is the register --- measured by
the register instrument of Part B (\cref{sec:freq-descriptive}): a member\textquotesingle s rate of occurrence, per 100k words,
of the 407 style words that Kobak et al.\textquotesingle s excess-vocabulary method identifies
as the vocabulary language models overuse, computed over the member\textquotesingle s own
...
AUC 0.61 --- and it is used here as a measure of how far a member\textquotesingle s speech sits
...
against its full member population, so a member\textquotesingle s score is a position among
that chamber\textquotesingle s own speakers.
...
reasoning for each choice is recorded with the estimator.\footnote{\texttt{member\_\allowbreak{}level\_\allowbreak{}estimation.\allowbreak{}py}; each choice\textquotesingle s reasoning is in its
  docstring.}

\section{Results}\label{sec:a-results}

\subsection{Class and education: the same shape at coarser grain}\label{sec:class}
...
across 22 chambers (n = 4,896, joint Wald \textbf{\(p < 10^{-4}\)}), reading the raw
...
\emph{The mean column above and the contrast sentences below are different
estimands: the means are each class\textquotesingle s raw within-chamber average, the contrasts
...
\textbf{The II-over-I crossover is established, by the test that needs no normalization at all.} Comparing class II to class I WITHIN each chamber --- each legislature its own control, cohort-adjusted, equal member weights --- the contrast is positive in 9 of 12 chambers, individually significant in each of the three with the power to detect it (UK +0.68, t = 2.48; US House +0.82, t = 2.10; Manitoba +2.26, t = 2.21), and the inverse-variance meta-analysis across chambers gives \textbf{+0.59 per 1,000, z = 3.50}. Teachers, nurses and journalists out-use lawyers, physicians and professors, within their own chambers, on both sides of the Atlantic. A full-population-z-scored variant reads positive and short of significance (Appendix~\ref{app:cut} item 20).
...
annotated). Error bars are \(\pm\)1 se, narrower than the 95\% intervals used
elsewhere and not directly comparable to them; the dotted line is each panel\textquotesingle s
...
every bin. (\texttt{python\ build\_\allowbreak{}class\_\allowbreak{}by\_\allowbreak{}era.\allowbreak{}py} --- the committed generator:
  ...
  disagreement is reported in Appendix~\ref{app:cut} item 11.)}
\label{fig:class-era}
\end{figure}
...
\emph{The two numeric columns are different estimands: the mean is that level\textquotesingle s raw
...
runs \emph{highest} in class I (11.3\% vs II\textquotesingle s 5.7\%, 27-segment cell, suggestive
...
is the division of labour \cref{sec:freq-descriptive} sets out rather than a contradiction of
it.\footnote{\texttt{vector\_analysis.py}; methodology and full results in
  \texttt{VECTOR-ANALYSIS.md}.}
...
\textbf{The instrument, in one paragraph.} \cref{sec:occ-classification} gives the construction in full.
In brief: each legislator\textquotesingle s prior occupation, double-blind coded to an
O*NET-SOC code, joins 64 O*NET elements assigned by blind coders to four
accountability components --- \textbf{U} upward (advising without the final say;
discretion reverse-scored), \textbf{L} lateral (external service), \textbf{D} downward
(command), \textbf{N} undirected (organisational account-giving and contact). The
registered structure is \textbf{four levels, built two semi-independent ways}: the
\emph{theoretical ladder} (free / bottom / middle / top composed from the
components) and the \emph{folk ladder} (the same four levels scored from the
...
plus the \textbf{apex delta} (MIDDLE \(-\) TOP), registered in words as "insulated
command tracks more register than exposed command."

\textbf{The four-level U is the result.} Each level entered alone
...
the post-Stage-1 amendment. The exact record, to the minute, is in Appendix~\ref{app:cut} item 24.

\textbf{The delta is the study\textquotesingle s most covariate-robust occupational number.}
Registered in words, it reads +0.049 (t 3.3) raw and \textbf{+0.067 (t 3.9)} with
...
model it costs as much adjusted \(R^{2}\) as the entire six-dummy class block
...
it, exactly the headwind the registration\textquotesingle s prediction 5 named in advance.
...
term is the free level. Asked era by era --- each level\textquotesingle s slope entered alone,
\cref{fig:altitude}\textquotesingle s own estimand, on the same member-bin values and six equal
bins as \cref{sec:class}\textquotesingle s class-era panels --- free professionals sit below their
...
Bottom, middle and top stay small and flat: no level\textquotesingle s era trend approaches
...
machines.~(\cref{fig:ladder-era})\footnote{\texttt{python\ build\_\allowbreak{}ladder\_\allowbreak{}by\_\allowbreak{}era.\allowbreak{}py}, then
...
bins as the class panels; \(\pm\)1 se bands --- narrower than the 95\% intervals used
...
the framing is retired --- Appendix~\ref{app:superseded} item 14 preserves its numbers, and the
...
for all of it: \cref{sec:limits}\textquotesingle s calibration --- small effects by any benchmark,
...
\subsection{Prominence: a gradient in the provinces, an arc in the national chambers}\label{sec:prominence}

Each member\textquotesingle s Wikipedia article length is the third status marker available
...
single slope, it splits the chambers into two shapes (Appendix~\ref{sec:prominence-full}; all 22
chambers, 6,896 members, 99\% coverage):

\begingroup\small
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.20\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.18\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.19\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.19\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.18\linewidth}@{}}
...
\emph{Cells are that quintile\textquotesingle s mean within-chamber register z; group sizes are in
the column headers. Standard errors on the pooled buckets run \(\pm\)0.026--0.027, so
the last column\textquotesingle s flatness is measured rather than merely noisy
(Appendix~\ref{sec:prominence-full}).}
...
and falling at both ends. Pooled over all 22 the buckets are flat --- \cref{tab:prominence-class}\textquotesingle s last column --- so the two shapes cancel and no single slope describes
...
Appendix~\ref{sec:prominence-full} shows the shape is not linear. Each block is also fitted alone
...
of cohort, one unit throughout: years per category against that block\textquotesingle s own
baseline. †\footnote{\texttt{python\ joint\_\allowbreak{}predictors.\allowbreak{}py}. Canonical member-level spec --- one
...
\begingroup\small
\begin{longtable}[]{@{}p{0.27\linewidth}@{\hspace{5pt}}r@{\hspace{9pt}}p{0.145\linewidth}@{\hspace{5pt}}p{0.145\linewidth}@{\hspace{5pt}}p{0.145\linewidth}@{\hspace{5pt}}l@{}}
\caption{Every member-level categorical predictor in one model: each block alone, the joint fit of cohort, all EGP classes, all education levels and prominence quintiles (n = 4{,}056), and the same fit with the occupational blocks added (n = 3{,}631); t in parentheses; the last column converts jointly significant effects into years of cohort, per category.}\label{tab:four-predictors}\\
\toprule\noalign{}
term & n & alone & joint \(-\) occ. blocks & joint & \(\approx\) yrs per category \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
term & n & alone & joint \(-\) occ. blocks & joint & \(\approx\) yrs per category \\
...
(n = 3,631): \cref{sec:occupation}\textquotesingle s two altitude ladders, each with all four levels per sd,
plus Indoors. The occ-coverage sample is thinner than \cref{sec:occupation}\textquotesingle s (3,631 against
4,762), and the level slopes below are that sample\textquotesingle s slopes, not row-for-row
restatements of \cref{sec:occupation}\textquotesingle s table. They are tabled apart because their terms are per standard
...
\begingroup\small
\begin{longtable}[]{@{}p{0.34\linewidth}@{\hspace{7pt}}p{0.18\linewidth}@{\hspace{7pt}}p{0.18\linewidth}@{\hspace{7pt}}l@{}}
\caption{The occupational blocks of the same simultaneous fit — both altitude ladders and Indoors — each alone and in the joint model with the categorical blocks (n = 3{,}631); t in parentheses. Their terms are per standard deviation, so the last column converts jointly significant effects into years of cohort per standard deviation.}\label{tab:four-predictors-occ}\\
\toprule\noalign{}
term & alone & joint & \(\approx\) yrs per sd \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
term & alone & joint & \(\approx\) yrs per sd \\
...
\(\cdot\) top & +0.008 (0.21) & +0.130 (2.09) & +5 \\
--- block Wald (4 df) & \(p < 10^{-4}\) & \textbf{p = 0.28} & \\
\textbf{folk ladder} (per sd, n 3,631) & & & \\
\(\cdot\) free & \(-0.109\) (\(-4.70\)) & \textbf{\(-0.090\) (\(-3.07\))} & \(-3\) \\
...
resolved than the rest. Prominence, entered as the bins Appendix~\ref{sec:prominence-full} says it is,
survives everything at \(p < 10^{-4}\) in every column, and the hump sits inside the
...
one is itself a datum for the introduction\textquotesingle s classification point: take the
...
master\textquotesingle s and doctorate both sit at the bachelor plateau, so the null is not a
...
\textbf{+0.92 per 1,000 words per decade (t = 12.5)} in the member-year panel with
...
throughout as the identified cohort\(-\)age combination (\cref{sec:generational}: its level is an
...
A composition check normalises each member\textquotesingle s style-word vector to its own sum
and z-scores it across 1,356 members.\footnote{\texttt{vector\_analysis.py}; methodology and full results in
  \texttt{VECTOR-ANALYSIS.md}.} No prior-free clusters emerge
...
The shape resembles Labov\textquotesingle s crossover pattern --- the second-highest status
group out-using the highest --- with the middle tier\textquotesingle s described disposition
...
Jhering set out the diffusion mechanism in \emph{Der Zweck im Recht} (1883);
Durkheim\textquotesingle s summary supplied the phrase --- fashion, once universally adopted, is
...
\textbf{Flight: the groups at the top avoid the words that became common.}
What distinguishes the top strata is \emph{which} words they avoid. \textbf{The lift is
decoupled by construction}: a word\textquotesingle s rise is computed from the classes
outside both comparison strata, so a word cannot be high-lift \emph{because} the
compared groups moved it.\footnote{The original specification computed lift from the all-class
  ...
  kept as the record of the earlier spec.} Under that spec, the correlation
between a word\textquotesingle s rise and class I\textquotesingle s relative use of it against class II is
negative at every volume threshold. Class I under-uses \texttt{primary} (\(2.35\times\)
...
the within-member evidence of \cref{sec:careers} shows sitting members do move, at about
...
generally, not to one coding of class.\footnote{\texttt{python\ build\_\allowbreak{}ladder\_\allowbreak{}word\_\allowbreak{}year.\allowbreak{}py} (one provincial scan;
...
\caption{Flight and chase by minimum-occurrence threshold: the correlation between a word's rise and each group's relative use of it against its comparison group.}\label{tab:flight-thresholds}\\
\toprule\noalign{}
contrast (top vs peak) & \(\geq\)100 & \(\geq\)300 & \(\geq\)800 & \(\geq\)1,500 \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
contrast (top vs peak) & \(\geq\)100 & \(\geq\)300 & \(\geq\)800 & \(\geq\)1,500 \\
...
\textbf{folk bottom vs middle (chase)} & \textbf{+0.11} & \textbf{+0.25} & \textbf{+0.43} & \textbf{+0.28} \\
\end{longtable}
\endgroup

\emph{Spearman \(\rho\) between a word\textquotesingle s rise and the first-named group\textquotesingle s relative use of
...
status-top\textquotesingle s avoidance --- mostly out of the game, which is what sitting
outside the hierarchy predicts: its low register level (\cref{sec:occupation}) is closer to
uniform abstention than to word-selective avoidance. That is a directional
reading --- at these word counts, equivalence within \textbar \(\rho\)\textbar{} \textless{} 0.15 is not
formally establishable. The folk-bottom series is the battery\textquotesingle s only
...
from one instrument\textquotesingle s levels, at group level.
...
\textbf{The word level adds the first direction-of-diffusion evidence.} Fitting
each word\textquotesingle s adoption-crossing date per tier, \textbf{the top tier\textquotesingle s crossing
precedes the bottom\textquotesingle s} --- by +0.27 years on the class coding (Wilcoxon \(p < 10^{-4}\) over 282 words; +0.15 on the folk ladder, p = 0.003) --- the order a
...
direction, not yet a diffusion measurement.\footnote{\texttt{python\ leadlag.py}, on the panel member cache: takeoff =
...
most conspicuous forms first. \emph{Flight} appears here in class I\textquotesingle s avoidance of
words that became common, and dynamically at the word level, where the most
conspicuous machine tells peak and then recede (\cref{sec:acceleration}). The class cycle itself
is \textbf{not} observed in motion. Neither sufficient signature is present: the
...
member-year weighting is in Appendix~\ref{app:cut} item 11).\footnote{\texttt{python\ class\_\allowbreak{}gap\_\allowbreak{}trend.\allowbreak{}py}, from \texttt{class\_\allowbreak{}by\_\allowbreak{}era\_\allowbreak{}grouped.\allowbreak{}csv} (the
  ...
  across the six equal five-year bins, 1997--2026.} A cycle
...
deep archive reaches that far, and occupations are recoverable (Appendix~\ref{app:future}).

\section{Limits}\label{sec:a-limits}

\textbf{The unit is the occupation, not the member.} Every level score is a
...
\textbf{The levels of one ladder cannot be read one at a time inside a joint fit.}
...
signs --- so entered together they are near-collinear, and a level\textquotesingle s partial
coefficient measures only the sliver its siblings leave unexplained. The
preregistration\textquotesingle s rule that a ladder\textquotesingle s levels never enter together (Appendix~\ref{app:superseded}, item 14) governs every level-wise result in \cref{sec:occupation}; where \cref{sec:four-predictors} enters a
ladder as a block, only the block test is interpretable, and the per-level
rows there are not.

\textbf{The effects are small.} In the field\textquotesingle s common currency the insulation
delta is a correlation of r \(\approx\) .07 and the class block r \(\approx\) .10 --- at or below
the typical published effect in individual-differences research. \cref{sec:limits}
...
\textbf{The measure explains destination, not origin.} It scores the job a member
...
from those occupations speak as they do; the interpretation offered in \cref{sec:chase-flight}
is exploratory and labelled as such.

\textbf{Coverage is not complete.} 4,762 members carry both register and
instrument; 3,594 carry every covariate, and the joint model runs on the 3,631
with occupational-score coverage. An occupation missing an element of a level\textquotesingle s signature
cannot be scored at all (\cref{sec:occ-classification}).

\section{Discussion}\label{sec:a-discussion}
...
a short lineage. Erikson and Goldthorpe\textquotesingle s EGP schema is the categorical
...
care. Whether the recovered structure is \emph{more useful} than what the market
prices is an open question that rests on an assumption: that position, as the
register instruments it, is the referent the market\textquotesingle s scales summarise
...
paper\textquotesingle s own hypothesis is the counter-reading: social structures --- spoken
registers among them --- often function to \textbf{mask} disparities in productivity,
...
(Appendix~\ref{app:future}, item 28): tie register intensity to productivity and success
measures. Signaling drag --- register predicting \emph{worse} real outcomes ---
favours the masking reading; register tracking outcomes the market\textquotesingle s
...
\part*{Part B --- Prevalence, history, and mechanism}\setcounter{section}{0}\renewcommand{\thesection}{B\arabic{section}}
...
not depend on the detector, because the human baseline is itself moving. \cref{sec:intro}
sets out both instruments and the division of labour between them; this part
reports what they found.

The order follows the evidence. \cref{sec:calibration}--\cref{sec:genre} establish the detector\textquotesingle s
...
chambers, and a concentration in scripted business. \cref{sec:register-shift}--\cref{sec:generational} turn to the
...
gradient older than the drift. \cref{sec:social-structure} states what Part A establishes about who
carries it. \cref{sec:permeation}--\cref{sec:evasion} close on the machines: permeation into human speech,
...
Part A\textquotesingle s measure enters this part twice: as the occupational structure
summarised at \cref{sec:social-structure}, and where the post-training results compare the
...
and has no uniform prevalence sample, and Tasmania\textquotesingle s transcription regime
shifted mid-corpus, leaving no usable control (\cref{sec:hazards}). The nineteen-chamber
constant-window register series of \cref{sec:climb-geography} is a different set again: it adds
...
\item
  ...
  CA-FED\textquotesingle s seven late controls were redrawn to the rule (\cref{sec:prevalence-audits});
...
longer --- the whole of the record Pangram\cite{pangram2024} will read. Segments are drawn at a
uniform rate across lengths, so the sample reproduces the corpus\textquotesingle s own length
...
band-reweighting constants (\cref{sec:prevalence}). Sampling is seeded per cell and
reproducible.\footnote{\texttt{build\_\allowbreak{}pangram\_\allowbreak{}expansion.\allowbreak{}py}, \texttt{build\_shortband.py}.}
...
\textbf{Specificity is therefore measured in-domain at two grains at once}: each
...
while the pooled 1,260 carries the statistical weight (\cref{sec:calibration}). The
...
That two-grain calibration is the study\textquotesingle s most consequential design choice,
and the reason its result differs from published comparators (\cref{sec:related}). The
comparators calibrate more coarsely: the closest parliamentary study\cite{suvanto2026} measures false positives on one pre-LLM hold-out per parliament, in two
parliaments; population-level studies\cite{liang2024monitoring} validate per venue against
semi-synthetic ground truth; and commercial-detector exercises have leaned
on a single fifty-speech calibration\cite{rice2026} or on vendor benchmarks alone.
...
When \(\mathrm{Sp} = 1\), the expression reduces to \(\tau = \pi/\mathrm{Se}\). \textbf{\(\mathrm{Se}\) is not estimated.} Because detector sensitivity cannot exceed 1, \(\tau = \pi/\mathrm{Se} \geq \pi\): under the specificity measured in \cref{sec:calibration}, every flag is genuine machine text and some machine text goes unflagged, so the observed flag rate is a conservative floor on true prevalence --- the detector\textquotesingle s misses lower the reading and never inflate it.

That floor assumes specificity measured on pre-2022 speech still holds for human speech in 2025--26, while the permeation result shows contemporary human speech drifting toward the register the detector reads. \cref{sec:calibration} bounds that assumption on the audited hits at no more than about 0.2 points; the Limits return to it.
...
So the API and dashboard routes are interchangeable \emph{only when the model is
named}, and the failure to name it is silent: you get verdicts, they just
aren\textquotesingle t the same instrument. Billing corroborates the diagnosis independently
--- the dashboard\textquotesingle s \texttt{ceil(words/100)} credit rule is exactly Pangram 4\textquotesingle s
\$0.05-per-100-words, while the defaulted API billed per document at Pangram
3\textquotesingle s 1,000-word unit.
...
false-positive problem (Appendix~\ref{app:cut} item 19).
...
the original verdicts \textbf{360 out of 360}.

\subsection{Why the register instrument is descriptive, not inferential}\label{sec:freq-descriptive}
...
The arm is therefore reported as a \textbf{descriptive series} --- which is what lets
the 1994--96 onset (\cref{sec:register-shift}) count as a result: the register was already moving
...
own occurrence rate for Kobak\textquotesingle s 407 style words --- separates Pangram-flagged
from unflagged legislative segments at \textbf{AUC 0.61} (0.60 on the 2025--26
prevalence sample alone, where both classes are drifted contemporary speech).
The study\textquotesingle s Opus screen, a separate model-based judge of the same distinction,
reaches 0.95 (Appendix~\ref{sec:screen-pangram}). On ground-truth evasion rewrites the register
threshold falls to \textbf{0.535}, barely above chance --- but that set is
adversarially quietened text, not ordinary model output, which runs far hotter
on these words (\cref{sec:posttraining}). One thin lexical rate carries population-level history
well and instance-level detection barely above chance: the division of labour
the study uses.\footnote{\texttt{python\ register\_\allowbreak{}auc\_\allowbreak{}check.\allowbreak{}py}: 5,439 text-resolved
...
Two instruments, run separately, carry the results. \emph{Pangram}, calibrated
against each chamber\textquotesingle s own pre-2022 record, answers the prevalence question
segment by segment. The \emph{register instrument} --- Kobak\textquotesingle s excess-vocabulary
style words, counted against each chamber\textquotesingle s own placebo-matched
...
that order: calibration and prevalence (\cref{sec:calibration}--\cref{sec:prevalence}) and the genre ladder
(\cref{sec:genre}); the register\textquotesingle s thirty-year history and its cross-chamber convergence
(\cref{sec:register-shift}--\cref{sec:acceleration}); the people who carry it --- birth year, then social structure
(\cref{sec:generational}--\cref{sec:social-structure}); permeation into human speech (\cref{sec:permeation}); the machine side of the
register (\cref{sec:posttraining}); deliberative quality (\cref{sec:quality}); and directed evasion (\cref{sec:evasion}).
...
\textbf{Zero false positives across every chamber\textquotesingle s own pre-AI control.} None of
the 1,260 pre-2022 control speeches is flagged: observed specificity is
\textbf{100\% --- 1,260 of 1,260} {[}99.7\%, 100.0\%{]} (Wilson).

Sensitivity is not estimated from our own data. The prevalence estimates use
\(\mathrm{Sp} = 1\) and \(\mathrm{Se} = 1\); under the zero-false-positive transfer assumption, any
real sensitivity \(\mathrm{Se} \leq 1\) makes the observed flag rate a \textbf{conservative
floor} on true prevalence, so the \cref{sec:prevalence} figures understate machine text when
the detector misses it. The strongest external anchor is Pangram\textquotesingle s
...
points in the same direction: Pangram flagged \textbf{all 40} synthetic legislative
speeches generated by an open model, a 100\% hit rate that is underpowered but
consistent with the published recall. Sensitivity on \emph{edited} machine text
remains the central unknown and is discussed in §6.\footnote{\texttt{prevalence\_report.py}.}
...
deixis to the room. We audited \textbf{all 316 flagged prevalence segments} in two
...
confined to prepared text: \textbf{277 prepared, 21 unclear, and 18 spontaneous} on
the first vote, \textbf{11 of the 18 still spontaneous on the re-vote} --- and \cref{sec:genre}\textquotesingle s
...
best candidates for contemporary false positives in the study. Treating \textbf{all
...
where contemporary authorship is near-certain, the floor\textquotesingle s transfer assumption
costs at most about a fifth of a point.\footnote{\texttt{python\ build\_\allowbreak{}flagged\_\allowbreak{}hits\_\allowbreak{}pool.\allowbreak{}py} assembles the 316 hits with
...
\textbf{7.36\% {[}6.13\%, 8.62\%{]} in 2025 to 11.70\% {[}9.86\%, 13.67\%{]} in 2026}.\footnote{\texttt{python\ prevalence\_\allowbreak{}robustness.\allowbreak{}py} --- the year split at the
...
Every estimate that follows is the share of \emph{what was said} that is
...
\caption{The twenty chamber rates: each point is the chamber\textquotesingle s word-weighted, fraction-weighted machine-drafted share of 2025--26 words; whiskers are the cluster-bootstrap 95\% CIs; chambers sorted by rate, direct-labelled, with the two US chambers emphasised --- their contrast recurs in \cref{sec:climb-geography} and Appendix~\ref{app:future} --- and the pooled 9.03\% marked as a reference line. Axis starts at zero. Exact values and intervals are tabulated in Appendix~\ref{app:cut} item 21.}
\label{fig:prevalence-dots}
\end{figure}

The spread is the finding, not noise around a mean. A random-effects fit
across the twenty chambers puts their rates at \textbf{4.3\% to 18.3\%, a
better-than-fourfold range}, after every chamber is pulled toward the pool in
proportion to its own noise. The fit\textquotesingle s own diagnostics say the spread is real:
logit-scale \(\tau\) = 0.56, and I\({}^{2}\) = 0.78 --- 78\% of the observed between-chamber
...
(the raw-extremes version is retired to Appendix~\ref{app:superseded}). What drives a chamber\textquotesingle s
position is not established here --- even two chambers of one legislature can
sit far apart (Appendix~\ref{app:future} item 8), and the study does not attribute the
between-chamber differences to any measured cause.

\emph{Rates are weighted by words because a segment is not a natural unit.} The
segmenter packs speaker turns into windows of at most 360 words, so a long
speech becomes three segments and a short interjection becomes one.\footnote{\texttt{segment.py}.} A
...
against the segment-weighted alternatives is in Appendix~\ref{app:cut} item 9.

No one large chamber drives the result: the equal-weight mean of the twenty per-chamber rates (\cref{tab:prevalence}, drawn in \cref{fig:prevalence-dots}) is 9.12\%, against the 9.03\% word-weighted pooled rate. Uniform
...
(Appendix~\ref{app:cut} item 2).\footnote{\texttt{banded\_prevalence.py}.}
...
segment\textquotesingle s words as fully machine over-states the rate by a third --- \textbf{12.03\%
...
harvested Mixed segments average \textbf{0.435} machine (n=132, sd 0.219, range
...
already the split-corrected figure, not an AI-or-Mixed count.\footnote{Fractions live in \texttt{fraction\_\allowbreak{}ai\_\allowbreak{}harvested.\allowbreak{}json} (151 harvested
...
middle of \cref{sec:genre}\textquotesingle s genre ladder.
...
segment counts cannot express that.\footnote{\texttt{python\ -\allowbreak{}c\ "import\ banded\_\allowbreak{}prevalence\ as\ B;\ B.\allowbreak{}table(B.\allowbreak{}load())"}.}
...
segments moves the pooled interval imperceptibly (design effect 1.23).\footnote{\texttt{python\ prevalence\_\allowbreak{}robustness.\allowbreak{}py} --- the year split at the
  banded headline estimand, the speaker-clustered bootstrap, and the
  random-effects fit behind the shrunken spread below.}

The low rows carry a second caveat, about calibration rather than sampling. No
chamber\textquotesingle s rate below \textasciitilde6\% is calibration-exact on its own 60/60 control alone,
because that control\textquotesingle s Wilson lower bound is 94\%. Under the pooled floor
(\(\geq\)99.7\%), false positives contribute at most 0.3 points to any chamber\textquotesingle s
rate --- slack that matters only for the lowest rows (US Senate 1.8\%, UK 2.5\%),
not for the spread.

\subsubsection{Estimate-specific audits}\label{sec:prevalence-audits}
...
and the uniform draw is what \cref{tab:prevalence} in Appendix~\ref{app:cut} item 21 reports. Dropping CA-FED entirely leaves
...
which is exactly where drafting concentrates (\cref{sec:genre}). The chamber\textquotesingle s controls are
clean before and after the repair, \textbf{0 of 18,342 words}. The extraction
defect, the 275-segment redraw and the checks on both are in Appendix~\ref{app:cut} item
22.
...
\begingroup\small
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.18\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.13\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.16\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.13\linewidth}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{SO31} --- one-minute scripted set-pieces & \textbf{32.3\%} & {[}21.5, 44.1{]} & 36.7\% & 35.8\% \\
Government Orders --- mixed prepared and spontaneous & 19.9\% & {[}10.5, 30.5{]} & 23.3\% & 25.7\% \\
\textbf{Oral Questions} --- not on notice & \textbf{9.8\%} & {[}2.2, 19.1{]} & 8.3\% & 11.8\% \\
\emph{ratio of the two rungs above} --- \textbf{SO31 ÷ Oral Questions} & \textbf{\(3.29\times\)} & --- & \(4.40\times\) & \(3.03\times\) \\
\end{longtable}
\endgroup

\emph{The first rate column is the study\textquotesingle s estimand throughout --- word-weighted and
weighted by each flagged segment\textquotesingle s own AI fraction. The last two columns are the
...
measures the packer\textquotesingle s behaviour across genres alongside the drafting. The
ordering of the three rungs is the same under all three weightings.}

All three pre-AI controls, pooled across genres: 0/180, \textbf{0.0\%} {[}0.0, 2.1{]}.

\textbf{SO31 vs OQ: \(3.29\times\), +22.5pp, permutation p = 0.0025.} The test is a label
permutation rather than Fisher exact, because Fisher takes segment counts and
the estimator is a ratio of summed words.\footnote{\texttt{python\ prevalence\_\allowbreak{}report.\allowbreak{}py} for the rates and bootstrap intervals;
...
\textbf{Weighting by the AI share widened this ladder rather than flattening it},
...
of 60) gives z = 3.70, \(p = 2.2\times 10^{-4}\).\footnote{\texttt{python\ prevalence\_\allowbreak{}report.\allowbreak{}py} for the rates and bootstrap intervals;
  ...
  the Cochran--Armitage trend statistic on the segment counts (doses
  OQ \textless{} DEBATE \textless{} SO31); the adjacent-rung Fisher exacts are computed from the
  \(3\times\)2 segment table.} The adjacent steps are thin on
...
drift \cref{sec:register-shift} post-stratifies away, and the register\textquotesingle s thirty-year climb
survives that test.
...
The Oral Questions rate is \textbf{mislabelled rather than wrong} --- 9.8\%
word-weighted in \cref{tab:genre}, 8.3\% on segments. It estimates the long-tail business carrying
...
to date it: UK Commons, extended to 1985, where the register \emph{declines}
through the late 1980s and then turns upward at that window. The date is
therefore that series\textquotesingle{} property rather than a pooled one --- only three
...
\textbf{The ruler, audited --- what the thirty-year rise is made of.} We selected the marker list on the post-2022 jump and then ran it backwards. That design invites two objections: a few words may carry the rise, or selection on the endpoint may create a trend by construction. We tested both.\footnote{\texttt{python\ build\_\allowbreak{}word\_\allowbreak{}year\_\allowbreak{}counts.\allowbreak{}py} (one scan, 1.62B tokens;
...
A few words do carry most of the level change. \textbf{Ten common function words carry 92.5\% of the 1994--96 \(\rightarrow\) 2024--26 climb} (\texttt{this} alone 35\%); with those ten removed, the aggregate is nearly flat (\(\times\)1.03 pooled, \(\times\)1.10 UK). Beneath that concentration lies \textbf{breadth}: the expected number of distinct style words per 1,000 tokens climbs \(\times\)1.26 on the UK series (14.4 \(\rightarrow\) 18.2), while 60.4\% of the 407 words trend positive (z = +4.2) at a median of only +0.5\%/yr. The result is a shallow, broad widening beneath a few surging function words, not a 407-word chorus.

Composition does not produce the rise. Post-stratifying every chamber-year to a fixed segment-length distribution --- the measurable proxy for the genre drift \cref{sec:genre} makes threatening --- leaves the onset in place and slightly \emph{strengthens} the UK climb (\(\times\)1.49 \(\rightarrow\) \(\times\)1.54). Nor does one ruler produce it. The independent Wikipedia signs-of-AI-writing patterns, built by different people for a different purpose, rise across the whole pre-LLM record and \emph{faster} (+2.7\%/yr against +1.4\%/yr, UK 1994--2019). Their UK trough sits in 1987, however, so the decades-long pre-LLM rise is a cross-instrument fact while the specific 1994--96 date belongs to this ruler\textquotesingle s UK series.

Matched donors separate two questions, and the answers differ. Did the register\textquotesingle s own level turn upward in 1994? Against donors matched on frequency and dispersion, the UK series\textquotesingle{} interior minimum at 1994 survives. Did the register pull away from ordinary vocabulary of the same shape? Against donors also matched on pre-2010 trend --- a control that by construction absorbs any onset inside its own window --- that \emph{distinctive} excess dates to roughly \textbf{2014--2018}, and the style words remain below their matched donors in level throughout the record until 2025. Both answers stand together: the drift is real, broad-based, composition-robust and instrument-robust; its dated 1994--96 turn belongs to the UK ruler; and its excess over matched controls is younger than the turn.

\subsubsection{Other chambers converge on the level the United States already held}\label{sec:climb-geography}
...
chambers hold both endpoints~(\cref{fig:convergence}), and on that single ruler the pattern is
...
Edward Island), and none of the nineteen approaches the UK\textquotesingle s +4.0\%/yr.\footnote{\texttt{python\ constant\_\allowbreak{}window\_\allowbreak{}trend.\allowbreak{}py}, reading
  ...
  in Appendix~\ref{sec:climb-table}\textquotesingle s table is descriptive.}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{convergence_slopegraph.png}
\caption{The convergence, drawn as a slopegraph: one line per chamber from its 2006 to its 2026 gap-vocabulary rate (occurrences per 100k words; values exactly as in \cref{tab:climb} in Appendix~\ref{sec:climb-table}), direct-labelled at both ends, the UK-group climbers and the two flat US chambers emphasised. The fan closes upward: the lowest 2006 starters climb steepest toward the level the United States already held.}
...
A US series that starts in 2006 cannot distinguish "the United States was always there" from "the United States got there first and stopped." Extending the Congressional Record to its 1994 beginning settles it.\footnote{\texttt{python\ us/\allowbreak{}fetch\_\allowbreak{}us\_\allowbreak{}hist.\allowbreak{}py\ -\allowbreak{}\/\allowbreak{}-\allowbreak{}years\ 1994-\allowbreak{}2005,2023\ -\allowbreak{}\/\allowbreak{}-\allowbreak{}per-\allowbreak{}year\ 60\ -\allowbreak{}\/\allowbreak{}-\allowbreak{}tag\ US\_\allowbreak{}DEEP\_\allowbreak{}DONE}, then re-extract with \texttt{us/\allowbreak{}us\_\allowbreak{}extract.\allowbreak{}py\ zips\ segments\_\allowbreak{}us.\allowbreak{}jsonl} and rebuild. 780 sitting-day zips at 60 days/year;
  ...
  sampling costs precision and not validity.} Three chambers reach that far, and the US point rests on the longer window:

\begingroup\small
\begin{longtable}[]{@{}l>{\raggedright\arraybackslash}p{0.19\linewidth}>{\raggedright\arraybackslash}p{0.19\linewidth}>{\raggedright\arraybackslash}p{0.17\linewidth}>{\raggedright\arraybackslash}p{0.16\linewidth}@{}}
\caption{The longest window: the three chambers whose series reach 1994.}\label{tab:climb-long}\\
\toprule\noalign{}
chamber & gap 1994, per 100k words & gap 2026, per 100k words & 1994\(\rightarrow\)2026 ratio & annual change \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
chamber & gap 1994, per 100k words & gap 2026, per 100k words & 1994\(\rightarrow\)2026 ratio & annual change \\
...
\bottomrule\noalign{}
\endlastfoot
UK Commons & 380 & 1,444 & \(\times\)3.80 & +4.3\%/yr \\
\textbf{US House} & 1,515 & 1,709 & \(\times\)1.13 & +0.4\%/yr \\
\textbf{US Senate} & 1,314 & 1,562 & \(\times\)1.19 & +0.5\%/yr \\
\end{longtable}
\endgroup

Only the UK and the two US chambers hold 1994 (the UK\textquotesingle s own series extends
to 1985, \cref{sec:register-shift}), and on that longest window the contrast sharpens: the US
chambers \emph{start} above where the UK still sits two decades later --- US House
at 1,515 per 100k in 1994 against the UK\textquotesingle s 1,444 in 2026. Ireland and
...
own floor. On 2020--26 means, with the gap to US House scaled by US House\textquotesingle s own
variability and each series\textquotesingle{} standard deviation in the last column:

\begingroup\small
\begin{longtable}[]{@{}l@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.13\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.17\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.17\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.13\linewidth}@{}}
\caption{Register level by chamber, 2020--26 means, benchmarked against the US House.}\label{tab:vs-ushouse}\\
\toprule\noalign{}
group & mean, per 100k & gap to US House, per 100k & gap ÷ US House\textquotesingle s sd (121) & sd of own series \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
group & mean, per 100k & gap to US House, per 100k & gap ÷ US House\textquotesingle s sd (121) & sd of own series \\
...
\bottomrule\noalign{}
\endlastfoot
CA provinces & 2,054 & \textbf{+155} & \textbf{+1.28} & 61 \\
...
the others past --- fails on the arithmetic. The shares here are of \emph{instrument
occurrences}, not of words, so they do not sit on the same scale as \cref{sec:prevalence}\textquotesingle s
prevalence figures: US House is \textbf{14.3\% machine by instrument occurrences}
against Ireland\textquotesingle s 9.0\% and the Australian states\textquotesingle{} 11.3\%, so removing machine
...
\cref{sec:prevalence}), dropping from apparently-above to clearly-below once corrected.\footnote{\texttt{python\ convergence\_\allowbreak{}check.\allowbreak{}py}, with shares from
...
parliamentary register" but that \textbf{human register had been moving, for thirty
years, toward what instruct-tuning later selected for.} †\footnote{Daggered numbers are carried from earlier in the study and were
  re-derived from their artifacts; the full key and per-number sources are
  in the provenance note in Appendix~\ref{app:repro}.}
...
Every member-year of speech carries two clocks: the year the words were \textbf{spoken} and the speaker\textquotesingle s \textbf{birth year}. We regressed the member-year register rate on both clocks together, with chamber fixed effects, across all 22 chambers (60,186 member-years, 6,913 members). Both clocks predict, net of each other:

\begingroup\small
\begin{longtable}[]{@{}l@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.26\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.20\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.24\linewidth}@{}}
\caption{The two clocks in one fit: the member-year register rate on birth year and spoken year together, with chamber fixed effects, across all 22 chambers.}\label{tab:two-clocks}\\
\toprule\noalign{}
...
\subsection{Social structure}\label{sec:social-structure}
...
contrasts once entered beside them (\cref{sec:occupation}, \cref{sec:four-predictors}). The juniority gradient of
\cref{sec:generational} survives all of it.

\subsection{Permeation: detector-independent and small but positive}\label{sec:permeation}

\textbf{The drift predates the models.} In a window where no machine text is
possible in either cell, 2018--19 against 2021 to November 2022, the register
moves \textbf{+0.0150} (positive in 7 of 10 chamber \(\times\) family cells, permutation
p = 0.017) on the identical scored occurrences.\footnote{\texttt{python\ word\_\allowbreak{}context\_\allowbreak{}prellm.\allowbreak{}py}, reusing the committed occurrence
  ...
  about twice the pre-to-post rate (\textasciitilde+0.0022/yr).} That is permeation
...
\textbf{+0.0099} (9 of 10 cells, permutation p = 0.022)†\footnote{\texttt{python\ word\_\allowbreak{}context\_\allowbreak{}delta.\allowbreak{}py\ pooled}, which prints the pooled figure,
  ...
  3,000 draws. Per-model cell table at \texttt{METHODOLOGY.md:1009}.}, but its post-era
cells contain machine-drafted text (\textasciitilde9\% of words) and cannot separate continued
...
The interval is reported as approximately \textbf{{[}0.000, +0.020{]}}. The point
estimate and sign count are exact, and the inferential routes agree: P(\(\leq\)0) is
...
correctness --- the design\textquotesingle s placebo. The evidence is an OLMo-2 ladder run on
...
because DPO\textquotesingle s generations are longest. Three independent corrected routes
agree on the picture below; the exact stratified estimator is the one shown.
Corrected, the stage shifts are \textbf{SFT +0.32, DPO +0.27 and RLVR +0.06} --- the
RLVR figure with a confidence interval straddling zero.

SFT and DPO contribute \textbf{indistinguishably} (paired
...
control pedestal\textquotesingle s artifact, not the data (Appendix~\ref{app:superseded}).\footnote{Ladder stages: \texttt{python\ olmo\_\allowbreak{}ladder.\allowbreak{}py\ report}, which prints the
...
manipulation check. The end-to-end base\(\rightarrow\)instruct shift (+1.24) is untouched by
the correction.
...
data isn\textquotesingle t chat" cannot be separated here. Three checks bound what can be
said.\footnote{\texttt{python\ ladder\_\allowbreak{}length\_\allowbreak{}control.\allowbreak{}py} (reproduces the shipped
  ...
  \texttt{python\ olmo\_\allowbreak{}urial\_\allowbreak{}arms.\allowbreak{}py}.}

The stage pattern is \textbf{length-robust}: truncating
...
The \textbf{prompting arms} bound the surface share directly. The committed ladder
used raw prompts at every stage, so its end-to-end base\(\rightarrow\)instruct shift (+1.24)
...
\emph{raw base model} reaches 46.7 per 1,000, against base 27.2 \(\rightarrow\) instruct 44.0 on
identical raw prompts --- \textbf{116\% of the base\(\rightarrow\)instruct span}. The instruct model
...
carries; post-training moves the \emph{default} of unconditioned generation, and
deployment conditions select far above that default. The ladder measures the
default\textquotesingle s installation; the ceiling was always promptable.

The tempting stronger story --- \emph{that the preference data itself prefers the
register} --- \textbf{does not survive its controls}. Across 601k chosen-vs-rejected
...
ladder carries the stage claim alone. \cref{sec:acceleration}\textquotesingle s reading --- that alignment
concentrates something humans already favoured --- stays consistent with this as
an association rather than an identified mechanism, and the section\textquotesingle s argument
no longer rests on it.

A separate experiment pools the same measurement over three other model
families: \textbf{+0.387} on the scaled generation --- 1,600 prompt pairs each, 1.19M
base words.
...
\subsubsection{Coverage: post-training moves the model\textquotesingle s vocabulary onto Hansard\textquotesingle s}\label{sec:coverage}
...
generated corpus has by far the fatter zero bin --- \textbf{158} style words never
occur in base generation and \textbf{118} in instruct, against \textbf{86} in pre-2023
Hansard and \textbf{85} in 2025--26 Hansard --- so part of the missing coverage is a
property of the generation rather than of the word list. The full distribution
over occurrence bins is in Appendix~\ref{app:cut} item 25.\footnote{\texttt{python\ style\_\allowbreak{}word\_\allowbreak{}frequency.\allowbreak{}py}. Three families at 1,600 prompts;
...
\subsubsection{Post-training aligns weakly with pre-model American usage}\label{sec:american-alignment}
...
\begingroup\small
\begin{longtable}[]{@{}lll@{}}
\caption{Post-training preference against the contrasts that separated national usage before the models: Spearman correlations, raw and holding word frequency fixed.}\label{tab:american-alignment}\\
\toprule\noalign{}
...
\subsubsection{Model lineage in the word mix}\label{sec:model-lineage}
...
other. This is register lineage, not attribution (Appendix~\ref{app:future} item 3).\footnote{\texttt{vector\_analysis.py}; methodology and full results in
  \texttt{VECTOR-ANALYSIS.md}.}

Register \emph{rate} is also a lineage property: Opus sits at base-model rate
...
2 of the study\textquotesingle s one grading campaign, whose later stages grade the evasion arms
(\cref{sec:evasion-quality}, stages 3--5) and the never-reviewed continuations (\cref{sec:stage6}, stage 6).
\textbf{Stage 1} is 840 genre-balanced federal-Canadian segments labelled by a
blinded, date-blind LLM screen of 37,801 segments; on the 618 segments where
that screen overlaps Pangram it separates Pangram\textquotesingle s classes at AUC 0.954 {[}0.934,
0.971{]} (Appendix~\ref{sec:screen-pangram}), so its score reads as a machine-involvement signal where
no Pangram verdict exists. \textbf{Stage 2} is 682 segments across 22 chambers
labelled by the \textbf{Pangram verdict} itself, with chamber fixed effects.
Different populations, different label sources, same answer --- and one half of
the original answer does not survive the controls below.\footnote{Stage 1: \texttt{cd\ quality\_\allowbreak{}expansion\ \&\&\ python\ analyze.\allowbreak{}py}. Stage 2:
...
instrument\textquotesingle s own published inter-coder bar (justification r = 0.716), and the
...
\bottomrule\noalign{}
\endlastfoot
justification & \textbf{+0.173} (t +4.1) ✱ & \textbf{+0.290} (t +4.4) ✱ \\
common\_good & \textbf{+0.089} (t +3.3) ✱ & \textbf{+0.229} (t +4.5) ✱ \\
respect\_groups & \textbf{+0.044} (t +2.0) (✱)\footnote{The one starred cell that does not survive multiple-testing
  ...
  judge leakage.} & \textbf{+0.220} (t +6.1) ✱ \\
respect\_demands & \(-0.027\) (t \(-0.5\)) & \(-0.029\) (t \(-0.4\)) \\
respect\_counterargs & +0.022 (t +0.4) & +0.073 (t +1.0) \\
constructive & \(-0.002\) (t \(-0.1\)) & \(-0.041\) (t \(-1.0\)) \\
evidence & +0.054 (t +1.4) & +0.065 (t +1.1) \\
\end{longtable}
\endgroup

\emph{Stage 1 reports effects per sd of the blinded screen score (sd 15.3 on the 0--100 scale, observed maximum 70), the comparison commensurable with stage 2\textquotesingle s binary contrast; unscaled fitted differences are in Appendix~\ref{app:cut}. Six cells carry the claim --- justification, common\_good and respect\_groups in each stage. ✱ marks the five that hold q \textless{} 0.003 under Benjamini--Hochberg across the table\textquotesingle s fourteen cells; (✱) marks the sixth, which does not survive correction and is disclosed in its footnote.}
...
against \(\pm\)0.22, the study\textquotesingle s own smallest substantive effect, the four null
...
\textless0.0001, 0.005). The three claimed positives fail equivalence, as they
should.\footnote{\texttt{python\ tost\_\allowbreak{}equivalence.\allowbreak{}py} --- SESOI fixed at \(\pm\)0.22 raw points,
...
reads. \textbf{Justification is a suppression effect, not a new result.} It is null
uncontrolled (t \(-0.2\)) and strongly positive within genre (t +4.1), because
machine drafting concentrates in SO31 and \textbf{SO31 has the lowest justification
...
a within-chamber log-length term, justification is \textbf{+0.290 raw \(\rightarrow\) +0.157 (t 2.4)
at fixed length} --- about half the lift travels through longer speech and half
survives without it; \textbf{respect\_groups is length-free} (+0.220 \(\rightarrow\) +0.222);
common\_good barely moves (+0.229 \(\rightarrow\) +0.199, t 3.8); and evidence\textquotesingle s marginal
+0.065 is entirely length (\(\rightarrow\) \(-0.017\)), so flagged text has more checkable
specifics only by being longer, consistent with stage 6\textquotesingle s
...
\subsubsection{Applicability: less to engage with}\label{sec:applicability}

\textbf{Applicability itself collapses at high screen
scores} --- the share of segments on which each sentinel dimension can be
scored at all, by band of the blinded screen score:\footnote{\texttt{applicability\_\allowbreak{}by\_\allowbreak{}band.\allowbreak{}py}.}

\begingroup\small
\begin{longtable}[]{@{}lr>{\raggedleft\arraybackslash}p{0.26\linewidth}>{\raggedleft\arraybackslash}p{0.26\linewidth}@{}}
\caption{Applicability of the two sentinel dimensions, by band of the blinded screen score.}\label{tab:applicability}\\
\toprule\noalign{}
screen score (0--100) & segments & respect\_demands applicable & respect\_counterargs applicable \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
screen score (0--100) & segments & respect\_demands applicable & respect\_counterargs applicable \\
...
\bottomrule\noalign{}
\endlastfoot
\textless{} 10 & 498 & 61.4\% & 50.8\% \\
10--49 & 297 & 63.0\% & 51.2\% \\
\textbf{\(\geq\) 50} & \textbf{45} & \textbf{31.1\%} & \textbf{28.9\%} \\
\end{longtable}
\endgroup

The engagement null of \cref{sec:quality} is conditional: AI-flagged speech \emph{engages no worse
when there is something to engage with}. Applicability is a separate outcome,
and there the groups differ: AI-flagged speech is about \textbf{half as likely to
...
\subsubsection{Unreviewed continuations reveal a capability ladder}\label{sec:stage6}
...
prompts (45-word openings of real segments), each with its member\textquotesingle s actual
...
the stage-1/2 model.\footnote{\texttt{quality\_\allowbreak{}expansion/\allowbreak{}pool6.\allowbreak{}json} (blind), \texttt{key6.json} (never left
...
\textbf{The answer is a capability ladder, not a constant.} The open-weight arms
...
arms --- the study\textquotesingle s existing audited generation caches (the \cref{sec:model-lineage}
family-signature traces), graded blind under the identical flow --- invert that
at the frontier:

\begingroup\small
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\emph{(human twins --- the reference)} & --- & 0.66 & 0.53 \\
...
\textbf{The applicability collapse is a capability artifact, not a machine
constant}: opus-class arms carry as much to engage with as the humans (and
engage counterarguments \emph{more}, 1.96--2.17 against the human 1.39), while weak
...
\textbf{Respect-toward-groups has no raw-machine counterpart at any tier} ---
...
\textbf{Evidence is the dimension humans keep}: every machine arm sits below the
...
\textbf{Post-training creates form} within a fixed model (mistral instruct-minus-base
...
\subsection{Directed effort evades the detector}\label{sec:evasion}

\textbf{How far above the floor the truth sits is set by what directed effort can
hide.} Prevalence counts only the machine text a detector can see, so every
figure in \cref{sec:prevalence} --- the 9.0\% headline included --- is a floor. This arm measures
...
were biased samplings and are reported in Appendix~\ref{app:superseded}; the figures here are
the two final searches only.\footnote{\texttt{python\ bypass\_\allowbreak{}report.\allowbreak{}py} reads the verdict files and prints every
...
AI-score, and a variant reaches the detector only to \emph{record whether the
attack worked}, never to steer it. There is no fine-tuning, no gradient, and
no evasion product. The detector\textquotesingle s one contribution is confirming that
...
rather than from querying the target. \Cref{fig:bypass} lays out the loop.
...
labels, and the search can move a speech between them: \textbf{AI}, the flagged
starting state; \textbf{Mixed}, "not confidently AI"; and \textbf{Human}, "a person wrote
this". "Success" throughout means \textbf{reaching a clean Human verdict} --- the
...
the same direction (Appendix~\ref{app:cut} item 7).

\textbf{The headline: a general-purpose model never shown the target detector beats
purpose-built humanizers, and the gap widens if the attacker can retry.} On
one blind submission, \textbf{11.1\%} of this search\textquotesingle s variants reach Human, against
\textbf{2.31\%} of documents for the 13 commercial "humanizer" services in Pangram\textquotesingle s
own technical report --- products built and tuned \emph{against} detectors --- so the
blind search runs \textbf{\(4.8\times\)} the purpose-built products. With query-and-retry
(\(\leq\)18 attempts), \textbf{22.5\%} of targets go past the detector: \textbf{\(9.8\times\)} the
humanizers and \(66\times\) Pangram\textquotesingle s clean-conditions false-negative rate of 0.34\%.
Which rate applies depends on one thing --- whether the attacker can submit to
the detector and retry: the per-variant rate (\textbf{11.1\%} {[}5.5, 17.6{]}) prices a
fire-and-forget attacker who cannot check their work; the per-target rate
(\textbf{22.5\%} {[}14.4, 33.5{]}) prices one who submits, sees the verdict, and tries
again. Both are realistic; they bracket the exposure. The gap between them is
about \textbf{\(2\times\)}. Both rates are tabled with their vendor comparators in §4.10a.

\subsubsection{Evasion accounting and robustness}\label{sec:evasion-accounting}

The result does not arise from a weak detector. Pangram produced 0 false positives in 1,260 pre-2022 controls (\cref{sec:calibration}), so the reversals are movements \emph{into} a class the detector otherwise reaches zero times in 1,260 --- not boundary noise, but a hard specificity floor that a directed search nevertheless walks text across. Those multiples describe the attacks actually run, not the attack class, and the experiment does not estimate how far either attack improves from here.
...
\begingroup\small
\begin{longtable}[]{@{}p{0.24\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.07\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.13\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.12\linewidth}@{\hspace{5pt}}p{0.31\linewidth}@{}}
\caption{Measured rates by attacker model: the vendor-reported one-submission false-negative rates, this study's per-variant rate, and its retry-enabled per-target rate.}\label{tab:bypass-summary}\\
\toprule\noalign{}
...
Pangram 4, Epoch AI Style Imitation & 2.86\% & not reported & 1 & document-level adversarial benchmark, vendor\footnote{2.86\% (17/594) is Pangram 4\textquotesingle s
...
\textbf{design effect of 3.40} puts the effective sample at about 100 variants
rather than 341. That correction says only that repeated rewrites of one text
add little new information; it rests on no hypothesis about \emph{which} texts are
evadable, a question that stays exploratory (Appendix~\ref{app:superseded}). The per-target
...
\begingroup\small
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.175\linewidth}@{\hspace{6pt}}r@{\hspace{6pt}}r@{\hspace{6pt}}r@{\hspace{3pt}}>{\raggedright\arraybackslash}p{0.115\linewidth}@{\hspace{6pt}}>{\raggedright\arraybackslash}p{0.245\linewidth}@{\hspace{3pt}}>{\raggedright\arraybackslash}p{0.115\linewidth}@{}}
\caption{Detector-evasion search yield per target.}\label{tab:bypass-yield}\\
\toprule\noalign{}
run & searched & zero-yield & \(\geq\)1 Human & Human rate & \(\geq\)1 at \texttt{fraction\_ai\ =\ 0.0} & strict-zero rate \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
run & searched & zero-yield & \(\geq\)1 Human & Human rate & \(\geq\)1 at \texttt{fraction\_ai\ =\ 0.0} & strict-zero rate \\
...
\textbf{pooled} & \textbf{71} & \textbf{6} & \textbf{16} & \textbf{22.5\%} {[}14.4, 33.5{]} & \textbf{15} & \textbf{21.1\%} {[}13.2, 32.0{]} \\
\end{longtable}
\endgroup
...
in-sample; the clean out-of-sample point is GO all-31\textquotesingle s \textbf{19.4\%} (n = 31), a
...
interval of roughly {[}\(-17\), +25{]} points, equally compatible with one chamber\textquotesingle s
rate being half or double the other\textquotesingle s, so the honest single replication
expectation is "\(\approx\)19--22\%, wide interval".

More than a restatement of that interval: the two chambers\textquotesingle{} \emph{humanising
...
outcomes.\footnote{\texttt{BYPASS\_METHODOLOGY.md}.}
...
\textbf{Pangram-evading variants also pass the available alternative detectors,
...
\textbf{Seventy-nine percent of the Pangram-evading variants evade all four
...
(log-rank), and 6\% (Binoculars), reproducing the collapse \cref{sec:related} documents for
this register. The test therefore does not demonstrate a strong evader
defeating strong alternative detectors. It shows that Pangram\textquotesingle s reversals are
not rescued by any detector available here: variants that pass Pangram
generally pass the weaker alternatives as well.\footnote{\texttt{python\ score\_\allowbreak{}multistat.\allowbreak{}py} (Fast-DetectGPT, Binoculars and LRR
...
\textbf{One firm negative stands where the exploratory questions do not.} Whether
...
weakly and neither survives a test, and Appendix~\ref{app:superseded} carries the numbers and the
...
\textbf{What this means for the prevalence number.} Evasion is real and clears the
detector on roughly one flagged speech in five under directed search, so 9.0\%
is a floor on \emph{use}: it measures undisguised machine drafting, and disguised
...
opposite --- instructed rewriting \emph{raises} Pangram\textquotesingle s score, and human text
...
\subsubsection{Evasion rewriting does not measurably change deliberative quality}\label{sec:evasion-quality}

\textbf{Two paired arms price the evasion edit.} 63 within-text pairs, blinded and
...
\textbf{GO Opus-selected run} --- not the GO all-31 arm of the bypass section above,
with which it shares no segments. What these arms measure is the effect of
rewriting \emph{under an evasion instruction}, not of successful evasion: selection
was by the Opus proxy, not the Pangram outcome, so the graded set is not
conditioned on failure --- and in the event \textbf{61 of the 63 graded rewrites did
...
\textbf{Every dimension is null on both.}\footnote{\texttt{cd\ quality\_expansion\ \&\&\ python\ analyze\_stage3.py\ {[}RUNDIR{]}} and
  \texttt{python\ analyze\_stage3.py\ -\/-key4\ {[}RUNDIR{]}}; values cached in
  \texttt{results\_stage34.json}, which now also caches \texttt{r\_words} per dimension (reported in Appendix~\ref{app:cut}).
  ...
  stage 3; 18 and 16 in stage 4).} Within-text pairing holds content,
speaker and occasion fixed by construction, so these are the raw paired
differences --- the design\textquotesingle s own estimand, needing no covariate. What the pairs
can exclude is bounded and stated: detectable effects run roughly \(\pm\)0.12--0.26
per dimension for stage 3 --- commensurate with the study\textquotesingle s own headline effects
...
shows TOST equivalence within \(\pm\)0.22 on five of seven dimensions (the two
...
MDE statement says.\footnote{\texttt{python\ tost\_\allowbreak{}equivalence.\allowbreak{}py} --- SESOI fixed at \(\pm\)0.22 raw points,
  the low end of the stage-2 headline range, chosen once and applied to
  every equivalence statement in the paper; two one-sided tests on the
  committed estimators.} None of the 28 paired cells reaches nominal
...
dimension by dimension, are tabled in Appendix~\ref{app:cut} item 15.
...
5).} The DQI judge\textquotesingle s own \texttt{ai\_guess} adds a note worth its own sentence. It rated the
...
a quality difference in it.\footnote{\texttt{stage5\_scores.json}, \texttt{PREREG-\allowbreak{}stage5-\allowbreak{}successful-\allowbreak{}evasion.\allowbreak{}md}.}

The paper\textquotesingle s quality conclusion does not rest on stages 3 and 4 in any case. It
...
frontier models \emph{beat} the human twins on form) --- four different contrasts
agreeing.

The quality clause is a separate claim from the prevalence floor of \cref{sec:evasion},
...
baseline rather than the field\textquotesingle s post-2020 windows. Where the estimates meet
they agree: the 9.0\% floor falls within the 6.5--17.5\% range Liang et al.\cite{liang2024monitoring,liang2025quantifying} and Gray\cite{gray2025} report for professional prose.
...
threshold that does not match his headline rate (Appendix~\ref{app:cut} item 12), but that
...
on this genre is close to uninformative, and Rice\textquotesingle s own numbers (Binoculars
...
\item
  \textbf{The evasion rates are what two days of directed effort bought, not the
  adversary\textquotesingle s ceiling.} Four runs over two days with a frontier model. A staff
...
\item
  \textbf{Specificity is measured pre-2022, on the genres the controls happen to
  contain.} The floor claim needs \(\mathrm{Sp} = 1\) on contemporary human speech;
  1,260/1,260 is strong, but it is qualified in two ways. Temporally: the
  controls end at 2022, and \cref{sec:register-shift}/\cref{sec:permeation} say the human baseline is moving toward
  the detected register; \cref{sec:calibration}\textquotesingle s audit of every hit bounds the worst case at
  ...
  five Oral Questions flags at maximal confidence (\cref{sec:genre}); until a pre-AI
  tribute control exists (Appendix~\ref{app:future} item 12), the specificity claim does not
  extend to that register.
\item
  \textbf{Mixed verdicts are fractional, and not every analysis can use the fraction.} Prevalence weights each Mixed segment by its reported AI fraction. Analyses that require a binary label instead pool Mixed with AI, so they rest on a coarser label than the prevalence figures do; the fractions themselves are reported separately in the CSV.
\item
  \textbf{Tasmania is uninterpretable} and excluded. The transcription-regime
...
\item
  \textbf{The permeation effect is small}, and it rests on one word list: its two
...
\item
  \textbf{The juniority gradient\textquotesingle s components are separated; its mechanism is
  ...
  fourth operationalisation of the same kind (Appendix~\ref{app:future} items 9--11).
\item
...
\subsection{Calibration of small effects}\label{sub:calibration-small}
...
of d \(\approx\) 0.2. Those sit at or below the typical published effect in
individual-differences research: Gignac \& Szodorai\textquotesingle s pool of 708
meta-analytically derived correlations puts the 25th/50th/75th percentiles
at r = .10/.20/.30 (\emph{Personality and Individual Differences} 102, 2016).
Funder \& Ozer\textquotesingle s guidelines read the same numbers forward: r = .05 is "very
small" for single events "but potentially consequential in the not-very
long run," r = .10 "small\ldots{} but potentially more ultimately consequential,"
r = .20 "medium" (\emph{Advances in Methods and Practices in Psychological
Science} 2, 2019).
...
correlation toward zero. And the study\textquotesingle s claim is structural, not
...
One benchmark cuts against us and is kept: Funder \& Ozer flag r \(\geq\) .40 as "likely to be a gross
...
obvious reading of \cref{sec:evasion} is that one detector was beaten. The more useful
...
The 9.0\% in \cref{sec:prevalence} exists precisely because nobody currently bothers to evade:
...
above entirely: \textbf{detection is a norms instrument, not a security
...
launched on or after \textbf{2 August 2026}, and is retrofitting existing models.
Detection is not yet public --- the company says it is
"working to enable users and other third parties to detect Claude\textquotesingle s embedded
watermarks", with technical documentation forthcoming. Google DeepMind\textquotesingle s
SynthID-Text (Dathathri et al., \emph{Nature} 2024) is the deployed precedent.
...
which a transparency measure requires. Anthropic\textquotesingle s own documentation concedes
...
worry (\cref{sec:intro}) actually calls for. Because register and prevalence can now be
...
dynamics \cref{sec:chase-flight} documents, which are expected to persist.
...
\textbf{Unclustered inference produced a hypothesis-flattering result more than once
in this study, and each instance is recorded as cautionary.} Item 3 below is
one; \cref{sec:class}\textquotesingle s provincial estimates were unclustered (items 16 and 17 below;
Appendix~\ref{app:superseded} item 3); and so is the cohort t-statistic itself (\cref{sec:generational}): member-year
HC1 errors report t \(\approx\) 17.7, clustering on member gives t \(\approx\) 8--9. That last one
...
\textbf{mechanism is not exposure as we can measure it.} Items 8, 9 and 11 reject
the tested composition explanations at the \emph{chamber} level:
...
\item
  \textbf{\cref{sec:posttraining}\textquotesingle s stage ordering} --- The claim that "the shift is largest at the
  ...
  routes agree:
  SFT \(\approx\) DPO, RLVR \(\approx\) 0. The association with \cref{sec:register-shift} remains, but no inference
...
\item
  \textbf{New Brunswick on Pangram 3} --- superseded by the Appendix~\ref{app:cut} item 19
  rescore. Kept
...
\item
  ...
  the total calendar drift (\cref{sec:careers}). The null was an underpowered window
...
\item
  \textbf{The three-profile linear rank, retired with its numbers.} The fourth
  retirement behind \cref{sec:occupation}: the three-profile linear rank (free / front-line /
...
\caption{Superseded per-seed bypass transitions.}\label{tab:superseded-bypass}\\
  \toprule\noalign{}
  seed verdict & variants & \(\rightarrow\) AI & \(\rightarrow\) Mixed & \(\rightarrow\) Human & Human rate {[}95\% CI{]} \\
  \midrule\noalign{}
\endfirsthead
  \toprule\noalign{}
  seed verdict & variants & \(\rightarrow\) AI & \(\rightarrow\) Mixed & \(\rightarrow\) Human & Human rate {[}95\% CI{]} \\
...
Grouped by what they would settle. Five groups stay inside this study\textquotesingle s own
...
\item
  \textbf{A register-feature instrument, and the test that would settle §4.8b.}
...
\item
  \textbf{Vendor attribution for the family signature.} \cref{sec:model-lineage}\textquotesingle s vector result ---
...
\item
  \textbf{Measuring the human contribution against an automated counterpart.}
  ...
  \textbf{The baseline must move.} A fixed model vintage would repeat the current
  debate\textquotesingle s error: work relative to dumb computers counts as obviously
...
\item
  ...
  objective \emph{is} a model of the human reviewer --- rewrite to minimise a
  ...
  \emph{measurement} of an adversary\textquotesingle s reachable frontier, not as a deployable
  ...
  privately.
\item
  \textbf{The detection game --- items 5 and 6 as one self-generating instrument.}
  ...
  lineage-correlated judge behind \cref{sec:gut-judgment}). The game frame is what makes it
...
\item
  ...
  decomposed and would bring the US into \cref{sec:genre}\textquotesingle s genre ladder, which it
...
\item
  ...
  metadata is missing or carries topic titles instead (Appendix~\ref{sec:panel-limits}). A
  ...
  also supply the control the quality arm most wants (\cref{sec:quality}), and the one the
...
\item
  \textbf{The professionalised-communications rival} --- message discipline and
...
\tightlist
\item
  \textbf{Does the counterfactual residual behave?} The cheapest first test of item
  4\textquotesingle s residual, on data already in hand: take the \cref{sec:evasion-quality} pairs, generate the
...
\textbf{F. Follow-ons to the register and occupational arms.} Directions that
...
\item
  ...
  about any one detector\textquotesingle s opinion of it.
\item
...
\item
  ...
  \textbf{Why it matters beyond linguistics.} This is the prototype question for
...
\textbf{G. Court transcripts, and the expert-witness problem.}
...
\textbf{H. What the legislator panel enables beyond the register.}
...
comparators, and everything is reproducible from \texttt{analysis/s10/}.

\textbf{Verification status.} Everything unmarked was computed or re-verified on
...
the footnote on each gives the script and invocation. \cref{sec:chase-flight}\textquotesingle s literature
citations were web-verified on 2026-08-24: the result is placed against Labov
(crossover/hypercorrection, \emph{Sociolinguistic Patterns}, U. Pennsylvania Press,
1972), Simmel (1904), Veblen (1899), Jhering (1883) with Durkheim\textquotesingle s summary
supplying the quote, and Lieberson (\emph{A Matter of Taste}, Yale, 2000), and a
fetched entry wrong on Labov\textquotesingle s year and publisher was corrected.

\textbf{\cref{sec:class}\textquotesingle s standard errors are not clustered on member} --- an omission that has
now bitten the study three times; it is flagged in place.

\subsection{What this was run on}\label{sec:run-on}
...
\bottomrule\noalign{}
\endlastfoot
Corpus-wide screen (Appendix~\ref{sec:screen-pangram}), effort A/B (Appendix~\ref{sec:screen-effort}) & Claude Opus & low, and max in the A/B \\
Bypass search: scoring / rewriting / hypotheses (\cref{sec:evasion}) & Claude Opus & low / medium / high \\
DQI grading, stages 1-5 (\cref{sec:quality}, \cref{sec:evasion-quality}) & \textbf{Claude Fable 5} & default \\
...
The screen/grader split is load-bearing rather than incidental. \cref{sec:limits} reports
...
is real --- but it holds between \textbf{two different models}, which makes it a fact
...
segments), separates Pangram\textquotesingle s AI and human classes at \textbf{AUC 0.954} {[}0.934,
0.971{]} on the 618-segment overlap.\footnote{\texttt{python\ opus\_\allowbreak{}screen\_\allowbreak{}auc.\allowbreak{}py}. The deployed screen is
  \texttt{opus\_screen\_full.js} (473 batches of 40), distinct from the lean
  validation run used in the effort A/B in Appendix~\ref{sec:screen-effort}; the two differ in
...
pre-2023 text. Those observations are \emph{provably} pre-LLM and therefore clean
...
to \textbf{0.940} {[}0.915, 0.961{]}. Contemporary human speech is the harder and more
operationally relevant class to separate, as the permeation finding (\cref{sec:permeation})
predicts; the restricted estimate therefore gives the more demanding
assessment of the screen.

\subsection{Reasoning effort in the Opus screen}\label{sec:screen-effort}

\textbf{Reasoning effort buys nothing here.} The screen was tuned and validated at
\texttt{effort=low}, and low had never been compared against anything. Re-run on the
...
moved Qwen3-32B by \textbf{+0.172} and gpt-oss-120b by \textbf{+0.015}, against
\textbf{\(-0.007\)} for Opus. Reasoning closes part of the gap for weak detectors and
...
run for about \textbf{\(4\times\) fewer reasoning tokens (\textasciitilde\(2\times\) all-in)} with no loss, a
saving seen in the run logs rather than the committed AUC artifacts.\footnote{\texttt{opus\_effort\_ab.py}, \texttt{opus\_effort\_ab.csv}.}
...
The seed column counts targets \textbf{searched}, not targets that yielded a
...
about which texts evade (an exploratory question --- Appendix~\ref{app:superseded}).
...
\texttt{nb\_p3\_vs\_p4.py} & Appendix~\ref{app:cut} item 19 model-tier comparison \\
...
\texttt{cohort\_vs\_period.py}, \texttt{formation\_window.py} & \cref{sec:generational} and \cref{sec:careers} (birth vs period) \\
...
\texttt{build\_\allowbreak{}member\_\allowbreak{}vectors.\allowbreak{}py}, \texttt{vector\_analysis.py} & \cref{sec:class} and \cref{sec:four-predictors} word-mix geometry and \cref{sec:model-lineage} model lineage; VECTOR-ANALYSIS.md is the full log \\
...
\texttt{CLASS-\allowbreak{}REGISTER-\allowbreak{}LITERATURE.\allowbreak{}md} & \cref{sec:chase-flight} citation and verification ledger, with suspect entries flagged \\
...
\texttt{quality\_expansion/} & \cref{sec:quality} and \cref{sec:evasion-quality}, self-contained (\texttt{RUNME.md}) \\
...
\subsection{The classification\textquotesingle s element signatures}\label{sub:element-signatures}
...
\subsubsection{The folk ladder\textquotesingle s consensus signature}\label{sub:folk-signature}

Every O*NET element, its level, consensus sign, and how many of the eighteen
blind coders nominated it (\cref{sec:occ-classification}):
...
\subsubsection{The theoretical ladder\textquotesingle s component assignments}\label{sub:theoretical-components}

From three blind workflows, consensus elements only; the \emph{rev.} column marks
the elements that are reverse-scored (the discretion items within U upward,
\cref{sec:occ-classification}):
...
\subsection{The constant-window register trend, chamber by chamber}\label{sec:climb-table}

The endpoints behind \cref{sec:climb-geography}\textquotesingle s convergence result, chamber by chamber: gap
...
\subsection{The provincial class estimates (the discovery record)}\label{sub:class-provincial}
...
adjudicated\footnote{\texttt{provinces/\allowbreak{}OCCUPATION\_\allowbreak{}CODING.\allowbreak{}md} records the workflow and every
  rubric ambiguity.} --- gives 897 members and 5,294 member-years, 57.5\% of
...
\textbf{Class III\textquotesingle s premium is composition, not class.} Routine non-manual members
are the chamber\textquotesingle s young class: their median birth year is 1974 against
...
two rows should not be quoted as significant. The provincial table\textquotesingle s result
rests on II and IVab, at 435 and 180 members. The panel table in \cref{sec:class} is a
different estimand with far larger cells: III 147, VIIab 109, II 2,049.

\subsection{Education: the provincial ladder, which did not replicate}\label{sub:education-provincial}

At panel scale the education ladder does not replicate (Appendix~\ref{app:null} item 16):
...
secondary, college, bachelor, graduate --- register rises \textbf{+0.366 per rung}
(t = 3.30), and \textbf{+0.315} with birth cohort controlled (t = 2.96). Treating a
professional degree as a rung above a master\textquotesingle s zeroes the linear term entirely,
because law and medicine sit \textbf{\(-0.785\) below bachelor\textquotesingle s} (t = \(-3.49\)). A
professional degree outranks a bachelor\textquotesingle s on any ordering, so that dip cannot
...
review and closed here. Once run it proved a \textbf{bad control} for reasons given
...
then with the grading judge\textquotesingle s own \texttt{ai\_guess} added (\texttt{analyze.py} §3b):
...
\textbf{What it shows.} On the stage-2 external label, the two largest coefficients ---
...
(\cref{sec:limits}); the external-label result is robust to the one control the
...
\subsection{Prominence: no usable pooled slope, and the buckets that replace it}\label{sec:prominence-full}

The \cref{sec:prominence} prominence effect was first estimated from the Canadian provinces at
...
\textbf{There is no usable pooled slope.} A single slope assumes one gradient where
there are two shapes, so the earlier \(-0.033\sigma\) per log-byte estimate across 13
...
of \(\pm\)0.026--0.027, so the flatness is measured rather than merely noisy.
...
which is why \cref{sec:prominence} reports prominence as buckets and quotes no pooled
...
inflation as the class arm and is superseded. †\footnote{\texttt{python\ plot\_\allowbreak{}class\_\allowbreak{}by\_\allowbreak{}era.\allowbreak{}py}-adjacent computation; per-bucket means,
...
no split is possible. The eight Canadian provinces print "Hon. " (in
...
the office flag attaches to the member-\emph{year} rather than to the member: a
backbencher who becomes a minister contributes to both groups
(\texttt{office\_split.py}, 8,289 member-year cells with a birth year: 1,893 office,
6,396 non-office).
...
\subsection{What the panel cannot hold fixed}\label{sec:panel-limits}

\textbf{Role is not controlled.} The \texttt{ROLE} pattern applied during extraction is
speaker-identity hygiene: it drops chair and presiding-officer strings that
cannot be joined to a person, and removes \textbf{0.47\% of words} (4.1M of 870.4M,
0.64\% of segments) --- the right order for a name-parsing filter, and no
substitute for the office split above.

\textbf{Genre cannot be controlled outside federal Canada.} The \texttt{section} field is
empty in 43 of the 53 chamber files, and where it is populated it carries topic
titles (\texttt{ONTARIO\ ECONOMY}, \texttt{PUBLIC\ BILLS\ FOR\ SECOND\ READING}) rather than order
of business. Federal Canada\textquotesingle s \texttt{order} field is the exception, which is why the
genre ladder in \cref{sec:genre} is measured there and nowhere else --- a data constraint,
not a choice of scope, and one an LLM genre classifier could lift (Appendix~\ref{app:future}).

\section{Cut for length}\label{app:cut}

Material sound enough to keep but not needed in the reader\textquotesingle s path.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{The stage-1 quality fits on the rubric\textquotesingle s 0--100 scale} (cut from \cref{sec:quality}\textquotesingle s
  table for readability). \cref{sec:quality}\textquotesingle s table originally carried a third stage-1
  column restating the per-sd fits as fitted 0\(\rightarrow\)100 differences --- the same fits
  and the same t statistics at a different scale. The stage-1 regressor has sd
  15.3 on the 0--100 scale and an observed maximum of 70, so the fitted 0\(\rightarrow\)100
  difference extrapolates beyond the data; the per-sd column is the fitted
  0\(\rightarrow\)100 difference times sd/100 and is the commensurable comparison with stage
  2. The fitted 0\(\rightarrow\)100 differences: justification +1.134, common\_good +0.583,
  respect\_groups +0.287, respect\_demands \(-0.179\), respect\_counterargs +0.143,
  constructive \(-0.015\), evidence +0.350.
\item
  \textbf{The ratio estimator\textquotesingle s finite-sample bias.} \cref{sec:prevalence}\textquotesingle s rates come from
  ...
  The script is \texttt{banded\_prevalence.py}, the same one behind \cref{sec:prevalence}\textquotesingle s banded
  table.
\item
  \textbf{The original run\textquotesingle s two group-pairs on respect\_demands}, cut from §4.9.
  The original run\textquotesingle s two group-pairs also disagreed in sign on
  respect\_demands --- candidate-AI minus candidate-human \textbf{\(-0.356\)} (Welch
  t \(-1.47\), n = 20 vs 32), uniform-sample AI minus human \textbf{+0.650}, resting on
  \textbf{5} applicable AI segments. Both are noise-sized samples, and two noisy
  ...
  \cref{sec:quality}\textquotesingle s genre control. Cut for length: the passage is self-described
  consistent texture, and no claim in \cref{sec:quality} or \cref{sec:discussion} rests on it.
\item
  \textbf{The length correlations in the paired evasion arms}, cut from §4.10b.
...
\item
  \textbf{The education case, and its two costs}, cut from §7.3. A different
  institution and a speculative extension; \cref{sec:disc-substitution} ends on the artifact
  sentence, and both costs travel with the passage.

  Deliberation is judged on the artifact; education is not: there the text
  is a proxy for what is in someone\textquotesingle s head, and a proxy that can be generated
  is no proxy at all.
  That case needs a \textbf{more rigorous proof of understanding}, and machine
  intelligence supplies the means as well as the problem --- adaptive
  examination against a person\textquotesingle s \emph{entire} corpus of work, with provenance and
  ...
  student\textquotesingle s entire corpus" is a surveillance instrument before it is an
  assessment instrument; the version worth building is the one that is legible
  to the person being assessed.
\item
  \textbf{The introduction\textquotesingle s prospectus paragraph} (cut from \cref{sec:intro}). A statement of
  which research fields the pattern of findings opens. It carries no
  measurement, and it left the reader\textquotesingle s path because \cref{sec:intro} already previews the
  findings over five consecutive paragraphs while Appendix~\ref{app:future} names the
  ...
  \end{quote}
\item
  \textbf{The flip rates behind \cref{sec:evasion}\textquotesingle s success rates} (cut from \cref{sec:evasion}). The
  ...
  merely leaving AI, the \emph{flip}, was measured too and moves with effort in
  ...
  \begin{quote}
  A \emph{flip} (leaving AI for Mixed-or-Human) is far easier than a \emph{success}
  ...
  the New Brunswick protocol change: the blind first attempt flipped \textbf{10\%}
  of variants (4/40) and reached Human on \textbf{2.5\%} (1/40); the contrastive
  search flipped \textbf{28\%} (59/212) and reached Human on \textbf{13.7\%} (29/212).
  Effort raised both bars, and by \emph{more} on the success bar (\(5.5\times\)) than on
  the flip bar (\(2.8\times\)). And at the target level, if merely "soften off AI" is
  the goal, 17 of 38 New Brunswick targets (\textbf{45\%}) cleared it. So the
  ...
  \end{quote}
\item
  \textbf{The judge-leakage caveat\textquotesingle s fourth statement} (cut from \cref{sec:quality}). The
  correlation between the screen\textquotesingle s and the grading judge\textquotesingle s independent AI
  guesses is disclosed in \cref{sec:limits}\textquotesingle s Limits, tested in Appendix~\ref{sec:judge-leakage}, and read in
  Appendix~\ref{sec:run-on} as a fact about two different models rather than one model
  agreeing with itself. \cref{sec:quality} stated it a fourth time; that statement, with
  ...
  a caveat on reading either judge\textquotesingle s \texttt{ai\_guess} as ground truth, not on the
  DQI scores. Its bearing on the quality claim is tested in Appendix~\ref{app:robustness} and
  does not overturn the external-label result; the genre/era fixed effects in \cref{tab:dqi} control confounds between cells, which is a separate
  matter from leakage within a text.
  \end{quote}
\item
  \textbf{What the weighting choice is worth in \cref{sec:prevalence}} (cut from \cref{sec:prevalence} for length).
  The pooled prevalence rate on the same segments, under each weighting:
  \textbf{words} (used throughout) \textbf{9.03\%}; segments, binary 8.98\%, \(-0.05\) points
  against the word rate; segments, fraction-weighted mean 6.83\%, \(-2.20\) points
  against the word rate. It left the reader\textquotesingle s path because \cref{sec:prevalence} already gives
...
\item
  \textbf{The tenure view} (from \cref{sec:generational}). A sound view that reproduces quantities
  the section reports elsewhere; it left the reader\textquotesingle s path because \cref{sec:generational}
  already runs a family of views and this one adds a warning rather than an
  estimate --- the entry-cohort term it recovers is reported in \cref{sec:generational}\textquotesingle s
  ...
  instead --- rate minus its chamber \(\times\) year mean --- it falls, monotonically,
...
\setcounter{enumi}{10}
\item
  \textbf{The compression trend inside the class-era panels}, cut from \cref{sec:class} for
  length: \cref{sec:chase-flight}\textquotesingle s closing statement already reports the same gap trend, and
  no claim in \cref{sec:class}--\cref{sec:chase-flight} turns on the member-level detail. Two slow
  ...
  member-level view\textquotesingle s and is held lightly. The peak itself never moves ---
  under either aggregation.
\item
  \textbf{Rice 2026\textquotesingle s own specificity calibration}, cut from §5. Verified and
  sound, but \cref{sec:related}\textquotesingle s answer to Rice is explicitly about sensitivity rather than
  specificity, and this study\textquotesingle s own specificity figure is already reported in
  \cref{sec:related}\textquotesingle s opening comparison and in \cref{sec:calibration}; the paragraph argued a point the
  section then sets aside.

  Rice\textquotesingle s own calibration is thin on specificity: \textbf{4 of 50} pre-ChatGPT
  speeches flagged, Wilson CI {[}3.2\%, 18.8\%{]}, at the "highly likely AI"
  threshold (\(\geq\)8/10), and it characterises a Haiku-class judge rather than
  the Sonnet-class judge behind his headline run. The 8\% FPR and the 7.1\%
  corpus rate at "possibly AI" (\(\geq\)6/10) are measured at different thresholds
  and are not directly comparable. Our corresponding specificity is 0 in
  1,260, measured chamber by chamber.
\item
  \textbf{The control-surface note on the word \emph{policing}} (Matthew,
  2026-08-19), cut from Appendix~\ref{app:future} item 31. A commentary on one word of the transmission item that bears on
  ...
  policing already exists --- \textbf{control over the LLM or otherwise computerized
  outputs the individual consumes}. Person-targeted influence need not look
  like discipline; it can be a quiet reweighting of each person\textquotesingle s machine
...
\item
  \textbf{The distribution and image-provenance detail on Anthropic\textquotesingle s text
  watermark}, cut from §7.2. Accurate vendor deployment detail; it left the
  reader\textquotesingle s path because \cref{sec:disc-norms}\textquotesingle s argument turns on the mark\textquotesingle s existence,
  hiddenness and robustness limits rather than on which clouds carry it, and
  this study measures text only.

  \begin{quote}
  The mark ships across all the company\textquotesingle s products and its AWS, Google
  Cloud and Microsoft distribution. Generated image files additionally
  carry C2PA-signed provenance metadata.
  \end{quote}
\item
  \textbf{The stage 3/4/5 paired-difference table, dimension by dimension} (cut
  from \cref{sec:evasion-quality}). The cell-level nulls behind \cref{sec:evasion-quality}\textquotesingle s summary that
  evasion-directed rewriting (stages 3/4) and evasion-achieving rewriting
  (stage 5) move no deliberative-quality dimension; it left the reader\textquotesingle s path
  because nothing in \cref{sec:quality}, \cref{sec:evasion-quality} or \cref{sec:discussion} turns on any individual cell --- the
  claim is the nullness, which \cref{sec:evasion-quality} states in prose. Raw paired differences
  (humanized \(-\) original, within text), t in parentheses; \texttt{-1} (inapplicable)
  pairs excluded, which is why the two sentinel dimensions have smaller n (12
  and 12 in stage 3; 18 and 16 in stage 4):

  \begingroup\small
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.16\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.16\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.17\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.17\linewidth}@{}}
\caption{Paired within-text quality comparisons, all four graded contrasts: evasion-directed rewriting (stages 3 and 4) and successful evasions against their targets (stage 5, at variant and target level), by dimension.}\label{tab:quality-paired}\\
  \toprule\noalign{}
...
\endhead
  ...
  common\_good & \(-0.05\) (t \(-0.8\)) & 0.00 (t 0.0) & +0.01 (t +0.3) & +0.03 (t +0.4) \\
  respect\_groups & \(-0.08\) (t \(-1.8\)) & +0.04 (t +1.0) & +0.04 (t +1.0) & +0.10 (t +1.2) \\
  respect\_demands & \(-0.08\) (t \(-0.6\)) & 0.00 (t 0.0) & \(-0.01\) (t \(-0.6\)) & 0.00 (t 0.0) \\
  respect\_counterargs & +0.33 (t +1.8) & +0.12 (t +1.0) & +0.11 (t +0.9) & +0.10 (t +0.4) \\
  constructive & 0.00 (t 0.0) & \(-0.12\) (t \(-1.8\)) & +0.03 (t +1.0) & +0.07 (t +1.0) \\
  evidence & +0.03 (t +0.4) & +0.08 (t +0.7) & \(-0.01\) (t \(-0.2\)) & +0.04 (t +0.3) \\
  \end{longtable}
\endgroup
\item
  \textbf{The registered horse race and the component pattern} (cut for length
  from \cref{sec:occupation}, where it was reported as a secondary registered comparison;
  the decisive folk-over-theoretical comparison is carried by \cref{sec:four-predictors}\textquotesingle s joint
  ...
  shuffles --- suggestive, unconfirmed. Under the covariate lattice, L\textquotesingle s raw
...
\item
  \textbf{Coefficients against variance shares in the joint model} (cut from
  \cref{sec:four-predictors} for length). A reading note rather than a result: the joint table (\cref{tab:four-predictors}) already carries each term\textquotesingle s n and its years-per-category column, and the
  explanatory-power accounting that follows the table carries the shares, so
  the paragraph reconciled two columns the reader can already see. A term\textquotesingle s
  ...
  the panel, so the term carries \textasciitilde0.25\% of variance alone. Class II\textquotesingle s far
  smaller +0.122, spread over 42\% of members, carries more (\textasciitilde0.36\%), and
  cohort\textquotesingle s dominance comes from a modest slope applied to everyone across a
  two-decade spread. Coefficients answer "how much for whom"; variance
  shares answer "how much of the panel".
\item
  \textbf{The introduction\textquotesingle s classification by-product paragraph} (cut from \cref{sec:intro}).
  ...
  from a categorical schema --- which \cref{sec:occupation} establishes in full and \cref{sec:market-measure} argues
  at length. It carries no measurement of its own, and it left the reader\textquotesingle s
  path because \cref{sec:intro} already previews the study\textquotesingle s findings over five consecutive
  paragraphs.
  ...
  \end{quote}
\item
  \textbf{The New Brunswick Pangram 3 vs Pangram 4 rescore} (cut from \cref{sec:calibration}). The
  rescore that established what the 2026-07 New Brunswick run\textquotesingle s silent
  Pangram 3 scoring did and did not change; it left the reader\textquotesingle s path because
  \cref{sec:detector} carries the same findings in prose and Appendix~\ref{app:superseded} item 6 keeps the
...
\endgroup
  ...
  Specificity is \textbf{identical}. The model-tier defect was an \textbf{undercount,
  ...
  conservative.\footnote{\texttt{nb\_p3\_vs\_p4.py}.}
\item
  \textbf{The full-population z-scoring variant of the class II-over-I contrast}
  (cut from \cref{sec:class}). A secondary specification beside the within-chamber
  ...
  \end{quote}
\item
  \textbf{The per-chamber prevalence table} (cut from \cref{sec:prevalence}). The twenty rates the
  dot plot in \cref{sec:prevalence} draws, given exactly: machine-drafted share of 2025--26
  words, word-weighted and weighted by each flagged segment\textquotesingle s AI fraction,
  with cluster-bootstrap 95\% intervals, sorted by rate; bold marks the two US
  chambers, whose contrast recurs in \cref{sec:climb-geography} and Appendix~\ref{app:future}. It left the
  reader\textquotesingle s path because the figure carries the finding --- the sorted spread
...
\endhead
  ...
  MB & 13.1\% & {[}7.9, 18.8{]} \\
  NI & 13.1\% & {[}8.0, 18.6{]} \\
  \textbf{US House} & \textbf{12.1\%} & {[}7.0, 17.7{]} \\
  ON & 11.1\% & {[}6.1, 16.8{]} \\
  NS & 10.0\% & {[}5.5, 15.1{]} \\
  AB & 7.8\% & {[}3.9, 12.3{]} \\
  IE & 7.5\% & {[}3.5, 12.3{]} \\
  ...
  NL & 3.3\% & {[}0.8, 6.6{]} \\
  UK & 2.5\% & {[}0.7, 4.8{]} \\
  \textbf{US Senate} & \textbf{1.8\%} & {[}0.1, 4.1{]} \\
  \end{longtable}
\endgroup
\item
  \textbf{Manitoba: the extraction defect and its repair} (cut from \cref{sec:prevalence-audits}). The
  forensic detail behind \cref{sec:prevalence-audits}\textquotesingle s one-paragraph summary; it left the main text
  because the chamber\textquotesingle s corrected rate, not the mechanics of the bug, is what
  the reader needs.

  The original extraction lost 42\% of the Manitoba record. From mid-2018 the
  Manitoba Hansard export splits a speaker\textquotesingle s name across several bold runs
  ...
  the Speaker\textquotesingle s --- where it was discarded as chair voice. Chair share of page
  ...
  exactly where drafting concentrates (\cref{sec:genre}). The extractor was
  preferentially deleting the machine-drafted half of the chamber\textquotesingle s record.
  ...
  The superseded extraction is kept at \texttt{provinces/superseded/} so the pre-fix
  verdicts stay reproducible --- 135 of the old 278 seg\_ids do not exist in the
  corrected file, because re-extraction shifts turn indices. \texttt{python\ build\_\allowbreak{}mb\_\allowbreak{}redraw.\allowbreak{}py}, then \texttt{pangram\_\allowbreak{}mb\_\allowbreak{}redraw\_\allowbreak{}verdicts.\allowbreak{}csv}.
\item
  \textbf{The provincial class estimates under member clustering} (cut from
  \cref{sec:class}). The per-term t-drops behind \cref{sec:class}\textquotesingle s opening statement that
  clustering by member, then replicating, removed every individual certainty
  in the provincial panel; it left the reader\textquotesingle s path because that statement,
  Appendix~\ref{app:null} items 16 and 17, and Appendix~\ref{sub:class-provincial}\textquotesingle s own caveat all carry the
  conclusion already, and no claim in \cref{sec:class} rests on an individual t value.

  Clustering the standard errors by member --- the study\textquotesingle s third encounter with
...
\item
  \textbf{The occupational registration\textquotesingle s minute-by-minute record} (cut from
  \cref{sec:occupation}, which keeps the claim --- prediction preceding the first fit by 73
  ...
  \textbf{12:04}, session log, before any fit or instrument freeze: "I would
  actually expect the highest reading to come with middle managers with
  workers below them" (Matthew). \textbf{12:41}: the four-level structure directed
  ("three levels of corporate and a free"). \textbf{13:03}: the charged/uncharged
  horse race directed (result in Appendix~\ref{app:cut} item 16). \textbf{13:05}, commit
  \texttt{f10d502}: the folk ladder and the apex delta ("insulated command tracks
  more register than exposed command") committed to the prereg. \textbf{13:17},
  \texttt{ad303ad}: Stage 1 run and unblinded. \textbf{13:36}, \texttt{53a02ff}: amendment
  adding the theoretical ladder and the explicit middle+/free\(-\) sign
  declarations, self-labelled as post-unblinding. \textbf{17:21}, \texttt{ccba83d}: the
...
\item
  \textbf{The occurrence-bin distribution behind \cref{sec:coverage}\textquotesingle s fatter zero bin} (cut
  from \cref{sec:coverage}). The four corpora\textquotesingle s style words binned by how many times each
  occurs, at matched volume; \cref{sec:coverage} keeps the zero-bin counts, which are the
  half of the distribution its argument uses, and \cref{sec:coverage}\textquotesingle s own text identifies
  ...
  \begingroup\small
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.17\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.145\linewidth}@{\hspace{5pt}}rrrrrr@{}}
...
\endgroup
  ...
  Volumes and script as in \cref{sec:coverage}\textquotesingle s footnote: \texttt{style\_\allowbreak{}word\_\allowbreak{}frequency.\allowbreak{}py}, three
  families at 1,600 prompts, every corpus truncated to 1,187,489 words, base
  and instruct counted separately.
\item
  \textbf{The market-as-social-oracle generalisation} (cut from \cref{sec:market-measure}). The
  broadest form of \cref{sec:market-measure}\textquotesingle s argument --- that cheap blind bespoke measures weaken
  the standing defence of prices as the best available reading of social
  structure. It carries no measurement of its own beyond what \cref{sec:market-measure} already
  reports (the blind folk ladder absorbing the categorical schema in the joint
  model, \cref{sec:four-predictors}, and standing beside ISEI and wage quartiles, \cref{sec:occupation}), and it
  left the reader\textquotesingle s path because \cref{sec:market-measure} already lands its measurement point one
  ...
  \end{quote}
\item
  \textbf{The scope condition of the moving-baseline measure} (cut from Appendix~\ref{app:future}
  item 4). A generalization of Appendix~\ref{app:future} item 4\textquotesingle s proposed instrument beyond
  ...
  the reader\textquotesingle s path for length.
  ...
  sampled only sparsely. But the majority of \emph{average} human intellectual
