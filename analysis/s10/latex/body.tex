...
vocabulary in biomedical abstracts (Kobak et al.\cite{kobak2025}) and in scholarly writing at large (Gray\cite{gray2025}).

Closest to this study, Suvanto et al.\cite{suvanto2026} detect undisclosed LLM text in UK and
...
\textbf{100.00\% {[}99.7\%, 100.0\%{]}} (Wilson).
...
table in Appendix~\ref{app:cut} item 21 reports the raw estimates; the shrunken range is the honest spread
...
The loss was \textbf{not neutral}. A frame missing 42\% of text at random would
...
where drafting concentrates (\cref{sec:genre}). The extractor was preferentially deleting
the machine-drafted half of the chamber\textquotesingle s record.
...
\textbf{SO31 ÷ Oral Questions} & \textbf{\(3.29\times\)} & & \(4.40\times\) & \(3.03\times\) \\
\end{longtable}
\endgroup
...
Matched donors narrow the dated claim further. Against donors matched on frequency and dispersion, the UK series\textquotesingle{} interior minimum at 1994 survives. Against donors also matched on pre-2010 trend --- a control that by construction absorbs any onset inside its own window --- the register\textquotesingle s \emph{distinctive} excess over ordinary same-shaped vocabulary dates to roughly \textbf{2014--2018}, and the style words remain below their matched donors in level throughout the record until 2025. The drift is real, broad-based, composition-robust and instrument-robust: its dated 1994--96 turn belongs to the UK ruler, while its excess over matched controls is younger than the turn.
...
\begingroup\small
\begin{longtable}[]{@{}l@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.16\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.20\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.24\linewidth}@{}}
...
\emph{highest} in class I (11.3\% vs II\textquotesingle s 5.7\%, 27-segment cell, suggestive
only).\footnote{\texttt{vector\_analysis.py}; methodology and full results in
...
footnote.\footnote{The registration record, all times 2026-08-18 US-Pacific, from the
  ...
  \textbf{13:03}: the charged/uncharged horse race directed (result in
  Appendix~\ref{app:cut}). \textbf{13:05}, commit
...
the framing is retired --- Appendix~\ref{app:superseded} item 13 preserves its numbers, and the
...
conspicuous machine tells peak and then recede (\cref{sec:register-shift}). The class cycle itself
...
the discriminating study is class-coded speech from before 1996 (Appendix~\ref{app:future}).
...
The RLVR row\textquotesingle s corrected shift has a confidence interval straddling zero:
...
the data (Appendix~\ref{app:superseded}).
...
ladder carries the stage claim alone. \cref{sec:register-shift}\textquotesingle s reading --- that alignment
...
At matched volume, style words by the number of times each occurs --- cells count how many style words fall in each bin:\footnote{\texttt{python\ style\_\allowbreak{}word\_\allowbreak{}frequency.\allowbreak{}py}. Three families at 1,600 prompts;
...
Two pools were graded, on 2026-08-10, under two different labels. \textbf{Stage 1} is
...
Pangram it separates Pangram\textquotesingle s classes at AUC 0.954 {[}0.934, 0.971{]} (Appendix~\ref{sec:screen-pangram}), so its score reads as a machine-involvement signal where no Pangram
verdict exists. \textbf{Stage 2} is 682 segments across 22 chambers labelled by the
\textbf{Pangram verdict} itself, with chamber fixed effects. Different populations,
different label sources, same answer --- and one half of the original answer does
not survive the controls below. The stage numbering runs across the study\textquotesingle s one
grading campaign: stages 3--5 grade the evasion arms (\cref{sec:evasion-quality}) and stage 6 the
never-reviewed continuations (\cref{sec:stage6}).\footnote{Stage 1: \texttt{cd\ quality\_\allowbreak{}expansion\ \&\&\ python\ analyze.\allowbreak{}py}. Stage 2:
...
\emph{Stage 1 reports per sd of the blinded screen score (sd 15.3 on the 0--100 scale, observed maximum 70), the commensurable comparison with stage 2\textquotesingle s binary contrast; unscaled fitted differences are in Appendix~\ref{app:cut}. Six cells carry the claim --- justification, common\_good and respect\_groups in each stage. ✱ marks the five that hold q \textless{} 0.003 under Benjamini--Hochberg across the table\textquotesingle s fourteen cells; (✱) marks the sixth, which does not survive the correction and is disclosed in its footnote.}
...
\textbf{Bypass study.} Prevalence counts only the machine text a detector can
see, so every figure in \cref{sec:prevalence} --- the 9.0\% headline included --- is a floor, and
...
can retry.} This search\textquotesingle s commensurable one-submission rate is \textbf{11.1\%} of
variants; the 13 commercial "humanizer" services benchmarked in Pangram\textquotesingle s own
technical report --- products built and tuned \emph{against} detectors --- get \textbf{2.31\%}
of documents past it, so a blind general-purpose search runs \textbf{\(4.8\times\)} the
purpose-built products. With query-and-retry (\(\leq\)18 attempts) the search gets
\textbf{22.5\%} of targets past the detector: \textbf{\(9.8\times\)} the humanizers, and \(66\times\)
Pangram\textquotesingle s clean-conditions false-negative rate of 0.34\%. Which rate applies
depends on one thing --- whether the attacker can submit to the detector and
retry:

\begingroup\small
\begin{longtable}[]{@{}lp{0.19\linewidth}p{0.24\linewidth}p{0.30\linewidth}@{}}
...
\textbf{per target} & \textbf{query + retry} (\(\leq\)18 attempts) & \textbf{22.5\%} {[}14.4, 33.5{]} --- induced, \textbf{\(9.8\times\) the humanizers} & an attacker who submits, sees the verdict, and tries again \\
\end{longtable}
\endgroup
...
\begingroup\small
\begin{longtable}[]{@{}p{0.21\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.09\linewidth}@{\hspace{5pt}}>{\raggedleft\arraybackslash}p{0.13\linewidth}@{\hspace{5pt}}p{0.42\linewidth}@{}}
...
Pangram 4, Epoch AI Style Imitation & 2.86\% & 1 & document-level adversarial benchmark, vendor\footnote{2.86\% (17/594) is Pangram 4\textquotesingle s
  ...
  tabled here.} \\
\textbf{this study, per variant} & \textbf{11.1\%} & \textbf{1} & \textbf{induced FNR, one blind submission; no attacker detector access --- \(4.8\times\) the humanizers} \\
\textbf{this study, per target} & \textbf{22.5\%} & \textbf{\(\leq\)18} & \textbf{induced; attacker may query and retry --- \(9.8\times\) the humanizers} \\
\end{longtable}
\endgroup
...
The paired cells, dimension by dimension, are tabled in Appendix~\ref{app:cut} item 15.
...
find. \textbf{Detection is a norms instrument, not a security instrument} --- locks,
...
\item
  ...
  Until a pre-AI tribute control exists (Appendix~\ref{app:future} item 12), the specificity claim
...
match his headline rate (Appendix~\ref{app:cut} item 12). But that is not what carries his
null.
...
and Tasmania\textquotesingle s transcription regime shifted mid-corpus, leaving no usable
control (\cref{sec:hazards}). The nineteen-chamber constant-window register series of \cref{sec:climb-geography}
is a different set again: it adds Prince Edward Island, which is not in the
table, and omits New Brunswick, the discovery corpus of \cref{sec:register-shift}.
...
\item
  ...
  \textbf{Unclustered inference produced a hypothesis-flattering result more than
  once in this study, and each instance is recorded as cautionary.} Besides
  this item, \cref{sec:class}\textquotesingle s provincial estimates were unclustered (items 16 and 17
  below; Appendix~\ref{app:superseded} item 3), and so is the cohort t-statistic itself (\cref{sec:generational}):
  member-year HC1 errors report t \(\approx\) 17.7, clustering on member gives
  t \(\approx\) 8--9. That last one does not flip --- 888 member clusters, not 10
...
\item
  ...
  (Appendix~\ref{sec:panel-limits}). A classifier labelling segments by order of business
...
\tightlist
\item
  \textbf{Does the counterfactual residual behave?} Take the \cref{sec:evasion-quality} pairs, generate
...
separates Pangram\textquotesingle s AI and human classes at \textbf{AUC 0.954} {[}0.934, 0.971{]} on
the 618-segment overlap.\footnote{\texttt{python\ opus\_\allowbreak{}screen\_\allowbreak{}auc.\allowbreak{}py}. The deployed screen is
...
Class III\textquotesingle s premium is composition, not class. Controlling birth year alone
...
+2.00): III\textquotesingle s median birth year is 1974 against 1956--1960 for every other
class --- routine non-manual members are the chamber\textquotesingle s young class, by sixteen
...
members, carry that provincial table. The panel table in \cref{sec:class} is a different
