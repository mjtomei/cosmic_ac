...
The register, however, \textbf{predates the models by decades}. That history is invisible to the excess-vocabulary designs that found the jump, which measure acceleration against a local trend in corpora reaching back roughly a decade. Run as a level series, the register\textquotesingle s rise spans three decades and two instruments, dated to 1994--96 on the longest-reaching series. The climb is concentrated in a few common function words over a broad shallow widening.
...
\begingroup\small
\begin{longtable}[]{@{}lrr@{}}
\caption{What the weighting choice is worth: the pooled prevalence rate under three weightings.}\label{tab:weighting}\\
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{words} (used throughout) & \textbf{9.03\%} & --- \\
...
36.7\%, Government Orders 23.3\% and Oral Questions 8.3\%, a \(4.40\times\) ratio; word-
and fraction-weighted the ordering is unchanged and \textbf{the gap settles at
...
\begingroup\small
\begin{longtable}[]{@{}l@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.13\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.12\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.16\linewidth}@{\hspace{8pt}}>{\raggedleft\arraybackslash}p{0.20\linewidth}@{}}
...
\textbf{Two slow relative movements are visible inside the stable shape, and
...
the member-level view\textquotesingle s and is held lightly. The peak itself never moves
--- under either aggregation.†\footnote{\texttt{python\ build\_\allowbreak{}class\_\allowbreak{}by\_\allowbreak{}era.\allowbreak{}py} --- the committed generator:
...
\textbf{Prominence: a gradient in the provinces, an arc in the national chambers.}
...
out-using the highest is Labov\textquotesingle s crossover pattern, and the middle tier\textquotesingle s
...
the within-member evidence of \cref{sec:generational} shows sitting members do move, at about
...
\toprule\noalign{}
contrast (top vs peak) & \(\rho\), words at \(\geq\)100 occurrences & \(\geq\)300 & \(\geq\)800 & \(\geq\)1,500 \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
contrast (top vs peak) & \(\rho\), words at \(\geq\)100 occurrences & \(\geq\)300 & \(\geq\)800 & \(\geq\)1,500 \\
...
\(-0.022\) per half-decade, t \(-1.67\); short of significance).\footnote{\texttt{python\ class\_\allowbreak{}gap\_\allowbreak{}trend.\allowbreak{}py}, from \texttt{class\_\allowbreak{}by\_\allowbreak{}era\_\allowbreak{}grouped.\allowbreak{}csv} (the
...
\subsubsection{Word mix: the effects live in rate, not vocabulary}\label{sub:wordmix}
...
Scored AI use itself runs \emph{highest} in class I
(11.3\% vs II\textquotesingle s 5.7\%, 27-segment cell, suggestive only).

\subsection{Permeation: detector-independent and small but positive}\label{sec:permeation}

\textbf{The register is permeating human speech, and the drift predates the
...
cell. Across that window the register moves \textbf{+0.0150} (positive in 7 of 10
chamber \(\times\) family cells, permutation p = 0.017), on the identical scored
occurrences.\footnote{\texttt{python\ word\_\allowbreak{}context\_\allowbreak{}prellm.\allowbreak{}py}, reusing the committed occurrence
  ...
  about twice the pre-to-post rate (\textasciitilde+0.0022/yr).} That window establishes permeation without
...
The \textbf{prompting arms} bound the surface share directly, and the bound is
striking. The committed ladder used raw prompts at every stage, so the
ladder\textquotesingle s end-to-end base\(\rightarrow\)instruct shift (+1.24) was never a chat-template
artifact. But a three-shot stylistic prefix on the \emph{raw base model} elicits
more register (46.7 per 1,000) than the whole post-training pipeline
installs as a default (base 27.2 \(\rightarrow\) instruct 44.0 on identical raw prompts):
\textbf{116\% of the base\(\rightarrow\)instruct span, from conditioning alone}. And the
...
model already carries: what post-training moves is the \emph{default} of
unconditioned generation, and deployment conditions select far above that
default. The stage ladder measures the default\textquotesingle s installation; the ceiling
...
\emph{The stage-1 regressor is the blinded screen score (sd 15.3 on the rubric\textquotesingle s 0--100 scale, observed maximum 70), so the per-sd column is the commensurable comparison with stage 2\textquotesingle s binary contrast; the unscaled fitted differences are in Appendix~\ref{app:cut}. ✱ marks the six cells the section\textquotesingle s claim rests on --- justification, common\_good and respect\_groups in each stage; five of the six hold q \textless{} 0.003 under Benjamini--Hochberg across the table\textquotesingle s fourteen cells, and the sixth is disclosed in its footnote.}
...
\textbf{+0.290 raw \(\rightarrow\) +0.157 (t 2.4) at fixed length} --- about half the lift travels
through longer speech and half survives without it; \textbf{respect\_groups is
length-free} (+0.220 \(\rightarrow\) +0.222); common\_good barely moves (+0.229 \(\rightarrow\) +0.199,
t 3.8); and evidence\textquotesingle s marginal +0.065 is entirely length (\(\rightarrow\) \(-0.017\)), so
flagged text has more checkable specifics only by being longer, consistent
with stage 6\textquotesingle s humans-keep-evidence result.
...
purpose-built products. With query-and-retry the search gets \textbf{22.5\%} of
targets past the detector: \textbf{\(9.8\times\)} the humanizers, and \(66\times\) Pangram\textquotesingle s
...
expectation is "\(\approx\)19--22\%, wide interval". What is more than a restatement of
...
involved no fine-tuning, no gradients, and \textbf{no query access to the detector
during the attack} (the detector\textquotesingle s only role was labelling pre-2022 text
...
Two dimensions do run lower on the machine side, and they are the right targets for improvement rather than counterexamples: wild flagged speech is about half as likely to contain anything to engage with (the applicability split, \cref{sec:applicability} --- a raw contrast, not genre-adjusted), and machine text sits below humans on checkable specifics (stage 6) --- with the continuation evidence attributing the applicability collapse to weaker tools in use, not to machine assistance as such (opus-class continuations carry as much to engage with as the humans). Both deficits are visible only because the work is checked directly, which is the argument.
...
recovered structure is \emph{more useful} than what the market prices --- that
position, as the register instruments it, is the referent the market\textquotesingle s
...
directly. The counter-reading is the paper\textquotesingle s own hypothesis: social
structures --- spoken registers among them --- often function to \textbf{mask}
...
thirty-year baseline against the field\textquotesingle s post-2020 windows. Its 9.0\% floor
falls within the 6.5--17.5\% range that Liang et al.\cite{liang2024monitoring,liang2025quantifying} and Gray\cite{gray2025} report for
...
threshold (\(\geq\)8/10), and it characterises a Haiku-class judge rather than the
Sonnet-class judge behind his headline run. The 8\% FPR and the 7.1\% corpus
rate at "possibly AI" (\(\geq\)6/10) are measured at different thresholds and are
...
uniform prevalence sample, and Tasmania\textquotesingle s transcription regime shifted
mid-corpus, which leaves it with no usable control (\cref{sec:hazards}). The
nineteen-chamber constant-window register series of \cref{sec:climb-geography} is drawn differently
again --- it adds Prince Edward Island, which is not in the table below, and it
omits New Brunswick, the discovery corpus of \cref{sec:register-shift}.
...
LLM causation --- which is what lets the 1994--96 onset (\cref{sec:register-shift}) count as a result:
...
\item
  ...
  \textbf{Why it matters beyond linguistics (the Cosmic AC connection):} this is
  ...
  One note on the word \emph{policing} (Matthew, 2026-08-19): at the individual
  level, the mechanism for imperceptible, minimal-control policing already
  exists --- \textbf{control over the LLM or otherwise computerized outputs the
  individual consumes}. Person-targeted influence need not look like
  discipline; it can be a quiet reweighting of each person\textquotesingle s machine diet.
...
uninformative about which texts evade (an exploratory question --- Appendix~\ref{app:superseded}).
...
\texttt{build\_\allowbreak{}member\_\allowbreak{}vectors.\allowbreak{}py}, \texttt{vector\_analysis.py} & \cref{sub:wordmix} word-mix geometry and \cref{sec:model-lineage} model lineage; VECTOR-ANALYSIS.md is the full log \\
...
moves it from +1.03 to \(-0.32\): III\textquotesingle s median birth year is 1974 against
1956--1960 for every other class --- routine non-manual members are the chamber\textquotesingle s
...
\subsection{Education: the provincial ladder (did not replicate at panel scale; Appendix~\ref{app:null} item 16)}\label{sub:education-provincial}
...
The \cref{sec:occupation} prominence effect was first estimated from the Canadian provinces at
...
\textbf{There is no usable pooled slope.} A single slope assumes a gradient that
only some chambers have, so the earlier \(-0.033\sigma\) per log-byte estimate across
...
that is why \cref{sec:occupation} reports prominence as buckets and why no pooled coefficient
...
national chambers arcing --- are reported in \cref{sec:occupation}. What this section adds is
