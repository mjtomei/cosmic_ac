...
in all six complete half-decades (\cref{fig:class-era}) --- and in the partial 2025--26 bin too
...
other bin\textquotesingle s five, cells at half size, VIIab down to 17 members --- is excluded
from the figure and the trend test; its cells are tabulated in
\texttt{class\_by\_era\_all.csv}, and none of them changes a conclusion. The one cell
that moves, V/VI at +0.14 \(\pm\) 0.11 on 56 members, is a one-bin bounce with no
...
\caption{The class profile of the register, 1995--2024, one panel per class:
member-level means over 22 chambers, z-scored within chamber \(\times\) period, so
the overall era rise is removed and each panel reads as that class\textquotesingle s
...
(count annotated). Error bars are \(\pm\)1 se. Class II holds above class I
throughout; the manual and farm classes hold the floor; III --- thin at 26--81
members --- is persistently elevated and the only series that climbs. (\texttt{python\ build\_\allowbreak{}class\_\allowbreak{}by\_\allowbreak{}era.\allowbreak{}py} --- the committed generator
...
\(-0.010\) per half-decade, t \(-0.7\)).\footnote{\texttt{python\ class\_\allowbreak{}gap\_\allowbreak{}trend.\allowbreak{}py}, from \texttt{class\_\allowbreak{}by\_\allowbreak{}era\_\allowbreak{}grouped.\allowbreak{}csv} (the
  ...
  excluded throughout.} Jhering set out the diffusion
...
separation is constant across thirty years (+0.155 \(\rightarrow\) +0.137, trend t \(-0.7\)). A cycle already in progress before our first frame would look
