...
classification buys legibility by discarding information; Scott\cite{scott1998} made that argument about states, and the class schemas are a mild case of it. Machine
...
measure\textquotesingle s best-performing form is exactly that: a folk category,
...
\textbf{The ruler, audited --- what the thirty-year rise is made of.} Selected on
...
endpoint would trend by construction --- so both were tested.\footnote{\texttt{python\ build\_\allowbreak{}word\_\allowbreak{}year\_\allowbreak{}counts.\allowbreak{}py} (one scan, 1.62B tokens;
  ...
  frequency; the donor pool here does not exclude them.} The
rise is concentrated: \textbf{ten common connectives carry 92.5\% of the 1994--96
\(\rightarrow\) 2024--26 climb} (\texttt{this} alone 35\%), and with those ten removed the
aggregate is nearly flat (\(\times\)1.03 pooled, \(\times\)1.10 UK). What rises beneath them
is \textbf{breadth}: the expected distinct style words per 1,000 tokens climbs
\(\times\)1.26 on the UK series (14.4 \(\rightarrow\) 18.2), and 60.4\% of the 407 words trend
...
drift \cref{sec:genre} makes threatening --- leaves the onset in place and slightly
\emph{strengthens} the UK climb (\(\times\)1.49 \(\rightarrow\) \(\times\)1.54). It is not a one-ruler artifact
...
record and \emph{faster} (+2.7\%/yr against +1.4\%/yr, UK 1994--2019) --- though
...
ruler\textquotesingle s UK series. And against donors matched on frequency and dispersion,
the UK series\textquotesingle{} interior minimum at 1994 survives; matched additionally on
pre-2010 trend --- a control that by construction absorbs any onset inside
its own window --- the register\textquotesingle s \emph{distinctive} excess over ordinary
same-shaped vocabulary dates to roughly \textbf{2014--2018}, and the style words
...
instrument-robust; its dated 1994--96 turn is the UK ruler\textquotesingle s; and its
...
\item
  ...
  ambiguous-surname filter): birth \textbf{+0.86 per 1,000 words per decade}
  (t = 14.7 clustered on member, 31.9 on raw member-years) against spoken
  year \textbf{+1.26 per decade} (t = 13.4 clustered, 28.2 raw). A legislator
  ...
  more of the register \emph{in the same year and chamber}. †\footnote{\texttt{python\ cohort\_\allowbreak{}vs\_\allowbreak{}period.\allowbreak{}py}. The headline pair is the two-stamp
    ...
    within-chamber correlations are on the same rows. The script\textquotesingle s
...
\textbf{Formally identified, the two components separate --- and each half of the
...
invariant along the whole line is the same-age decade-on total, \textbf{period +
cohort = +2.11 per 1,000 per decade}.\footnote{\texttt{python\ apc\_\allowbreak{}solution\_\allowbreak{}line.\allowbreak{}py} (re-derives the committed
  two-stamp fit exactly, prints the solution line and the Senate-cell
  bound: n = 1,107 member-years, 160 senators, ages 31--100),
  \texttt{python\ apc\_i.py} (the Luo--Hodges APC-I estimator with a synthetic
  ...
  and \texttt{python\ tenure\_\allowbreak{}within\_\allowbreak{}cells.\allowbreak{}py}. The identification-free contrasts
  are projections off the span of the linear cohort term, so they do not
  depend on the constraint choice.} Reading the pre-drift
...
\textbf{\(-0.88\)/decade} (clustered t \(-3.5\)) and propagates to a \emph{linear} cohort
slope of \textbf{\(-0.03\) {[}\(-0.52\), +0.46{]}}: under the paper\textquotesingle s own predates premise,
the standing level of the juniority gradient is an age-or-career clock, not
a birth-cohort effect. What is cohort-specific is the \textbf{bend}: the
nonlinear cohort deviations --- identified with no assumption at all --- reject
linearity outright (Wald \(p \approx 10^{-7}\)), and the constraint-invariant contrasts
put the cohorts born \textbf{1975 onward +0.46 above the linear profile
(z = 5.1)} --- those born 1985 onward +0.65 (z = 2.6) --- with a
late-minus-early local slope difference of \textbf{+0.70/decade (z = 2.7)}:
...
profile bends up at \emph{both} ends --- the 1920s--30s cohorts also sit above the
line, on the panel\textquotesingle s thinnest cells --- so the identified fact is "nonlinear,
...
within birth-year \(\times\) year \(\times\) chamber cells the tenure slope is large
...
positive, \cref{sec:generational}\textquotesingle s fourth bullet) distinguishes their predictions at all.
...
\textbf{The same knot, along the tenure axis, pre-empts the seniority reading.}
...
legislator\textquotesingle s rate against their own career mean --- register \textbf{rises} with
...
colleagues instead --- rate minus its chamber \(\times\) year mean --- it \textbf{falls},
...
curve is \cref{sec:generational}\textquotesingle s cohort gradient seen sideways, not seniority wearing it
down. The figure makes the crossing visible.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{tenure_profile.png}
\caption{Register by length of service, two readings. Blue: within-member, each
legislator\textquotesingle s rate minus their own career mean --- rises with tenure because
...
minus its chamber \(\times\) year mean --- falls with tenure, but this is the
...
member-cluster bootstrap 95\% intervals. (\texttt{python\ tenure\_\allowbreak{}profile.\allowbreak{}py}. Entry is a member\textquotesingle s first observed
  year in a chamber; members present in the chamber\textquotesingle s first covered year
  are dropped as left-censored (mirrors \texttt{cohort\_\allowbreak{}vs\_\allowbreak{}period.\allowbreak{}py\ -\allowbreak{}\/\allowbreak{}-\allowbreak{}censor\ 1}).
  Word-weighted; tenure binned 0--2 / 3--5 / 6--9 / 10--14 / 15--19 / 20--29 /
  30+; the within-member slope on tenure is +0.36/decade and the
  period-adjusted slope \(-1.03\)/decade --- the \(-0.95\) tenure test of \cref{sec:generational},
  drawn.)}
\label{fig:tenure}
\end{figure}
...
was in place decades before the machines.\footnote{\texttt{python\ build\_\allowbreak{}ladder\_\allowbreak{}by\_\allowbreak{}era.\allowbreak{}py}, then
...
metric keeps it clearly ahead. The \textbf{prompting arms} bound the surface
...
measures the default\textquotesingle s installation; the ceiling was always promptable. And the tempting stronger story --- \emph{that the preference data itself
...
family \textbf{the change is carried within sitting members} --- pooled post\(-\)pre
+0.0053, within-member +0.0054, composition \(\approx\) 0 --- though the within
component alone, at 280 stayers, carries a wide interval ({[}\(-0.028\),
+0.046{]}): turnover is not the driver at the point estimate, and
member-level power is what limits the standalone claim.\footnote{\texttt{python\ permeation\_\allowbreak{}member\_\allowbreak{}logprob.\allowbreak{}py} --- the seg-id join, the
  ...
  committed per-family result already showed.} On the
frequency side, panel-wide across all 22 chambers, the post-2022 rise of
the arm\textquotesingle s own subtler subset (the 350 rare style words) decomposes as
\textbf{within-member 26\% {[}17, 35{]}, composition 58\%, interaction 16\%} --- a
...
within term is \emph{negative} --- stayers\textquotesingle{} use of the instrument\textquotesingle s common-word
bulk falls while entrants carry the rise --- and \cref{sec:generational}\textquotesingle s member-level
regressions run on exactly that pooled rate.\footnote{\texttt{python\ permeation\_\allowbreak{}decompose.\allowbreak{}py}, on the panel member cache.
...
the paper\textquotesingle s claims are stated per dimension rather than from the
index.\footnote{\texttt{python\ quality\_\allowbreak{}latent.\allowbreak{}py} --- polychoric matrix on the seven
  ...
  is the remaining refinement.} The
...
\subsection{The measure against the market\textquotesingle s measure}\label{sec:disc-proxy}
...
joint model (\cref{sec:class}) --- and, with the anchors joined, holds against the
...
--- and the check that discriminates them is already registered (\cref{sec:future},
...
\subsection{The occupational classification, in full}\label{sec:retired}
...
widened errors are shown rather than hidden (\cref{sec:class}).
...
\subsection{Instruments retired, and why}\label{sec:retired-h}
