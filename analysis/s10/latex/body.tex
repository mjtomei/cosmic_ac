...
(Appendix~\ref{app:future} item 8), and the study does not attribute the between-chamber differences to
any measured cause.
...
that block\textquotesingle s baseline, and for the continuous ladder terms, marked \texttt{/sd}, it
is years per standard deviation. Those two are not on the same footing and
should not be read down the column as one scale: †\footnote{\texttt{python\ joint\_\allowbreak{}predictors.\allowbreak{}py}. Canonical member-level spec --- one
...
Free work against the middle: \(-0.07\) to \(-0.10\), far shy of any status-top\textquotesingle s
avoidance --- mostly out of the game, which is what sitting outside the
hierarchy predicts, its low register level (\cref{sec:occupation}) being closer to uniform
abstention than to word-selective avoidance (a directional reading: at these
word counts, equivalence within \textbar \(\rho\)\textbar{} \textless{} 0.15 is not formally establishable). The
folk-bottom series is the battery\textquotesingle s only positive one, strengthening with
volume. Chase below the peak, flight above it, near-indifference off the
ladder: the full fashion-cycle geometry, read from one instrument\textquotesingle s levels, at
group level.
...
cell --- it moves \textbf{+0.0150} (positive in 7 of 10
...
is a \emph{data} claim, not an objective claim. On this ladder the two are
...
data isn\textquotesingle t chat" cannot be separated here. Three checks tighten what can
...
A blinded, date-blind LLM screen of 37,801 segments separates Pangram\textquotesingle s
classes at AUC 0.954 {[}0.934, 0.971{]} on the 618-segment overlap
(Appendix~\ref{sec:screen-pangram}).
...
\begingroup\small
\begin{longtable}[]{@{}lrp{0.26\linewidth}p{0.26\linewidth}@{}}
\caption{Applicability of the two sentinel dimensions, by band of the blinded screen score.}\label{tab:applicability}\\
\toprule\noalign{}
screen score & segments & respect\_demands applicable & respect\_counterargs applicable \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
screen score & segments & respect\_demands applicable & respect\_counterargs applicable \\
...
The engagement null below is conditional: AI-flagged speech \emph{engages no worse
...
arms --- the study\textquotesingle s existing audited generation caches (the \cref{sec:chase-flight}
...
The result does not arise from a weak detector or a trivial prompt. Pangram produced 0 false positives in 1,260 pre-2022 controls (\cref{sec:calibration}), and a single "make this sound human" prompt makes text \emph{more} detectable, not less. That is what makes the reversals worth reporting: they are movements \emph{into} a class the detector otherwise reaches zero times out of 1,260. This is not a detector that is merely noisy at the boundary --- it is one with a hard specificity floor that a directed search nevertheless walks text across. Under the tested attacks, the frontier-model search has a 4.8-fold advantage over purpose-built evasion products on one submission and a 9.8-fold arithmetic advantage when retries are allowed. The experiment does not estimate how either attack will improve.
...
\subsection{Evasion rewriting leaves deliberative quality unchanged}\label{sec:evasion-quality}
...
The paper\textquotesingle s quality conclusion does not rest on these arms in any case. It is
...
it with no usable control (\cref{sec:hazards}). The register series in \cref{sec:climb-geography} additionally
...
The constant-window register series in \cref{sec:climb-geography} additionally includes Prince
Edward Island; New Brunswick, the discovery corpus of \cref{sec:register-shift}, is not in that
...
alone, and none is asked to. This is the study\textquotesingle s most consequential design
choice and the reason its result differs from published comparators (\cref{sec:related}).
The comparators calibrate more coarsely: the closest parliamentary study\cite{suvanto2026} measures false positives on one pre-LLM hold-out per parliament, in two
...
multi-label, the elements that mark the \textbf{free / bottom / middle / top} of a
...
\textbf{There is no usable pooled slope.} An earlier estimate of \(-0.033\sigma\) per
