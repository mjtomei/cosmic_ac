...
remains the central unknown and is discussed in \cref{sec:limits}.\footnote{\texttt{prevalence\_report.py}.}
...
itself is the nineteen-chamber comparison of \cref{sec:climb-geography}. Whatever this measures,
...
\subsubsection{Class and education: the same shape at coarser grain}\label{sec:class}
...
\subsubsection{Socioeconomic position as organisational altitude: the register peaks at the insulated middle, and free work sits lowest}\label{sec:occupation}
...
\subsubsection{Prominence: a gradient in the provinces, an arc in the national chambers}\label{sec:prominence}
...
\subsubsection{All four predictors at once}\label{sec:four-predictors}
...
\subsubsection{Chase and flight: an exploratory interpretation of the social gradients}\label{sec:chase-flight}
...
+0.34, t +3.5), the quality-side echo of \cref{sec:posttraining}.
...
about \textbf{\(2\times\)}. Both rates are tabled with their vendor comparators in \cref{sec:evasion-accounting}.
...
\subsubsection{Calibration of small effects}\label{sub:calibration-small}
...
discovery corpus of \cref{sec:acceleration}.
...
\item
  ...
  null and is reported in \cref{sec:prominence}.
\item
...
national chambers arcing --- are reported in \cref{sec:prominence}. What this section adds is
...
\item
  ...
  table.
\item
  \textbf{The original run\textquotesingle s two group-pairs on respect\_demands}, cut from \cref{sec:quality}.
...
\item
  \textbf{The length correlations in the paired evasion arms}, cut from \cref{sec:evasion-quality}.
...
\item
  \textbf{The education case, and its two costs}, cut from \cref{sec:disc-substitution}. A different
...
\item
  \textbf{Rice 2026\textquotesingle s own specificity calibration}, cut from \cref{sec:related}. Verified and
...
\item
  ...
  watermark}, cut from \cref{sec:disc-norms}. Accurate vendor deployment detail; it left the
