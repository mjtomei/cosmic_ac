...
run over 124,000 Australian federal speeches, found no post-ChatGPT rise (Rice\cite{rice2026}) --- and a phrase-spike claim in the UK Commons with no prevalence
...
are specified in \cref{sec:method} and alongside each result below; every number reproduces
...
rises from \textbf{7.36\% {[}6.13\%, 8.62\%{]} in 2025 to 11.70\% {[}9.86\%, 13.67\%{]} in 2026}
--- the 9.03\% scalar averages a still-rising series, not a steady state, and
comparisons against it should say which year they mean.\footnote{\texttt{python\ prevalence\_\allowbreak{}robustness.\allowbreak{}py} --- the year split at the
...
bias. Nor does one large chamber drive the result: the equal-weight mean of the per-chamber rates (\cref{tab:prevalence}, drawn in \cref{fig:prevalence-dots}) is 9.12\%, against the 9.03\% word-weighted pooled
rate.\footnote{\texttt{banded\_prevalence.py}.}
...
\toprule\noalign{}
group & mean, per 100k & sd of own series & gap to US House, per 100k & gap ÷ US House\textquotesingle s sd \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
group & mean, per 100k & sd of own series & gap to US House, per 100k & gap ÷ US House\textquotesingle s sd \\
...
\bottomrule\noalign{}
\endlastfoot
CA provinces & 2,054 & 61 & \textbf{+155} & \textbf{+1.28} \\
...
\caption{Register by length of service, two readings, each curve labelled in the panel; the vertical axis is register rate per 1,000 words. The within-member curve --- each legislator\textquotesingle s rate minus their own career mean --- rises with tenure, because within a career years served and calendar year are one clock (its slope reproduces the +0.51 within-member drift). The period-adjusted curve --- rate minus its chamber \(\times\) year mean --- falls with tenure, but that fall is the entry-cohort gradient (longer tenure at a fixed year = earlier cohort), not a within-career decline. Left-censored members dropped; error bars member-cluster bootstrap 95\% intervals.}
\label{fig:tenure}
\end{figure}
...
The ancestry is Weeden and Grusky\textquotesingle s\cite{weeden2005} case for measuring class at the disaggregated occupation, and Kohn and Schooler\textquotesingle s\cite{kohn1983} occupational self-direction, where closeness of supervision explained class differences in psychology. What the measure adds to that line is distance from the hierarchy, not only altitude within it. What follows is the U over those levels, the off-ladder level\textquotesingle s stability across three decades, and, in the joint model below, the absorption of the class label the measure refines.
...
that holds it abandons the most conspicuous forms first. \emph{Flight} appears here
in class I\textquotesingle s avoidance of words that became common, and dynamically at the
word level, where the most conspicuous machine tells peak and then recede
(\cref{sec:register-shift}). The class cycle itself is \textbf{not} observed in motion. Its peak does
...
significance).\footnote{\texttt{python\ class\_\allowbreak{}gap\_\allowbreak{}trend.\allowbreak{}py}, from \texttt{class\_\allowbreak{}by\_\allowbreak{}era\_\allowbreak{}grouped.\allowbreak{}csv} (the
  ...
  across the six equal five-year bins, 1997--2026.} Jhering set out the diffusion mechanism in \emph{Der Zweck im
Recht} (1883); Durkheim\textquotesingle s summary supplied the phrase --- fashion, once
...
support the cycle\textquotesingle s dynamic half on either sufficient signature: the class
profile holds its shape in every half-decade rather than migrating, and the
II-over-I separation narrows rather than widens (\cref{sec:class}). A cycle already in
progress before our first frame would look the same, which is why the
discriminating study is class-coded speech from before 1996 (Appendix~\ref{app:future}).
...
other. This is register lineage, not attribution (Appendix~\ref{app:future} item 3).
...
segments; on the 618 segments where it overlaps Pangram it separates Pangram\textquotesingle s
classes at AUC 0.954 {[}0.934, 0.971{]} (Appendix~\ref{sec:screen-pangram}), so its score can be read
...
\toprule\noalign{}
dimension & stage 1, per sd of score (n = 840) & stage 2, AI/Mixed vs Human verdict (n = 682) & stage 1, fitted 0\(\rightarrow\)100 difference \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
dimension & stage 1, per sd of score (n = 840) & stage 2, AI/Mixed vs Human verdict (n = 682) & stage 1, fitted 0\(\rightarrow\)100 difference \\
...
\bottomrule\noalign{}
\endlastfoot
justification & \textbf{+0.173} (t +4.1) ✱ & \textbf{+0.290} (t +4.4) ✱ & +1.134 \\
common\_good & \textbf{+0.089} (t +3.3) ✱ & \textbf{+0.229} (t +4.5) ✱ & +0.583 \\
respect\_groups & \textbf{+0.044} (t +2.0) ✱\footnote{The one starred cell that does not survive multiple-testing
  ...
  judge leakage.} & \textbf{+0.220} (t +6.1) ✱ & +0.287 \\
respect\_demands & \(-0.027\) (t \(-0.5\)) & \(-0.029\) (t \(-0.4\)) & \(-0.179\) \\
respect\_counterargs & +0.022 (t +0.4) & +0.073 (t +1.0) & +0.143 \\
constructive & \(-0.002\) (t \(-0.1\)) & \(-0.041\) (t \(-1.0\)) & \(-0.015\) \\
evidence & +0.054 (t +1.4) & +0.065 (t +1.1) & +0.350 \\
\end{longtable}
\endgroup

\emph{The first and last stage-1 columns report the same fits and the same t statistics at different scales. The stage-1 regressor has sd 15.3 on a 0--100 scale and an observed maximum of 70; the fitted 0\(\rightarrow\)100 difference therefore extrapolates beyond the data. The per-sd column is the fitted 0\(\rightarrow\)100 difference times sd/100 and is the commensurable comparison with stage 2\textquotesingle s binary contrast. ✱ marks the six cells the section\textquotesingle s claim rests on --- justification, common\_good and respect\_groups in each stage; five of the six hold q \textless{} 0.003 under Benjamini--Hochberg across the table\textquotesingle s fourteen cells, and the sixth is disclosed in its footnote.}
...
arms --- the study\textquotesingle s existing audited generation caches (the \cref{sub:wordmix}
...
Pangram\textquotesingle s own technical report --- products built and tuned \emph{against} detectors
--- which get \textbf{2.31\%} of documents past it, this search\textquotesingle s commensurable
one-submission rate is \textbf{11.1\%} of variants (\textbf{\(4.8\times\)}); with query-and-retry
it gets \textbf{22.5\%} of targets past it (\textbf{\(9.8\times\)}). Against Pangram\textquotesingle s
clean-conditions false-negative rate of 0.34\%, the retry-enabled rate is \(66\times\).

The result does not arise from a weak detector or a trivial prompt. Pangram produced 0 false positives in 1,260 pre-2022 controls (\cref{sec:calibration}), and a single "make this sound human" prompt makes text \emph{more} detectable, not less. That is what makes the reversals worth reporting: they are movements \emph{into} a class the detector otherwise reaches zero times in 1,260 --- not boundary noise, but a hard specificity floor that a directed search nevertheless walks text across. Those multiples describe the attacks actually run, not the attack class. What the experiment does not do is estimate how far either attack improves from here.
...
\caption{One-submission false-negative rates: the vendor-reported rates and this study's per-variant rate.}\label{tab:bypass-summary}\\
\toprule\noalign{}
...
\begingroup\small
\begin{longtable}[]{@{}p{0.22\linewidth}p{0.20\linewidth}p{0.50\linewidth}@{}}
\caption{Retry-enabled exposure: the per-target success rate when the attacker may query the detector and retry.}\label{tab:bypass-exposure}\\
\toprule\noalign{}
...
The composite weights the seven DQI dimensions equally --- the index\textquotesingle s conventional additive form --- rather than by any measured optimum. Weighting turns out to matter, and the direction survives it.
...
and time depth (a thirty-year baseline against the field\textquotesingle s post-2020
windows). Its 9.0\% floor also falls within the 6.5--17.5\% range that Liang et al.\cite{liang2024monitoring,liang2025quantifying} and Gray\cite{gray2025} report for professional prose, providing population-level
corroboration.
...
prevalence sample, and Tasmania\textquotesingle s transcription regime shifted mid-corpus,
which leaves it with no usable control (\cref{sec:hazards}). The constant-window register
series in \cref{sec:climb-geography} additionally includes Prince Edward Island; New Brunswick, the
discovery corpus of \cref{sec:register-shift}, is not in that nineteen-chamber table.
...
That conclusion requires a transfer assumption. Specificity is measured on each chamber\textquotesingle s pre-2022 record, while the permeation result shows contemporary human speech drifting toward the register on which the detector relies. Thus, \(\mathrm{Sp} = 1\) for human speech in 2025--26 is assumed rather than measured. \cref{sec:calibration} bounds the cost of that assumption at no more than about 0.2 points under the harshest reading of the audited hits; the Limits return to it.
...
reaches past this study\textquotesingle s question, in follow-ons to the register and
...
\item
  \textbf{Vendor attribution for the family signature.} \cref{sub:wordmix}\textquotesingle s vector result ---
...
\item
  ...
  the claim is about the behavior, not about any one detector\textquotesingle s opinion of it.
\item
...
\texttt{build\_\allowbreak{}member\_\allowbreak{}vectors.\allowbreak{}py}, \texttt{vector\_analysis.py} & \cref{sub:wordmix} word-mix geometry; VECTOR-ANALYSIS.md is the full log \\
...
from their evidence-URL titles (\texttt{build\_\allowbreak{}t1\_\allowbreak{}wiki\_\allowbreak{}depth.\allowbreak{}py}, 3,999 of 4,008
...
The quintile table in \cref{sec:chase-flight} is the estimate; this section is why it is the
only form worth quoting.

The two shapes themselves --- seventeen sub-national chambers declining, five
national chambers arcing --- are reported in \cref{sec:chase-flight}. What this section adds is
