...
**Counts.** 114 open (0 critical, 42 major, 72 minor; 92 CONFIRMED, 22 PARTIAL); 1 refuted; 0 unresolved.
...
- **[major] [cohort+class]** 4.6a, era figure + text: "class II sits above class I in all seven half-decades, and the manual-and-farm tail stays ..." — *Matthew asked whether the inversion looks like noise when charted; the per-class small-multiples (scratchpad class_by_era_smallmultiples.png) say yes: V/VI 2025-26 is +0.166 +/- 0.118 on 55 members, ~1.4 se from its own series mean, one bin after the series minimum, with no preceding trend; IVc stays at the floor (-0.22) and VIIab has no 2025-26 cell (n<25), so the pooled tail's rise to +0.02 is composition, not movement. The floor sentence, figure title and caption now state exactly that (six of seven at the floor; the seventh a small-cell wobble, watched as a candidate inversion, read as noise). The class_by_era generating-script half of the finding lives on in CC10 ADDENDUM (Matthew, 2026-08-26): the partial 2025-26 bin is now excluded from the figure and trend test entirely (two years vs five, half-sized cells) and the pooled single-axes figure replaced by per-class small-multiple panels drawing every cell including n<25 (the thin 1995-2004 manual cells turn out to exist and sit at the floor, extending the floor claim back to 1995); partial-bin cells tabulated in class_by_era_all.csv with none changing a conclusion. The era CSVs now come from the committed build_class_by_era.py (also closing CC10's class half); its documented spec reproduces the old uncommitted values to within hundredths, and the text requotes from it*
...
- **[major] [cohort+class]** 4.6a, 'Flight' + footnote r46e: "ρ = −0.13 at 100+ occurrences, −0.22 at 300+, **−0.42 at 800+ (p = 0.004)**, −0.46 at 1,50..." — *closed in two halves: (1) reproducibility — flight_correlation.py recovers the exact spec and reproduces -0.13/-0.22/-0.42/-0.46 to the digit, cited at r46e; (2) Matthew's cross-ladder extension run and reported in the Flight passage — the signature reproduces at every status-top (professional vs bachelor -0.33 at 1500+; executive vs middle -0.37 at 800+; office vs non-office -0.20) and fails at the one top that is not one (graduate, flat-to-positive), stated as a robustness family with the non-independence caveat (build_ladder_word_year.py + flight_ladders.py committed). The CC2 noise-vs-inversion decision remains its own open item*
