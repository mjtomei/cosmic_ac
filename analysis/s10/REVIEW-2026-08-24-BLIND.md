...
**Counts.** 117 open (0 critical, 44 major, 73 minor; 95 CONFIRMED, 22 PARTIAL); 1 refuted; 0 unresolved.
...
- **[major] [cohort+class]** 4.6a, 'All four predictors at once' + footnote r46joint: "| **education** (block Wald) | p = 0.0025 | **p = 0.2150** |..." — *adjudicated (Matthew: dummies — matching the education table's own coding — and truly-alone explained and adopted): joint_predictors.py now codes education as level dummies (bachelor baseline) and its alone fits were already block-only; the printed table regenerated from the committed script. Joint column essentially unchanged (it was the dummies spec); the alone column repaired and every raw association larger (class II +0.122 t3.63, VIIab -0.384 t-3.70, edu p<1e-4 alone -> p=0.215 joint); prominence's joint linear term now honestly dies (-0.017 t-0.46), strengthening the see-D.3 caveat. Footnote states the spec; the explanatory-power table checked and unaffected (always dummies; no alone column)*
