...
def load_style():
    """Instrument = the third of Kobak style words that instruct-tuning most
    over-produces relative to its own base checkpoint. The split is defined
    ENTIRELY from paired model output -- no Hansard, no post-2023 data -- so it
    cannot be selection on the outcome. The protocol's own placebo then
    frequency- and dispersion-matches to whatever profile that subset has."""
    import re as _re
    ...
    TOKL = _re.compile(r"[a-z']+")
    METAL = _re.compile(r"\b(this (text|passage|speech|statement|excerpt)|the speaker|"
                        r"appears to be|summary|analysis|here is|sure[,!]|as an ai)\b", _re.I)
    B, I = Counter(), Counter()
    bn = inn = 0
    for fam in ("qwen3", "mistral"):
        ...
        for k in range(len(i)):
            if METAL.search(i[k][:300]) or METAL.search(b[k][:300]):
                continue
            tb = TOKL.findall(b[k].lower()); ti = TOKL.findall(i[k].lower())
            B.update(tb); I.update(ti); bn += len(tb); inn += len(ti)
    seen = [w for w in style if B[w] + I[w] > 0]
    seen.sort(key=lambda w: -math.log(((I[w] + 0.5) / inn) / ((B[w] + 0.5) / bn)))
    sub = sorted(seen[:len(seen) // 3])
    print(f"INSTRUMENT: {len(sub)} most instruct-preferred of {len(style)} style words")
    print(f"  e.g. {', '.join(sub[:14])}")
    return sub
