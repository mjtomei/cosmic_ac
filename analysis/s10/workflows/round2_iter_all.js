...
const SCOPES = (args && args.scopes) || [{ id: 'full', scope: 'all' }]
...
const PROPOSE_PROMPT = (variant, SCOPE) => `ROLE: PROPOSE.
...
const runScope = async ({ id, scope }) => {
  const variants = variantsFor(scope)
  const jobs = []
  for (const m of PANEL) for (const v of variants) jobs.push({ m, v })

  const raw = await parallel(jobs.map(({ m, v }) => () =>
    agent(PROPOSE_PROMPT(v.id, scope),
          { agentType: m.type, label: `propose:${id}:${m.id}:${v.id}`, phase: 'Propose',
  ...
  const faiths = await parallel(candidates.map(c => () =>
    agent(FAITH_PROMPT(c), { agentType: 'check-faithfulness', label: `faith:${c.id}`,
  ...
  const scored = await parallel(survivors.map(cand => async () => {
    ...
    const ballots = await parallel(PANEL.map(v => () =>
      agent(prompt, { agentType: v.type, label: `vote:${cand.id}:${v.id}`, phase: 'Vote',
...
phase('Propose')
log(`iteration ${ITER}, phase ${PHASE}: ${SCOPES.length} scope(s), ${PANEL.length} seats`)
...
return { iteration: ITER, phase: PHASE, scopes: ok.map(r => r.scope_id), totals, by_scope: ok }
