...
const CODE_SCHEMA = {
  ...
  properties: {
    chunk: { type: 'integer', description: 'the chunk INDEX you were given in the prompt (0-based), not an id' },
    n_items: { type: 'integer', description: 'how many entries you coded' },
    notes: { type: 'string' },
    codings: {
      type: 'array',
      items: {
...
'',
'Read ' + POOL + ' in the current directory — an array of {id, occupation, n_members, egp_existing, chambers}, sorted by how many legislators held that occupation. Take entries at zero-based indices ' + lo + ' through ' + hi + ' inclusive (the last chunk may run past the end; do the entries that exist). Code ALL of them.',
'',
'You are chunk index ' + i + '. Return that number as "chunk", and the pool "id" VERBATIM for every entry. It is the join key; an altered id makes the record unusable.',
'',
pass === 2
...
// RESUME: batches whose first pass survived the 2026-08-17 session-limit stop
// are loaded from soc_pass1_salvaged.json rather than re-run. Those agents saw
// the byte-identical prompt this script still generates, so the salvaged votes
// are the same measurement, and blindness is unaffected -- pass 2 has never
// seen them either way.
const DONE = new Set(ARGS.pass1_done || [])
phase('Code')
const pass1 = await parallel(Array.from({ length: N_BATCH }, (_, i) => () =>
  DONE.has(i) ? Promise.resolve(null)
...
const m1 = {}, m2 = {}
// salvaged first votes, then anything this run produced
for (const [id, c] of Object.entries(SALVAGE)) m1[id] = { id: Number(id), ...c }
