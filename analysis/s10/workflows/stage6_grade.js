export const meta = {
  name: 'stage6-dqi-grading',
  description: 'Grade never-human-reviewed machine continuations against their human twins on the frozen v2b DQI rubric, two blind passes',
  phases: [{ title: 'Grade' }, { title: 'Write' }],
}

// WHY THIS RUN EXISTS
//
// Stage 6 of the quality arm (plans/S10-stage6-unreviewed-continuations.md).
// Tests the worried-humans-vetted-harder account of the form lift (review
// item Q1, Matthew 2026-08-19): the pool mixes 60 real pre-2023 human
// continuations with 180 machine continuations (mistral/qwen3 instruct +
// mistral base) that NO human ever reviewed, blind, length-matched within
// prompt. Rubric is the frozen v2b text from stage 5, VERBATIM -- the
// instrument must not drift between stages.
//
// JUDGE MODEL IS PINNED to opus: stages 1-5 were graded in Opus sessions,
// and this run is dispatched to arch-home, whose session model is Fable.
// An unpinned judge would silently change the instrument.
const ARGS=(typeof args==='string')?JSON.parse(args):(args||{})
const POOL=ARGS.pool||'analysis/s10/quality_expansion/pool6.json'
const PASSES=ARGS.passes||2
const BATCH=ARGS.batch||16
...
phase('Grade')
const ids = ARGS.ids            // array of pool ids, supplied by the runner
const chunks=[]
for(let i=0;i<ids.length;i+=BATCH) chunks.push(ids.slice(i,i+BATCH))
const jobs=[]
for(let pass=1;pass<=PASSES;pass++)
  for(let c=0;c<chunks.length;c++) jobs.push({pass,c,b:chunks[c]})
const res=await parallel(jobs.map(({pass,c,b})=>()=>
  agent(RUBRIC
    + "\n\nRead the JSON file "+POOL+" (Read tool): an array of {id, text}."
    + " Score ONLY these segments, one score object per id, all fields"
    + " required, integers only, using the anchors above: "+b.join(', '),
    {label:'p'+pass+':b'+c,phase:'Grade',schema:SCHEMA,
      model:'opus',effort:'medium'})
    .then(r=>({pass,scores:(r&&r.scores)||[]}))))
const rows=[]
for(const r of res.filter(Boolean))
  for(const s of r.scores) rows.push({pass:r.pass,...s})
log(rows.length+' score rows across '+PASSES+' passes of '+ids.length+' texts')

phase('Write')
await agent('Write the JSON array below, EXACTLY as given, to'
  +' analysis/s10/quality_expansion/results_stage6.json (Write tool), then'
  +' reply with the row count and distinct id count.\n\n'
  +JSON.stringify(rows),
  {label:'write',phase:'Write',model:'opus',effort:'low'})
return {rows:rows.length,texts:ids.length,passes:PASSES}
