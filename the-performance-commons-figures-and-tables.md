...
**Figure 2 — The embargo natural experiment, drawn.**  (§5)
...
**Figure 3 — Distributed computing is a collective-action problem, not a hardware limit.**  (§7)
...
**Figure 4 — Machine demand has no floor.**  (§7)
...
**Figure 5 — Participation through the platform's death.**  (§8)
...
**Figure 6 — Where the optimum lands as the commons matures.**  (§11)
...
**Figure 7 — Reconfigurable hardware is already moving in; the commons is the inflection.**  (§11)
...
| — | §3–§4 | privilege → collective action (prose only) |
| Figure 2 | §5 | the embargo natural experiment: chips restricted, parity anyway |
| Table 2 | §6 | the same failure is everywhere in the stack |
| Figure 3 | §7 | distributed computing is the same problem — the wasted supply (Q2: software) |
| Figure 4 | §7 | unbounded machine demand makes the idle resource valuable |
| Table 3 | §8 | the entry cost of first silicon has collapsed |
| Figure 5 | §8 | the commons survives its platform's death, measured |
| — | §9–§10 | (AI multiplies the commons; the vendor is the aggregation point — prose only) |
| Figure 6 | §11 | where the optimum lands as the commons matures |
| Figure 7 | §11 | and it is already underway |
...
- **Figure 6's boundary** — the real position of the ASIC / CPU / CGRA optimal regions needs the whole-system workload characterization §12 calls the prerequisite (achieved-vs-achievable parallelism, dataflow locality, phase and input statistics across the real consumer mix).
