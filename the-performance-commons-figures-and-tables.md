...
*Read top to bottom, the visuals carry the paper's argument on their own: **magnitude → mechanism → the same problem, distributed → generality → proof → destination → already underway.** Each entry notes the beat it carries and whether it rests on measured data or is an argued/conceptual construct.*
...
**Figure 2 — Distributed computing is a collective-action problem, not a hardware limit.**  (§3)
...
**Figure 3 — Machine demand has no floor.**  (§3)
...
**Table 3 — Open commons with higher relative evolutionary fitness.**  (§7)
...
**Figure 4 — Where the optimum lands as the commons matures.**  (§10)
...
*Beat: over time the optimum for real workloads migrates into reconfigurable silicon — not fixed ASICs.* — **conceptual**; the boundary is exactly what a bigger whole-system characterization (§11) would pin down.

**Figure 5 — Reconfigurable hardware is already moving in; the commons is the inflection.**  (§10)
...
| Figure 2 | §3 | distributed computing is the same problem — the wasted supply (Q2: software) |
| Figure 3 | §3 | unbounded machine demand makes the idle resource valuable |
| Table 2 | §6 | the same failure is everywhere in the stack |
| Table 3 | §7 | the commons has solved it before, and wins |
...
- **Figure 4's boundary** — the real position of the ASIC / CPU / CGRA optimal regions needs the whole-system workload characterization §11 calls the prerequisite (achieved-vs-achievable parallelism, dataflow locality, phase and input statistics across the real consumer mix).
- **Figure 3's demand curve** — conceptual; the ~$0.50 vs ~$0.08 cost anchors are from Omerta's reliability-market simulation and the task-value ladder from its economic-analysis notes. Omerta itself is partially built: a substantial, well-tested mesh-networking layer (encryption, NAT traversal, gossip discovery, tunnelling, plus Terraform-managed bootstrap/STUN servers) and working ephemeral-VM and simulation components, but the end-to-end compute loop is mid-migration and the trust/payment chain is validated in simulation rather than running live (two of six transaction types tested).
