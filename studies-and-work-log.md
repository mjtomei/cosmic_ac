...
- The rise is **generational as well as period-wide**: regressing the
  ...
  Whether sitting members' own register changed is a separate null.
...
| 2026-08-07 | **S10 sixteen-chamber panel built**: 8 Canadian provinces + 6 Australian states (203.6M words) + Scotland/Wales/NI, each from its own archive with a bespoke extractor and a mandatory validation report | The register's rise is **generational as well as period-wide** (birth-vs-spoken-year split, §4.6, `cohort_vs_period.py`): birth cohort predicts **+0.95 per 1,000 per decade (t 28)** net of calendar period, the stronger organiser in 10/13 chambers. Sitting members' own register did not measurably change (null, Appendix A). |
| 2026-08-07 | **Member biographies collected** (1,396 provincial members, 79% birth-year coverage via Wikidata/enwiki) → the cohort regression | **Birth-decade gradient +1.05 per 1,000 per decade (t 8.5, clustered on member), +1.14 with occupation and education controlled.** Candidate professionalisation and educational expansion refuted: communications, law and post-secondary all the wrong sign. Cohort not age: chamber age flat (52.2→51.4) while the register rises. |
...
| 2026-08-10 | **S10 bypass arm** — four adversarial search runs across two chambers (461 rewritten variants of 92 distinct Pangram-flagged originals (106 attacked; the two NB runs re-attack the same 40 texts — **corrected 2026-08-12 from "130"**, which summed per-run seed counts and double-counted New Brunswick)): v2 blind, v3 contrastive, Opus-seeded, and a clean uniform-draw run. Selection filters differ per run and are documented in `BYPASS_METHODOLOGY.md`. | **An off-the-shelf model in a rewrite loop beats the commercial evasion industry ~10×.** Per target (final search per chamber, ≥1 variant reaching a clean Human verdict): **24.6%** [16, 36], 23.1% strict — against **2.31%** for the 13 humanizer services in Pangram's own report, i.e. **10.6×**, and 72× its clean FNR. No fine-tuning, no detector access, no evasion tooling. Like-for-like on single attempts the multiple is 3.7× (2.31% is one-shot, 24.6% allows ≤18 attempts — state this whenever the 10× is used). Conservative per-variant rate **8.5%** [6.3, 11.4], **8.0%** strict. Both against **0 AI labels in 1,260** pre-AI controls. The two chambers agree (26.3%/22.2%) despite contradicting each other on which edits work — the rate transfers, the playbook does not. Quality unaffected: 7 DQI dimensions null across 63 paired grades, and — **superseded 2026-08-12** — what was then read as "stage 4 shows stage 3's justification gain was length" does not hold: the length correlation flips sign between the arms (+0.33 stage 4, −0.24 stage 3, neither significant). `bypass_report.py` is the single source. |
| 2026-08-11 | **S10 verification close-out** — all nine † figures re-derived from their artifacts; footnotes added giving script and invocation for each. | All nine reproduce. **A figure I had reported as unsourced was real and my error:** +0.88 is `rlhf_pref_analyze.py` (`EXCESS +0.8797`), not `align_ratio.py`, which prints the Hansard-drift arm. Lesson recorded: for figures carried across sessions the transcript is the authoritative provenance — search it before re-running anything. |
...
| 2026-08-12 | **S10 review response, M2** (quality arm) — §4.9 had explained away a +0.16 justification drift in stage 4 as "the apparent gain was length", quoting the length-adjusted column. | **Two problems, and Matthew's framing fixed the deeper one.** Statistically the adjusted column is an intercept at zero word delta, and 24 of 25 stage-4 rewrites got longer — a zero delta sits 2.11 sd outside the sample on one observation, VIF 5.43, SE inflated 2.30× (predicted 2.33×), so the sign flip is extrapolation not evidence. Stage 3 is unaffected (VIF 1.00, raw and adjusted agree to 0.002). But the framing was wrong too: **length is not a confound here.** On this rubric a longer passage genuinely has room to justify more, and earning justification by adding words is a strategy a human with a longer slot could use. So the adjustment is dropped rather than repaired: **the table now reports raw paired differences for both stages** (all still null), with length reported as a co-travelling fact. `analyze_stage3.py` no longer says "Quote that column" unconditionally — the length-adjusted column is **removed entirely** (Matthew: overcomplicated). It was never needed — the design is within-text paired, so content, speaker and occasion are fixed by construction and there was nothing for a covariate to control. Replaced by `r(words)`, a plain correlation between length change and score change, reported as interesting rather than as a confound. Dead OLS/HC1 code deleted (28 lines). **r(words) now computed** — pulled the 126 grade records off arch-home over ssh (stage 3 `wf_b9bbcff8-1a7`, stage 4 `wf_7516f16c-386`) and cached per-dimension correlations. **The length story dies on the data, not the statistics:** justification tracks length at **r = +0.33** in stage 4 but **r = −0.24** in stage 3 — longer rewrites justify *less* there — and stage 3 is the better-identified arm (deltas both directions, sd 38, vs stage 4's one-sided 14). Neither significant (p = 0.11, 0.15). A sign that flips between two arms of one design is not a relationship. |
| 2026-08-12 | **S10 review response, M3** (ladder arm) — §4.7's pooled alignment effect. | **+0.88 was roughly double its honest value; corrected to +0.42.** Control candidates were drawn only from words the base model had emitted, so a control absent from base output was structurally impossible — while **57 of 212 style words (27%) are absent from base output** and carry mean pref **+1.45**. Controls were also bucketed on the ratio's own denominator, dragging the control median to −0.45 against ~0 symmetric. Fed 30 random frequency-matched lists the old procedure returned **+0.45** — a pedestal for lists with no special property; corrected returns +0.003. **The effect is real**: style words are instruct-only 57× vs base-only 20×, where matched controls sit 38.7 vs 36.6 (**+3.7 sd**), and +0.42 is **6.7 sd** above its own null. Also replaced the impossible per-family CIs (+1.04 [+1.05,+1.31], +0.65 [+0.67,+0.99] — point estimates outside their own intervals, from an unreconstructible run) with Qwen3 **+0.47** [+0.34,+0.59] and Mistral **+0.16** [+0.05,+0.27]. **New, beyond the review:** the 57 alignment-created words carry only **1.7%** of the style list's Hansard token mass, and TEST B already finds the alignment score does not predict which words rose — so the finding is about the *word list*, not the measurement. **Then Matthew challenged the conclusion and was right.** The generation corpus is only ~90k words per model (800 prompts x 45w, 180 new tokens), so 146 of 212 style words sit at base count <= 4 and cannot be measured. Stratified: excess **+1.49** for the 57 unmeasurable words (1.7% of Hansard mass), **-0.06** at counts 1-4, **+0.16** at 5-19, and **+0.21** for the 31 well-measured words carrying **84.9%** of the mass. So my "the alignment finding doesn't carry the Hansard measurement" was wrong — it does, at +0.21. The ladder is **+0.88 -> +0.42 -> +0.21**: asymmetric controls inflate once, the unmeasurable tail again. Also found: `llama3_instruct.json` was never generated, so the figure rests on 2 families not 3. Regeneration at 20x scale registered as task #20. |
| 2026-08-12 | **S10 review response, M4** (bypass arm) — the per-target denominator. | **22.5%, not 24.6%; 9.8×, not 10.6×.** The denominator counted targets that produced a variant clearing the search's own submission gate (Opus proxy < 50), not targets attacked. **Six of 71 ran four to six full rounds and never produced anything submittable** — attack failures, so they belong in the denominator; counting only yielders is the completers-only version of the rate. Corrected: NB 10/40 = 25.0%, GO 6/31 = 19.4%, pooled **16/71 = 22.5%** [14.4, 33.5], strict 21.1%. Appendix C.4's seed counts were wrong the same way (38/25/27 → 40/35/31); the Opus-selected run understated worst, 10 of 35 yielding nothing. **New result reported rather than hidden:** 16 of 106 searched targets (15%) yielded nothing across all three searches — the study's only direct evidence of text the attack could not move, though weak, since the four GO near-misses sit at 50.0–51.7 against a gate of <50 with re-score sd ~2.4. **Cannot be settled:** the gate was applied before persistence, so those variants' text was discarded — the counterfactual was destroyed, not merely unmeasured. §8's argument is untouched; 9.8× is still an order of magnitude. |
| 2026-08-12 | **S10 review response, M5** (bypass arm) — per-variant intervals assumed independence. | The 461 variants are up to 18 rewrites of each of **92** texts, and success clusters by text — which is what the arm's own band analysis asserts. Wilson treats each rewrite as an independent trial: cluster-robust SE **2.44pp** against naive **1.30pp**, **design effect 3.55**, effective n ~130 not 461. Intervals roughly double: **8.5% [6.3, 11.4] → [4.0, 13.4]**, strict **8.0% [5.9, 10.9] → [3.6, 12.9]**, AI-restricted **[6.1, 11.6] → [3.5, 14.2]**. Now a non-parametric cluster bootstrap over texts (20k draws). Consequence for §4.9's comparison table, now stated: the adversarial vendor figures (2.31–2.86%) sit below our point estimate but **not far below its lower bound** — the ordering holds, the margin is less certain than the point estimates suggested. Per-target rates need no correction (one observation per text by construction). The `wilson()` and `fisher()` code was correct; the defect was the independence assumption. **Implied ICC 0.64** — ~2/3 of the variation in whether a rewrite succeeds is *between texts*, not between rewrites of one text. That is the band hypothesis restated as a variance decomposition, and it says precision comes from **more source texts, not more attempts per text**. |
| 2026-08-12 | **S10 corpus audit — what the study can see, and in what units** (`corpus_audit.py`). Matthew's call: **word-weighted is primary**, because segments are our construct — `segment.py` greedily packs speaker turns into ≤360-word windows, so a segment-weighted rate partly measures our packer. | Over 853.8M words: **long band 82.3% of words [scored], short band 10.7% [scoring now], and 7.0% OUTSIDE ANY BAND** — 3.5% under 50 words (Pangram refuses it outright, so unmeasurable not unmeasured), 1.8% translated, 1.7% over 360. **Implied error bar now stated: a rate r on the reachable 93.0% bounds the truth to [0.930r, 0.930r + 0.070]; at 12.4% that is [11.5%, 18.5%]**, and no sampling narrows it. **Two exclusions bias opposite ways** — short band up, over-360 down — and the second is almost entirely American: **US Senate 11.4% of words over 360, US House 7.7%, everywhere else <1%**, because the US extractor skips the 360-word packer. US Senate reports the study's lowest rate (3.3%) while 11.4% of its record sits in the excluded band where rates should be highest; flagged as not a clean chamber comparison. Also: 16% of short-band segments are packer offcuts of longer turns, not whole utterances — split the rate by origin when scoring completes. |
